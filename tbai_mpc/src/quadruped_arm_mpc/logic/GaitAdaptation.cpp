...
// anymal ctrl ocs2
#include "tbai_mpc/quadruped_arm_mpc/logic/GaitAdaptation.h"

#include "tbai_mpc/quadruped_arm_mpc/core/MotionPhaseDefinition.h"
#include "tbai_mpc/quadruped_arm_mpc/logic/SingleLegLogic.h"
...
GaitAdaptation::GaitAdaptation(const GaitAdaptationSettings &settings, const contact_flag_t &measuredContactFlags)
    : timeUntilNextTouchDownPerLeg_(constantFeetArray<scalar_t>(0.0)),
      ...
      hasLiftedSinceLastContact_(
          constantFeetArray<bool>(true)),  // initialize at true to enable immediate early touchdown
      settings_(settings) {}
...
void GaitAdaptation::advanceSwingEvents(const GaitSchedule &gaitSchedule) {
    ...
    const auto contactTimingsPerLeg = tbai::mpc::quadruped_arm::extractContactTimingsPerLeg(modeSchedule);
...
std::function<void(scalar_t &currentPhase, tbai::mpc::quadruped_arm::Gait &currentGait, scalar_t currTime,
                   tbai::mpc::quadruped_arm::Gait &nextGait)>
earlyTouchDownAdaptation(const tbai::mpc::quadruped_arm::feet_array_t<bool> &earlyTouchDownPerLeg) {
    return [=](scalar_t &currentPhase, Gait &currentGait, scalar_t currTime, Gait &nextGait) {
        ...
        for (unsigned int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
            if (earlyTouchDownPerLeg[leg]) {
                const int modeIndexOfNextContact =
                    getModeIndexOfNextContactStateOfLeg(true, currentModeIndex, leg, currentGait);
...
void setContactStateOfLegBetweenModes(bool contact, int startModeIdx, int lastModeIdx, size_t leg, Gait &gait) {
    for (int modeIndex = startModeIdx; modeIndex <= lastModeIdx; ++modeIndex) {
        auto stanceLegs = tbai::mpc::quadruped_arm::modeNumber2StanceLeg(gait.modeSequence[modeIndex]);
        stanceLegs[leg] = contact;
        gait.modeSequence[modeIndex] = tbai::mpc::quadruped_arm::stanceLeg2ModeNumber(stanceLegs);
    }
}
