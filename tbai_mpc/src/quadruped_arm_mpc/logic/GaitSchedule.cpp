...
#include "tbai_mpc/quadruped_arm_mpc/logic/GaitSwitching.h"
...
void GaitSchedule::adaptCurrentGait(const std::function<void(scalar_t &currentPhase, Gait &currentGait,
                                                             scalar_t currTime, Gait &nextGait)> &gaitAdaptor) {
    // Repeat the last gait until we have 3 gaits in the schedule. This prevents that the gaitAdaptor modifies the
    // nominal gait.
...
ocs2::ModeSchedule GaitSchedule::getModeSchedule(scalar_t timeHorizon) const {
    return ::tbai::mpc::quadruped_arm::getModeSchedule(phase_, time_, timeHorizon, gaitSchedule_.begin(), gaitSchedule_.end());
}
    ...
    while (tGaitEnd <= time) {
        if (std::next(gaitIt) == gaitSchedule_.end()) {
            ...
            gaitIt = std::prev(gaitSchedule_.end(),
                               2);  // Iterator can be invalidated after push_back, so reset set explicitly.
        }
