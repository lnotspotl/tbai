...
FootNormalConstraintMatrix computeFootNormalConstraint(const vector3_t &surfaceNormalInWorld,
    ...
    footNormalConstraint.constant =
        -surfaceNormalInWorld.dot(feedforwardVelocityInWorld + positionGain * feedforwardPositionInWorld);
...
vector_t FootNormalConstraint::getValue(scalar_t time, const vector_t &state, const vector_t &input,
    ...
    const auto &normalConstraint = computeFootNormalConstraint(
        switchedModelPreComp.getSurfaceNormalInOriginFrame(legNumber_), motionReference.footVelocity[legNumber_],
    ...
    vector_t h(1);
    h[0] = normalConstraint.positionMatrix.dot(o_footPosition) + normalConstraint.velocityMatrix.dot(o_footVelocity) +
...
VectorFunctionLinearApproximation FootNormalConstraint::getLinearApproximation(
    ...
    constraint.f[0] = normalConstraint.positionMatrix.dot(o_footPosition) +
                      normalConstraint.velocityMatrix.dot(o_footVelocity) + normalConstraint.constant;
