...
#include "tbai_mpc/quadruped_arm_mpc/cost/LinearStateInequalitySoftconstraint.h"
...
ScalarFunctionQuadraticApproximation getQuadraticApproximation(const LinearStateInequalitySoftConstraint &constraints,
    ...
    const auto penaltyDerivatives =
        h.unaryExpr([&](scalar_t hi) { return constraints.penalty->getDerivative(0.0, hi); });
    ...
    const auto penaltySecondDerivatives =
        h.unaryExpr([&](scalar_t hi) { return constraints.penalty->getSecondDerivative(0.0, hi); });
...
ScalarFunctionQuadraticApproximation getQuadraticApproximation(
    const SingleLinearStateInequalitySoftConstraint &constraint, const vector_t &f, const matrix_t &dfdx) {
    ScalarFunctionQuadraticApproximation cost;
