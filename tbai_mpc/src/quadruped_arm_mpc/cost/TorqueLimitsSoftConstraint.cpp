...
TorqueLimitsSoftConstraint::TorqueLimitsSoftConstraint(const joint_coordinate_t &torqueLimits,
    ...
    // Obtain the offset at zero joint angles. Just to compensate high negative costs when being far away from an
...
scalar_t TorqueLimitsSoftConstraint::getValue(const joint_coordinate_t &jointTorques) const {
    ...
    return upperBoundTorqueOffset.unaryExpr([&](scalar_t hi) { return jointTorquePenalty_->getValue(0.0, hi); }).sum() +
           lowerBoundTorqueOffset.unaryExpr([&](scalar_t hi) { return jointTorquePenalty_->getValue(0.0, hi); }).sum() +
           offset_;
}
...
ScalarFunctionQuadraticApproximation TorqueLimitsSoftConstraint::getQuadraticApproximation(
    ...
    cost.f =
        upperBoundTorqueOffset.unaryExpr([&](scalar_t hi) { return jointTorquePenalty_->getValue(0.0, hi); }).sum() +
        lowerBoundTorqueOffset.unaryExpr([&](scalar_t hi) { return jointTorquePenalty_->getValue(0.0, hi); }).sum() +
        offset_;
