...
#include "tbai_mpc/quadruped_arm_mpc/quadruped_models/DynamicsHelpers.h"
...
template <typename SCALAR_T>
tbai::mpc::quadruped_arm::vector3_s_t<SCALAR_T> QuadrupedCom<SCALAR_T>::centerOfMassInBaseFrame(
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointPositions) const {
    auto &data = pinocchioInterfacePtr_->getData();
    const auto &model = pinocchioInterfacePtr_->getModel();
    const auto configuration =
        getPinnochioConfiguration(tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T>::Zero(), jointPositions);
...
template <typename SCALAR_T>
tbai::mpc::quadruped_arm::vector3_s_t<SCALAR_T> QuadrupedCom<SCALAR_T>::centerOfMassInWorldFrame(
    const tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> &basePose,
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointPositions) const {
    auto &data = pinocchioInterfacePtr_->getData();
    const auto &model = pinocchioInterfacePtr_->getModel();
...
template <typename SCALAR_T>
tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> QuadrupedCom<SCALAR_T>::calculateBaseLocalAccelerations(
    const tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> &basePose,
    const tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> &baseLocalVelocities,
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointPositions,
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointVelocities,
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointAccelerations,
    const tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> &forcesOnBaseInBaseFrame) const {
    ...
    tbai::mpc::quadruped_arm::base_coordinate_s_t<SCALAR_T> pinocchioBaseForces;
    // Force
    ...
    tbai::mpc::quadruped_arm::vector6_s_t<SCALAR_T> baseForcesInBaseFrame = pinocchioBaseForces - dynamicBias.head(6);
    baseForcesInBaseFrame.noalias() -=
        data.M.template block<6, tbai::mpc::quadruped_arm::JOINT_COORDINATE_SIZE>(0, 6) * pinocchioMapping_.getPinocchioJointVector(jointAccelerations);

    // M are symmetric but pinocchio only fills in the upper triangle.
    tbai::mpc::quadruped_arm::matrix6_s_t<SCALAR_T> Mb = data.M.topLeftCorner(6, 6).template selfadjointView<Eigen::Upper>();
...
Eigen::Matrix<SCALAR_T, Eigen::Dynamic, 1> QuadrupedCom<SCALAR_T>::getPinnochioConfiguration(
    ...
    // basePost
    configuration.template head<3>() = tbai::mpc::quadruped_arm::getPositionInOrigin(basePose);
    // baseQuad
    const Eigen::Quaternion<SCALAR_T> baseQuat =
        tbai::mpc::quadruped_arm::quaternionBaseToOrigin<SCALAR_T>(tbai::mpc::quadruped_arm::getOrientation(basePose));
    configuration.template segment<4>(3) = baseQuat.coeffs();
    // JointsPos
    configuration.template segment<tbai::mpc::quadruped_arm::JOINT_COORDINATE_SIZE>(7) =
...
Eigen::Matrix<SCALAR_T, Eigen::Dynamic, 1> QuadrupedCom<SCALAR_T>::getPinnochioVelocity(
    ...
    velocity.template head<3>() = tbai::mpc::quadruped_arm::getLinearVelocity(baseLocalVelocities);
    // Base angular velocity in Base frame
    velocity.template segment<3>(3) = tbai::mpc::quadruped_arm::getAngularVelocity(baseLocalVelocities);
    // Joint velocity
    velocity.template segment<tbai::mpc::quadruped_arm::JOINT_COORDINATE_SIZE>(6) =
...
template class tbai::mpc::quadruped_arm::tpl::QuadrupedCom<ocs2::scalar_t>;
template class tbai::mpc::quadruped_arm::tpl::QuadrupedCom<ocs2::ad_scalar_t>;
