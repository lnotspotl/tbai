...
QuadrupedPinocchioMapping::QuadrupedPinocchioMapping(const FrameDeclaration &frameDeclaration,
    ...
    for (int i = 0; i < tbai::mpc::quadruped_arm::NUM_CONTACT_POINTS; ++i) {
    ...
    // Arm end-effector frame
    if (!frameDeclaration.arm.tip.empty()) {
        armEEFrameId_ = getBodyId(frameDeclaration.arm.tip, pinocchioInterface);
    }
}

namespace {
template <typename SCALAR_T>
tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> getPinocchioJointVectorImpl(
    const tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> &jointPositions,
    const tbai::mpc::quadruped_arm::feet_array_t<size_t> &mapFeetOrderOcs2ToPinocchio) {
    tbai::mpc::quadruped_arm::joint_coordinate_s_t<SCALAR_T> pinocchioJointPositions;
    // OCS2 LF (leg joints)
    pinocchioJointPositions.template segment<3>(3 * mapFeetOrderOcs2ToPinocchio[0]) =
        jointPositions.template segment<3>(0);
    // OCS2 RF
    pinocchioJointPositions.template segment<3>(3 * mapFeetOrderOcs2ToPinocchio[1]) =
        jointPositions.template segment<3>(3);
    // OCS2 LH
    pinocchioJointPositions.template segment<3>(3 * mapFeetOrderOcs2ToPinocchio[2]) =
        jointPositions.template segment<3>(6);
    // OCS2 RH
    pinocchioJointPositions.template segment<3>(3 * mapFeetOrderOcs2ToPinocchio[3]) =
...
tbai::mpc::quadruped_arm::joint_coordinate_t QuadrupedPinocchioMapping::getPinocchioJointVector(
    const tbai::mpc::quadruped_arm::joint_coordinate_t &jointPositions) const {
    return getPinocchioJointVectorImpl(jointPositions, mapFeetOrderOcs2ToPinocchio_);
}

tbai::mpc::quadruped_arm::joint_coordinate_ad_t QuadrupedPinocchioMapping::getPinocchioJointVector(
    const tbai::mpc::quadruped_arm::joint_coordinate_ad_t &jointPositions) const {
    return getPinocchioJointVectorImpl(jointPositions, mapFeetOrderOcs2ToPinocchio_);
}

size_t QuadrupedPinocchioMapping::getBodyId(const std::string &bodyName,
                                            const ocs2::PinocchioInterface &pinocchioInterface) {
