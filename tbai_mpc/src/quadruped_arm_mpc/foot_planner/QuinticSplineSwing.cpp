...
#include "tbai_mpc/quadruped_arm_mpc/foot_planner/QuinticSplineSwing.h"
...
void addStartVelocityConstraint(scalar_t v, scalar_t dt, Eigen::SparseMatrix<scalar_t, Eigen::RowMajor> &A, vector_t &b,
                                int constraintIdx, int splineIdx) {
    A.insert(constraintIdx, splineIdx + 4) = 1.0;
    b(constraintIdx) = v * dt;
}

void addEndVelocityConstraint(scalar_t v, scalar_t dt, Eigen::SparseMatrix<scalar_t, Eigen::RowMajor> &A, vector_t &b,
                              int constraintIdx, int splineIdx) {
...
void addStartAccelerationConstraint(scalar_t a, scalar_t dt, Eigen::SparseMatrix<scalar_t, Eigen::RowMajor> &A,
                                    vector_t &b, int constraintIdx, int splineIdx) {
    A.insert(constraintIdx, splineIdx + 3) = 2.0;
...
void addAccelerationContinuity(scalar_t dt_lhs, scalar_t dt_rhs, Eigen::SparseMatrix<scalar_t, Eigen::RowMajor> &A,
                               vector_t &b, int constraintIdx, int splineIdx_lhs, int splineIdx_rhs) {
...
void addJerkContinuity(scalar_t dt_lhs, scalar_t dt_rhs, Eigen::SparseMatrix<scalar_t, Eigen::RowMajor> &A, vector_t &b,
...
QuinticSwing::QuinticSwing(const std::vector<SwingNode> &nodes) {
    ...
    for (int splineNum = 0; splineNum < numSplines; ++splineNum) {
        ...
        const scalar_t dt = nodes[splineNum + 1].time - nodes[splineNum].time;
        splines.push_back(
            QuinticSpline(coefficients.segment<numVariablesPerSpline>(splineIdx), nodes[splineNum].time, dt));
