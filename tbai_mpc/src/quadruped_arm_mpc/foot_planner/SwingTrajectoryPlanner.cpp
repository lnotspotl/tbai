...
#include "tbai_mpc/quadruped_arm_mpc/foot_planner/KinematicFootPlacementPenalty.h"
    ...
    for (int leg = 0; leg < NUM_CONTACT_POINTS; leg++) {
        ...
        if (!contactTimings.empty()) {
            if (startsWithStancePhase(contactTimings)) {
                ...
                // If currently in swing -> verify that liftoff was before the horizon. If not, assume liftoff happened
                // exactly at initTime
        ...
        heuristicFootholdsPerLeg_[leg] =
            selectHeuristicFootholds(leg, contactTimings, targetTrajectories, initTime, currentState, finalTime);

        // Select terrain constraints based on the heuristic footholds.
        nominalFootholdsPerLeg_[leg] =
            selectNominalFootholdTerrain(leg, contactTimings, heuristicFootholdsPerLeg_[leg], targetTrajectories,
...
std::pair<std::vector<scalar_t>, std::vector<std::unique_ptr<FootPhase>>>
SwingTrajectoryPlanner::extractSwingTrajectoriesFromReference(int leg, const std::vector<ContactTiming> &contactTimings,
                                                              scalar_t finalTime) const {
    std::vector<scalar_t> eventTimes;
    std::vector<std::unique_ptr<FootPhase>> footPhases;
...
std::unique_ptr<ExternalSwingPhase> SwingTrajectoryPlanner::extractExternalSwingPhase(int leg, scalar_t liftOffTime,
    ...
    const auto touchdownIndex =
        ocs2::LinearInterpolation::timeSegment(touchDownTime, targetTrajectories_.timeTrajectory);

    // liftoff
    if (liftOffTime < targetTrajectories_.timeTrajectory[liftoffIndex.first + 1]) {
        const vector_t state =
            ocs2::LinearInterpolation::interpolate(liftoffIndex, targetTrajectories_.stateTrajectory);
        const vector_t input =
            ocs2::LinearInterpolation::interpolate(liftoffIndex, targetTrajectories_.inputTrajectory);
        time.push_back(liftOffTime);
        positions.push_back(
            kinematicsModel_->footPositionInOriginFrame(leg, getBasePose(state), getJointPositions(state)));
        velocities.push_back(
            kinematicsModel_->footVelocityInOriginFrame(leg, getBasePose(state), getBaseLocalVelocities(state),
    ...
    for (int k = liftoffIndex.first + 1; k < touchdownIndex.first; ++k) {
        ...
        velocities.push_back(kinematicsModel_->footVelocityInOriginFrame(
            leg, getBasePose(state), getBaseLocalVelocities(state), getJointPositions(state),
    ...
    if (time.back() < touchDownTime) {
        const vector_t state =
            ocs2::LinearInterpolation::interpolate(touchdownIndex, targetTrajectories_.stateTrajectory);
        const vector_t input =
            ocs2::LinearInterpolation::interpolate(touchdownIndex, targetTrajectories_.inputTrajectory);
...
std::vector<vector3_t> SwingTrajectoryPlanner::selectHeuristicFootholds(
    int leg, const std::vector<ContactTiming> &contactTimings, const ocs2::TargetTrajectories &targetTrajectories,
    scalar_t initTime, const comkino_state_t &currentState, scalar_t finalTime) const {
    ...
    const auto initBaseLinearVelocityInWorld =
        rotateVectorBaseToOrigin(getLinearVelocity(initBaseTwistInBase), initBaseOrientation);
    ...
    const base_coordinate_t initDesiredBaseTwistInBase =
        initDesiredState.segment<BASE_COORDINATE_SIZE>(BASE_COORDINATE_SIZE);
...
std::vector<ConvexTerrain> SwingTrajectoryPlanner::selectNominalFootholdTerrain(
    int leg, const std::vector<ContactTiming> &contactTimings, const std::vector<vector3_t> &heuristicFootholds,
    const ocs2::TargetTrajectories &targetTrajectories, scalar_t initTime, const comkino_state_t &currentState,
            ...
            if (timeTillContact < settings_.previousFootholdTimeDeadzone && previousIterationContact != nullptr) {
                ...
                if (previousIterationContact != nullptr) {
                    referenceFootholdPositionInWorld = filterFoothold(
                        referenceFootholdPositionInWorld, previousIterationContact->nominalFootholdLocation());
                }

                // Kinematic penalty
                const base_coordinate_t basePoseAtTouchdown =
                    getBasePose(targetTrajectories.getDesiredState(contactPhase.start));
                const auto hipPositionInWorldTouchdown =
                    kinematicsModel_->legRootInOriginFrame(leg, basePoseAtTouchdown);
                const auto hipOrientationInWorldTouchdown =
                    kinematicsModel_->orientationLegRootToOriginFrame(leg, basePoseAtTouchdown);
                const base_coordinate_t basePoseAtLiftoff =
                    getBasePose(targetTrajectories.getDesiredState(contactEndTime));
                const auto hipPositionInWorldLiftoff = kinematicsModel_->legRootInOriginFrame(leg, basePoseAtLiftoff);
                const auto hipOrientationInWorldLiftoff =
                    kinematicsModel_->orientationLegRootToOriginFrame(leg, basePoseAtLiftoff);
                ...
                if (contactPhase.start < finalTime) {
                    ...
                    ConvexTerrain convexTerrain;
                    convexTerrain.plane = terrainModel.getLocalTerrainAtPositionInWorldAlongGravity(
                        referenceFootholdPositionInWorld, scoringFunction);
                    nominalFootholdTerrain.push_back(convexTerrain);
                    ++heuristicFootholdIt;
                }
...
void SwingTrajectoryPlanner::subsampleReferenceTrajectory(const ocs2::TargetTrajectories &targetTrajectories,
    ...
    // Add first reference
    {
        const auto initInterpIndex =
            ocs2::LinearInterpolation::timeSegment(initTime, targetTrajectories.timeTrajectory);
    ...
    for (int k = 0; k < targetTrajectories.timeTrajectory.size(); ++k) {
        if (targetTrajectories.timeTrajectory[k] < initTime) {
            ...
            const auto finalInterpIndex =
                ocs2::LinearInterpolation::timeSegment(finalTime, targetTrajectories.timeTrajectory);
        ...
        while (targetTrajectories_.timeTrajectory.back() + settings_.maximumReferenceSampleTime <
               targetTrajectories.timeTrajectory[k]) {
...
void SwingTrajectoryPlanner::adaptJointReferencesWithInverseKinematics(scalar_t finalTime) {
    ...
    for (int k = 0; k < targetTrajectories_.timeTrajectory.size(); ++k) {
        ...
        for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
            ...
            const vector3_t positionBaseToFootInBaseFrame =
                rotateVectorOriginToBase(positionBaseToFootInWorldFrame, eulerXYZ);
            ...
            const vector3_t footVelocityInBaseFrame =
                rotateVectorOriginToBase(footPhase.getVelocityInWorld(t), eulerXYZ);
            const vector3_t footRelativeVelocityInBaseFrame =
                footVelocityInBaseFrame - getLinearVelocity(baseTwistInBaseFrame) -
...
SwingTrajectoryPlannerSettings loadSwingTrajectorySettings(const std::string &filename, bool verbose) {
    ...
    ocs2::loadData::loadPtreeValue(pt, settings.previousFootholdTimeDeadzone, prefix + "previousFootholdTimeDeadzone",
                                   verbose);
    ...
    ocs2::loadData::loadPtreeValue(pt, settings.maximumReferenceSampleTime, prefix + "maximumReferenceSampleTime",
                                   verbose);
    ocs2::loadData::loadPtreeValue(pt, settings.swingTrajectoryFromReference, prefix + "swingTrajectoryFromReference",
                                   verbose);
