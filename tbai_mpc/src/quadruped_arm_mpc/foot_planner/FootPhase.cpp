...
#include "tbai_mpc/quadruped_arm_mpc/foot_planner/FootPhase.h"

#include "tbai_mpc/quadruped_arm_mpc/foot_planner/CubicSpline.h"
...
FootTangentialConstraintMatrix tangentialConstraintsFromConvexTerrain(const ConvexTerrain &stanceTerrain,
    ...
    if (numberOfConstraints > 0) {
        for (int i = 0; i < stanceTerrain.boundary.size(); ++i) {
            ...
            vector2_t A_terrain{
                -start2end.y(),
                start2end.x()};     // rotate 90deg around z to get a vector pointing to interior of the polygon
            A_terrain.normalize();  // make it a unit vector.
            scalar_t b_terrain =
                -A_terrain.dot(stanceTerrain.boundary[i]) - margin;  // Take offset of one of the points
...
void SwingPhase::setFullSwing(const SwingProfile &swingProfile, const TerrainModel *terrainModel) {
    ...
    if (swingVector.head<2>().norm() > 0.01) {
        // Get a unit vector perpendicular to the swing vector and in the plane formed by the World-Z axis and the swing
        // vector.
        const vector3_t swingTrajectoryNormal =
            swingVector.cross(vector3_t(0.0, 0.0, 1.0).cross(swingVector)).normalized();

        // Terrain adaptation if information is available
        scalar_t maxObstacleHeight = 0.0;
        if (terrainModel != nullptr) {
            const auto heightProfile =
                terrainModel->getHeightProfileAlongLine(liftOffPositionInWorld, touchDownPositionInWorld);

            // Find the maximum obstacle. An obstacle is a height point sticking out above the line between liftoff and
            // touchdown.
            ...
            // Clip max obstacle to two times swing height to protect against outliers. (Total swing height will be max
            // 3 * swing height)
        ...
        for (const auto &node : swingProfile.nodes) {
            ...
            midPoint.velocity = node.tangentialVelocityFactor / swingDuration * swingVector +
                                node.normalVelocity * swingTrajectoryNormal;
        ...
        for (const auto &node : swingProfile.nodes) {
            ...
            midPoint.position.z() =
                std::max(liftOffPositionInWorld.z(), touchDownPositionInWorld.z()) + node.swingHeight;
...
void SwingPhase::setHalveSwing(const SwingProfile &swingProfile, const TerrainModel *terrainModel) {
    ...
    const vector3_t touchDownPositionInWorld =
        liftOffPositionInWorld + vector3_t(0.0, 0.0, swingProfile.nodes.back().swingHeight);
...
vector3_t SwingPhase::normalDirectionInWorldFrame(scalar_t time) const {
    ...
    return ((1.0 - scaling) * surfaceNormalInWorld(*liftOff_.terrainPlane) +
            scaling * surfaceNormalInWorld(*touchDown_.terrainPlane))
        .normalized();
}
