...
SwitchedModelPreComputation::SwitchedModelPreComputation(const SwingTrajectoryPlanner &swingTrajectoryPlanner,
    ...
    intermediateLinearOutputAdInterface_.reset(new ocs2::CppAdInterface(
        intermediateDiffFunc, STATE_DIM + INPUT_DIM, intermediateLibName, settings.autodiffLibraryFolder_));
...
void SwitchedModelPreComputation::updateMotionReference(scalar_t t) {
    ...
    if (uRef.head<3 * NUM_CONTACT_POINTS>().isZero()) {
        uRef.head<3 * NUM_CONTACT_POINTS>() =
            weightCompensatingInputs(robotMass_, contactFlags_, eulerAngles).head<3 * NUM_CONTACT_POINTS>();
    }
    ...
    // Arm joint reference - use the reference state's arm positions (neutral/stowed position)
    const auto qArmRef = getArmJointPositions(stateReference_);
    const auto dqArmRef = getArmJointVelocities(uRef);
    motionReference_.armJointPosition = qArmRef;
    motionReference_.armJointVelocity = dqArmRef;

    // Arm EE reference - hardcoded desired position in world frame
    motionReference_.armEEPosition << 0.6, 0.0, 0.5;
    motionReference_.armEEOrientation.setZero();
    motionReference_.armEEVelocity.setZero();
}
...
void SwitchedModelPreComputation::intermediateLinearOutputs(const ad_com_model_t &adComModel,
    ...
    const leg_joint_coordinate_ad_t qLegJoints = getLegJointPositions(x);
    const joint_coordinate_ad_t dqJoints = getJointVelocities(u);
    const feet_array_t<vector3_ad_t> contactForcesInBase = contactForcesToArray(u.head<3 * NUM_CONTACT_POINTS>());

    const auto o_feetPositionsAsArray = adKinematicsModel.feetPositionsInOriginFrame(basePose, qJoints);
    const auto o_footVelocitiesAsArray =
        adKinematicsModel.feetVelocitiesInOriginFrame(basePose, baseTwist, qJoints, dqJoints);
    const auto legJointTorques = torqueApproximation(qLegJoints, contactForcesInBase, adKinematicsModel);
    const auto o_collisions = adKinematicsModel.collisionSpheresInOriginFrame(basePose, qJoints);

    const int numberOfOutputs =
        3 * NUM_CONTACT_POINTS + 3 * NUM_CONTACT_POINTS + JOINT_COORDINATE_SIZE + 3 * o_collisions.size();
...
void SwitchedModelPreComputation::updateIntermediateLinearOutputs(scalar_t t, const vector_t &tapedStateInput) {
    ...
    // Read joint Torques
    jointTorques_ =
        intermediateLinearOutputs.segment<JOINT_COORDINATE_SIZE>(3 * NUM_CONTACT_POINTS + 3 * NUM_CONTACT_POINTS);

    // Read collision bodies (excluding the feet)
    for (int collisionIndex = 0; collisionIndex < collisionRadii_.size(); ++collisionIndex) {
        const int indexInOutputs =
            JOINT_COORDINATE_SIZE + 3 * (NUM_CONTACT_POINTS + NUM_CONTACT_POINTS + collisionIndex);
        const auto collisionGlobalId = NUM_CONTACT_POINTS + collisionIndex;
        collisionSpheresActive_[collisionGlobalId] = true;
        collisionSpheresInOriginFrame_[collisionGlobalId].position =
            intermediateLinearOutputs.segment<3>(indexInOutputs);
        collisionSpheresInOriginFrame_[collisionGlobalId].radius = collisionRadii_[collisionIndex];
    }
}

void SwitchedModelPreComputation::updateIntermediateLinearOutputDerivatives(scalar_t t,
                                                                            const vector_t &tapedStateInput) {
    // Evaluate autodiff
    const auto intermediateLinearOutputDerivatives = intermediateLinearOutputAdInterface_->getJacobian(tapedStateInput);

    for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
        const int indexInOutputs = 3 * leg;
        feetPositionInOriginFrameStateDerivative_[leg] =
            intermediateLinearOutputDerivatives.block<3, STATE_DIM>(indexInOutputs, 0);
        const auto &footPhase = getFootPhase(leg);
        collisionSpheresActive_[leg] = !footPhase.contactFlag();
        if (collisionSpheresActive_[leg]) {
    ...
    for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
        ...
        feetVelocitiesInOriginFrameDerivative_[leg].dfdx =
            intermediateLinearOutputDerivatives.block<3, STATE_DIM>(indexInOutputs, 0);
        feetVelocitiesInOriginFrameDerivative_[leg].dfdu =
            intermediateLinearOutputDerivatives.block<3, INPUT_DIM>(indexInOutputs, STATE_DIM);
    }

    // Read joint Torques
    jointTorquesDerivative_.f = jointTorques_;
    jointTorquesDerivative_.dfdx = intermediateLinearOutputDerivatives.block<JOINT_COORDINATE_SIZE, STATE_DIM>(
    ...
    for (int collisionIndex = 0; collisionIndex < collisionRadii_.size(); ++collisionIndex) {
        ...
        const auto collisionGlobalId = NUM_CONTACT_POINTS + collisionIndex;
        collisionSpheresActive_[collisionGlobalId] = true;
        collisionSpheresDerivative_[collisionGlobalId] =
            intermediateLinearOutputDerivatives.block<3, STATE_DIM>(indexInOutputs, 0);
    }
}
...
void SwitchedModelPreComputation::updatePrejumpLinearOutputDerivatives(scalar_t t, const vector_t &state) {
    ...
    for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
        const int indexInOutputs = 3 * leg;
        feetPositionInOriginFrameStateDerivative_[leg] =
            prejumpLinearOutputDerivatives.block<3, STATE_DIM>(indexInOutputs, 0);
    }
}
