...
QuadrupedPointfootInterface::QuadrupedPointfootInterface(const kinematic_model_t &kinematicModel,
                                                         ...
                                                         const InverseKinematicsModelBase *inverseKinematics,
                                                         Settings settings, std::vector<std::string> jointNames,
                                                         std::string baseName)
    ...
    // nominal values
    const auto stanceFlags = tbai::mpc::quadruped_arm::constantFeetArray(true);
    const auto uSystemForWeightCompensation =
        weightCompensatingInputs(getComModel(), stanceFlags, tbai::mpc::quadruped_arm::vector3_t::Zero());
    const auto legTorquesForWeightCompensation = torqueApproximation(
        getLegJointPositions(getInitialState()),
        contactForcesToArray(uSystemForWeightCompensation.head<3 * NUM_CONTACT_POINTS>()), kinematicModel);

    // Pad with zeros for arm joints (arm doesn't contribute to weight compensation)
    joint_coordinate_t jointTorquesForWeightCompensation;
    jointTorquesForWeightCompensation.head<LEG_JOINT_COORDINATE_SIZE>() = legTorquesForWeightCompensation;
    jointTorquesForWeightCompensation.tail<NUM_ARM_JOINTS>().setZero();
    ...
    auto lqrSolution =
        ocs2::continuous_time_lqr::solve(*problemPtr_, 0.0, getInitialState(), uSystemForWeightCompensation);
    ...
    nominalCostApproximation_ =
        ocs2::approximateCost(*problemPtr_, 0.0, getInitialState(), uSystemForWeightCompensation);
