...
Base2dReferenceTrajectory generate2DExtrapolatedBaseReference(const BaseReferenceHorizon &horizon,
                                                              const BaseReferenceState &initialState,
    ...
    for (int k = 1; k < horizon.N; ++k) {
        ...
        const auto commandedLinearVelocityInWorld = velocityCommand2dInWorld(
            command.headingVelocity, command.lateralVelocity, baseRef.yaw.back() + 0.5 * dt * command.yawRate);
...
BaseReferenceTrajectory generateExtrapolatedBaseReference(const BaseReferenceHorizon &horizon,
                                                          const BaseReferenceState &initialState,
                                                          const BaseReferenceCommand &command,
                                                          const TerrainPlane &projectedHeadingFrame) {
