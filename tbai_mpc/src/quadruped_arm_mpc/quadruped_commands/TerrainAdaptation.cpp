...
vector3_t adaptDesiredPositionHeightToTerrain(const vector3_t &desiredPosition, const TerrainPlane &terrainPlane,
    ...
    return adaptDesiredPositionHeightToTerrain(vector2_t{desiredPosition.x(), desiredPosition.y()}, terrainPlane,
                                               desiredHeight);
}

vector3_t adaptDesiredPositionHeightToTerrain(const vector2_t &desiredXYPosition, const TerrainPlane &terrainPlane,
                                              scalar_t desiredHeight) {
    const auto adaptedHeight =
        projectPositionInWorldOntoPlaneAlongGravity(desiredXYPosition, terrainPlane).z() + desiredHeight;
...
matrix3_t getOrientationProjectedHeadingFrameToWorld(const vector3_t &headingVector, const TerrainPlane &terrainPlane) {
    ...
    const vector3_t xAxisProjectedHeadingFrame =
        projectVectorInWorldOntoPlaneAlongGravity(headingVector, terrainPlane).normalized();
...
TerrainPlane getProjectedHeadingFrame(const vector3_t &eulerXYZ, const TerrainPlane &terrainPlane) {
    ...
    return {terrainPlane.positionInWorld,
            getOrientationProjectedHeadingFrameToWorld(xAxisInWorld, terrainPlane).transpose()};
}
