...
#include "tbai_mpc/quadruped_arm_mpc/dynamics/ComKinoSystemDynamicsAd.h"
...
base_coordinate_s_t<SCALAR_T> computeExternalForcesInBaseFrame(
    const KinematicsModelBase<SCALAR_T> &kinematicsModel, const comkino_state_s_t<SCALAR_T> &comKinoState,
    const comkino_input_s_t<SCALAR_T> &comKinoInput, const ComKinoSystemDynamicsParameters<SCALAR_T> &parameters) {
    ...
    // External forces
    const vector3_s_t<SCALAR_T> externalForceInBase =
        rotateVectorOriginToBase(parameters.externalForceInOrigin, baseEulerAngles);
...
ocs2::ad_vector_t ComKinoSystemDynamicsAd::systemFlowMap(ocs2::ad_scalar_t time, const ocs2::ad_vector_t &state,
    ...
    const com_state_ad_t comStateDerivative = computeComStateDerivative(
        *adComModelPtr_, *adKinematicModelPtr_, comkinoState, comkinoInput, ad_parameters_t(parameters));
...
template <typename SCALAR_T>
com_state_s_t<SCALAR_T> ComKinoSystemDynamicsAd::computeComStateDerivative(
    ...
    const base_coordinate_s_t<SCALAR_T> extForce =
    ...
    stateDerivativeCoM.segment(0, 3) =
        tbai::mpc::quadruped_arm::angularVelocitiesToEulerAngleDerivatives(baseLocalAngularVelocity, baseEulerAngles);
    ...
    stateDerivativeCoM.segment(6, 6) = comModel.calculateBaseLocalAccelerations(
        basePose, baseLocalVelocities, qJoints, qdJoints, joint_coordinate_s_t<SCALAR_T>::Zero(), extForce);
    return stateDerivativeCoM;
}

template com_state_t ComKinoSystemDynamicsAd::computeComStateDerivative(
    const ComModelBase<scalar_t> &comModel, const KinematicsModelBase<scalar_t> &kinematicsModel,
    const comkino_state_t &comKinoState, const comkino_input_t &comKinoInput, const parameters_t &parameters);
