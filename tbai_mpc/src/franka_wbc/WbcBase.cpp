...
WbcBase::WbcBase(const std::string &configFile, const std::string &urdfString,
    ...
    eeFrameId_ = model.getFrameId(frankaInfo_.eeFrame);

    // Get joint names
    jointNames_ = frankaInfo_.dofNames;
...
void WbcBase::loadSettings(const std::string &configFile, const std::string &configPrefix) {
    ...
    jointHomePosition_ << 0.0, -0.785, 0.0, -2.356, 0.0, 1.571, 0.785;  // Default Franka home
    ...
    scalar_t defaultTorqueLimit = 87.0;  // Default Franka torque limit
