...
#include "tbai_mpc/quadruped_arm_mpc/core/ModelSettings.h"
...
/**
 * Defines a cost over tracking base, feet, and arm end-effector references
 */
class MotionTrackingCost final : public ocs2::StateInputCostGaussNewtonAd {
 public:
  ...
  struct Weights {
    ...
    // Arm end-effector tracking weights (9D)
    vector3_t armEEPosition{500.0, 500.0, 500.0};
    vector3_t armEEOrientation{100.0, 100.0, 100.0};  // Axis-angle error weights
    vector3_t armEEVelocity{10.0, 10.0, 10.0};        // Linear velocity weights

    // Arm joint tracking weights (12D)
    arm_joint_t armJointPosition{arm_joint_t::Constant(1.0)};
    arm_joint_t armJointVelocity{arm_joint_t::Constant(0.01)};
  };
