...
/**
 * Switched model definition for quadruped with arm:
 *
 * state = [theta, p, w, v, q_legs (4x), q_arm (6x)]
 * theta: EulerXYZ (3x1)
 * p: Base position in Origin frame (3x1)
 * w: Base angular velocity in Base Frame (3x1)
 * v: Base linear velocity in Base Frame (3x1)
 * q_legs: Joint angles per leg [HAA, HFE, KFE] (3x1) [4x] = 12
 * q_arm: Arm joint angles [sh0, sh1, el0, el1, wr0, wr1] (6x1)
 *
 * input = [lambda (4x), qj_legs (4x), qj_arm (6x)]
 * lambda: Force at the EE [LF, RF, LH, RH] in Base Frame (3x1)
 * qj_legs: Leg joint velocities per leg [HAA, HFE, KFE] (3x1) [4x] = 12
 * qj_arm: Arm joint velocities (6x1)
 */

constexpr size_t NUM_CONTACT_POINTS = 4;
constexpr size_t BASE_COORDINATE_SIZE = 6;
constexpr size_t NUM_ARM_JOINTS = 6;
constexpr size_t LEG_JOINT_COORDINATE_SIZE = 12;
constexpr size_t JOINT_COORDINATE_SIZE = LEG_JOINT_COORDINATE_SIZE + NUM_ARM_JOINTS;  // 18
constexpr size_t STATE_DIM = 2 * BASE_COORDINATE_SIZE + JOINT_COORDINATE_SIZE;        // 30
constexpr size_t INPUT_DIM = 3 * NUM_CONTACT_POINTS + JOINT_COORDINATE_SIZE;          // 30

/* Import ocs2 types into the tbai::mpc::quadruped_arm namespace */
...
template <typename SCALAR_T>
using leg_joint_coordinate_s_t = Eigen::Matrix<SCALAR_T, LEG_JOINT_COORDINATE_SIZE, 1>;
using leg_joint_coordinate_t = leg_joint_coordinate_s_t<scalar_t>;
using leg_joint_coordinate_ad_t = leg_joint_coordinate_s_t<ocs2::CppAdInterface::ad_scalar_t>;

template <typename SCALAR_T>
using arm_joint_s_t = Eigen::Matrix<SCALAR_T, NUM_ARM_JOINTS, 1>;
using arm_joint_t = arm_joint_s_t<scalar_t>;
using arm_joint_ad_t = arm_joint_s_t<ocs2::CppAdInterface::ad_scalar_t>;
...
// Get all joint positions (legs + arm)
template <typename SCALAR_T>
...
// Get leg joint positions only (12D)
template <typename SCALAR_T>
leg_joint_coordinate_s_t<SCALAR_T> getLegJointPositions(const Eigen::Matrix<SCALAR_T, -1, 1>& comkinoState) {
  return comkinoState.template segment<LEG_JOINT_COORDINATE_SIZE>(2 * BASE_COORDINATE_SIZE);
}

template <typename SCALAR_T>
leg_joint_coordinate_s_t<SCALAR_T> getLegJointPositions(const comkino_state_s_t<SCALAR_T>& comkinoState) {
  return comkinoState.template segment<LEG_JOINT_COORDINATE_SIZE>(2 * BASE_COORDINATE_SIZE);
}

// Get arm joint positions (6D)
template <typename SCALAR_T>
arm_joint_s_t<SCALAR_T> getArmJointPositions(const Eigen::Matrix<SCALAR_T, -1, 1>& comkinoState) {
  return comkinoState.template segment<NUM_ARM_JOINTS>(2 * BASE_COORDINATE_SIZE + LEG_JOINT_COORDINATE_SIZE);
}

template <typename SCALAR_T>
arm_joint_s_t<SCALAR_T> getArmJointPositions(const comkino_state_s_t<SCALAR_T>& comkinoState) {
  return comkinoState.template segment<NUM_ARM_JOINTS>(2 * BASE_COORDINATE_SIZE + LEG_JOINT_COORDINATE_SIZE);
}

// Get all joint velocities (legs + arm)
template <typename SCALAR_T>
...
// Get leg joint velocities only (12D)
template <typename SCALAR_T>
leg_joint_coordinate_s_t<SCALAR_T> getLegJointVelocities(const Eigen::Matrix<SCALAR_T, -1, 1>& comkinoInput) {
  return comkinoInput.template segment<LEG_JOINT_COORDINATE_SIZE>(NUM_CONTACT_POINTS * 3);
}

template <typename SCALAR_T>
leg_joint_coordinate_s_t<SCALAR_T> getLegJointVelocities(const comkino_input_s_t<SCALAR_T>& comkinoInput) {
  return comkinoInput.template segment<LEG_JOINT_COORDINATE_SIZE>(NUM_CONTACT_POINTS * 3);
}

// Get arm joint velocities (6D)
template <typename SCALAR_T>
arm_joint_s_t<SCALAR_T> getArmJointVelocities(const Eigen::Matrix<SCALAR_T, -1, 1>& comkinoInput) {
  return comkinoInput.template segment<NUM_ARM_JOINTS>(NUM_CONTACT_POINTS * 3 + LEG_JOINT_COORDINATE_SIZE);
}

template <typename SCALAR_T>
arm_joint_s_t<SCALAR_T> getArmJointVelocities(const comkino_input_s_t<SCALAR_T>& comkinoInput) {
  return comkinoInput.template segment<NUM_ARM_JOINTS>(NUM_CONTACT_POINTS * 3 + LEG_JOINT_COORDINATE_SIZE);
}

// Convert full joint coordinates (18D) to feet array (uses first 12 elements only)
template <typename SCALAR_T>
...
// Convert any 12-element vector to feet array (for contact forces or leg joint coordinates)
template <typename Derived>
feet_array_t<vector3_s_t<typename Derived::Scalar>> contactForcesToArray(const Eigen::MatrixBase<Derived>& valuesAsVector) {
  static_assert(Derived::RowsAtCompileTime == 12 || Derived::RowsAtCompileTime == Eigen::Dynamic,
                "Expected 12-element vector for contact forces");
  return {valuesAsVector.template segment<3>(0), valuesAsVector.template segment<3>(3), valuesAsVector.template segment<3>(6),
          valuesAsVector.template segment<3>(9)};
}

// Convert feet array to 12D vector (for feet positions, velocities, or leg joint coordinates)
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3 * NUM_CONTACT_POINTS, 1> fromArray(const feet_array_t<vector3_s_t<SCALAR_T>>& valuesAsArray) {
  Eigen::Matrix<SCALAR_T, 3 * NUM_CONTACT_POINTS, 1> valuesAsVector;
