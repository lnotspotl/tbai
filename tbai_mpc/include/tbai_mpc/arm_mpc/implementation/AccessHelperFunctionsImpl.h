...
Eigen::Matrix<SCALAR, 3, 1> getBasePosition(const Eigen::Matrix<SCALAR, -1, 1>& state, const ArmModelInfo& info) {
...
template <typename SCALAR>
Eigen::Quaternion<SCALAR> getBaseOrientation(const Eigen::Matrix<SCALAR, -1, 1>& state, const ArmModelInfo& info) {
  assert(state.rows() == info.stateDim);
  assert(state.cols() == 1);
...
Eigen::Block<Derived, -1, 1> getArmJointAngles(Eigen::MatrixBase<Derived>& state, const ArmModelInfo& info) {
  assert(state.rows() == info.stateDim);
  assert(state.cols() == 1);
...
template <typename Derived>
const Eigen::Block<const Derived, -1, 1> getArmJointAngles(const Eigen::MatrixBase<Derived>& state, const ArmModelInfo& info) {
  assert(state.rows() == info.stateDim);
  assert(state.cols() == 1);
  const size_t startRow = info.stateDim - info.armDim;
