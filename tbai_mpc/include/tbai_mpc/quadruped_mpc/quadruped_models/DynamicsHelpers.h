...
tbai::mpc::quadruped::matrix3_s_t<SCALAR_T> inv33sym(const tbai::mpc::quadruped::matrix3_s_t<SCALAR_T>& A, SCALAR_T DinvMax = SCALAR_T(1e12)) {
  // Analytical inverse of symmetrical 3x3 matrix
  tbai::mpc::quadruped::matrix3_s_t<SCALAR_T> Ainv;
 ...
 */
template <typename SCALAR_T>
tbai::mpc::quadruped::vector3_s_t<SCALAR_T> solve33sym(const tbai::mpc::quadruped::matrix3_s_t<SCALAR_T>& A,
                                                 const tbai::mpc::quadruped::vector3_s_t<SCALAR_T>& b, SCALAR_T DinvMax = SCALAR_T(1e12)) {
  // Analytical inverse of symmetrical 3x3 matrix, stored in a flat array
  tbai::mpc::quadruped::vector6_s_t<SCALAR_T> Ainv;
  ...
  tbai::mpc::quadruped::vector3_s_t<SCALAR_T> c;
 ...
 */
template <typename SCALAR_T>
tbai::mpc::quadruped::vector6_s_t<SCALAR_T> inertiaTensorSolveLinearAngular(const tbai::mpc::quadruped::matrix6_s_t<SCALAR_T>& M,
                                                                      const tbai::mpc::quadruped::vector6_s_t<SCALAR_T>& b,
  ...
  const tbai::mpc::quadruped::vector3_s_t<SCALAR_T> scaledComVector{M(1, 5), M(2, 3), M(0, 4)};  // is actually m * comVector
  const tbai::mpc::quadruped::vector3_s_t<SCALAR_T> comVector = invm * scaledComVector;          // store a version that is pre-multiplied by 1/m

  // prepare rhs for solve33sym, tmp = b.tail(3) -  comVector x b.head(3)
  tbai::mpc::quadruped::vector3_s_t<SCALAR_T> tmp = b.template tail<3>() - comVector.template cross(b.template head<3>());

  // prepare lhs for solve33sym, fill only upper triangular part
  tbai::mpc::quadruped::matrix3_s_t<SCALAR_T> centroidalInertia;  // = theta_com = theta - 1/m * crossTerm * crossTerm'
  ...
  tbai::mpc::quadruped::vector6_s_t<SCALAR_T> c;
 ...
 */
template <typename SCALAR_T>
tbai::mpc::quadruped::vector6_s_t<SCALAR_T> inertiaTensorSolveAngularLinear(const tbai::mpc::quadruped::matrix6_s_t<SCALAR_T>& M,
                                                                      const tbai::mpc::quadruped::vector6_s_t<SCALAR_T>& b,
                                                                      SCALAR_T DinvMax = SCALAR_T(1e12)) {
  /*
  ...
  // extract underlying vector for the crossterms s.t. M.upperRight = crossTerm = [m * comVector]_x
  const tbai::mpc::quadruped::vector3_s_t<SCALAR_T> scaledComVector{M(2, 4), M(0, 5), M(1, 3)};  // is actually m * comVector
  const tbai::mpc::quadruped::vector3_s_t<SCALAR_T> comVector = invm * scaledComVector;          // store a version that is pre-multiplied by 1/m

  // prepare rhs for solve33sym, tmp = b.head(3) * crossTerm' * b.tail(3)
  tbai::mpc::quadruped::vector3_s_t<SCALAR_T> tmp = b.template head<3>() - comVector.template cross(b.template tail<3>());
