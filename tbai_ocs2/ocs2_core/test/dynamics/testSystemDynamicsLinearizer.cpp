...
static bool derivativeChecker(SystemDynamicsBase &nonlinearSystem, SystemDynamicsBase &reference, scalar_t tolerance,
...
TEST(testSystemDynamicsLinearizer, testLinearSystem) {
    ...
    SystemDynamicsLinearizer linearizedSys(std::unique_ptr<ControlledSystemBase>(linSys.clone()),
...
TEST(testSystemDynamicsLinearizer, testPendulum) {
    ...
    SystemDynamicsLinearizer linearizedSys(std::unique_ptr<ControlledSystemBase>(nonLinSys.clone()),
                                           /*doubleSidedDerivative=*/true,
                                           /*isSecondOrderSystem=*/false, EPSILON);
...
static bool derivativeChecker(SystemDynamicsBase &sys1, SystemDynamicsBase &sys2, scalar_t tolerance, scalar_t t,
