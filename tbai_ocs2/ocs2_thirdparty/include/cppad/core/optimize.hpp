...
void ADFun<Base, RecBase>::optimize(const std::string &options) {
    ...
    if (check_zero_order) {  // zero order coefficients for independent vars
        for (j = 0; j < n; j++) {
    ...
    switch (play_.address_type()) {
        case local::play::unsigned_short_enum:
            local::optimize::optimize_run<unsigned short>(options, n, dep_taddr_, &play_, &rec);
            break;

        case local::play::unsigned_int_enum:
            local::optimize::optimize_run<unsigned int>(options, n, dep_taddr_, &play_, &rec);
            break;

        case local::play::size_t_enum:
            local::optimize::optimize_run<size_t>(options, n, dep_taddr_, &play_, &rec);
            break;
    ...
    subgraph_info_.resize(ind_taddr_.size(),   // n_ind
    ...
    if (check_zero_order) {
        ...
        for (i = 0; i < m; i++)
            if (!abs_geq(eps99 * max_taylor, check[i] - y[i])) {
                ...
                msg += "\ny[i]     = " + to_string(y[i]);
                CPPAD_ASSERT_KNOWN(abs_geq(eps99 * max_taylor, check[i] - y[i]), msg.c_str());
            }
    ...
    std::cout << "optimize: size_op:  before = " << size_op_before << ", after = " << size_op_after << "\n";
#endif
}
