...
size_t ADFun<Base, RecBase>::sparse_jac_for(size_t group_max, const BaseVector &x,
                                            sparse_rcv<SizeVector, BaseVector> &subset,
    ...
    size_t n = Domain();
    //
    CPPAD_ASSERT_KNOWN(subset.nr() == m, "sparse_jac_for: subset.nr() not equal range dimension for f");
    CPPAD_ASSERT_KNOWN(subset.nc() == n, "sparse_jac_for: subset.nc() not equal domain dimension for f");
    //
    ...
    if (color.size() == 0) {  // compute work color and order vectors
        CPPAD_ASSERT_KNOWN(pattern.nr() == m, "sparse_jac_for: pattern.nr() not equal range dimension for f");
        CPPAD_ASSERT_KNOWN(pattern.nc() == n, "sparse_jac_for: pattern.nc() not equal domain dimension for f");
        //
        ...
        local::set_internal_sparsity(zero_empty, input_empty, transpose, internal_index, pattern_transpose, pattern);
        //
        ...
        if (coloring == "cppad")
            ...
            CPPAD_ASSERT_KNOWN(false, "sparse_jac_for: coloring is not valid.");
        //
    ...
    //
    while (color_count < n_color) {  // number of colors that will be in this group
        ...
        //
        // set dx
        for (size_t ell = 0; ell < group_size; ell++) {  // combine all columns with this color
            for (size_t j = 0; j < n; j++) {
                dx[j * group_size + ell] = zero;
                if (color[j] == ell + color_count) dx[j * group_size + ell] = one;
            }
        }
        if (group_size == 1)
            dy = Forward(1, dx);
        ...
        for (size_t ell = 0; ell < group_size; ell++) {  // color with index ell + color_count is in this group
            while (k < K && color[col[order[k]]] ==
                                ell + color_count) {  // subset element with index order[k] is included in this color
...
template <class Base, class RecBase>
template <class SizeVector, class BaseVector>
size_t ADFun<Base, RecBase>::sparse_jac_rev(const BaseVector &x, sparse_rcv<SizeVector, BaseVector> &subset,
                                            const sparse_rc<SizeVector> &pattern, const std::string &coloring,
                                            sparse_jac_work &work) {
    size_t m = Range();
    size_t n = Domain();
    //
    CPPAD_ASSERT_KNOWN(subset.nr() == m, "sparse_jac_rev: subset.nr() not equal range dimension for f");
    CPPAD_ASSERT_KNOWN(subset.nc() == n, "sparse_jac_rev: subset.nc() not equal domain dimension for f");
    //
    // row and column vectors in subset
    const SizeVector &row(subset.row());
    const SizeVector &col(subset.col());
    ...
    vector<size_t> &color(work.color);
    vector<size_t> &order(work.order);
    CPPAD_ASSERT_KNOWN(color.size() == 0 || color.size() == m,
    ...
    // check for case where input work is empty
    if (color.size() == 0) {  // compute work color and order vectors
        CPPAD_ASSERT_KNOWN(pattern.nr() == m, "sparse_jac_rev: pattern.nr() not equal range dimension for f");
        CPPAD_ASSERT_KNOWN(pattern.nc() == n, "sparse_jac_rev: pattern.nc() not equal domain dimension for f");
        //
        // convert pattern to an internal version
        local::pod_vector<size_t> internal_index(m);
        for (size_t i = 0; i < m; i++) internal_index[i] = i;
        ...
        if (coloring == "cppad")
            ...
            CPPAD_ASSERT_KNOWN(false, "sparse_jac_rev: coloring is not valid.");
        //
        // put sorting indices in color order
        SizeVector key(K);
        order.resize(K);
        for (size_t k = 0; k < K; k++) key[k] = color[row[k]];
        index_sort(key, order);
    }
    // Base versions of zero and one
    Base one(1.0);
    ...
    for (size_t ell = 0; ell < n_color; ell++)
        ...
        } else if (color[row[order[k]]] != ell) {  // kludge because colpack returns colors that are not used
            // (it does not know about the subset corresponding to row, col)
            CPPAD_ASSERT_UNKNOWN(coloring == "colpack");
        } else {
