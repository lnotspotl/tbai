...
AD<Base> operator*(const AD<Base> &left, const AD<Base> &right) {
    ...
    if (var_left) {
        if (var_right) {  // result = variable * variable
        ...
        } else if ((!dyn_right) & IdenticalZero(right.value_)) {  // result = variable * 0
        } else if ((!dyn_right) & IdenticalOne(right.value_)) {   // result = variable * 1
        ...
        if ((!dyn_left) & IdenticalZero(left.value_)) {        // result = 0 * variable
        } else if ((!dyn_left) & IdenticalOne(left.value_)) {  // result = 1 * variable
        ...
        result.taddr_ = tape->Rec_.put_dyn_par(result.value_, local::mul_dyn, arg0, arg1);
        result.tape_id_ = tape_id;
