...
void ADFun<Base, RecBase>::capacity_order(size_t c, size_t r) {  // temporary indices
    size_t i, k, ell;

    if ((c == cap_order_taylor_) & (r == num_direction_taylor_)) return;
    ...
    if (p > 0) {
        ...
        for (i = 0; i < num_var_tape_; i++) {  // copy zero order
