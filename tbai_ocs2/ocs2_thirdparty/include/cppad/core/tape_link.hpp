...
tape_id_t *AD<Base>::tape_id_ptr(size_t thread) {
    ...
    CPPAD_ASSERT_UNKNOWN((!thread_alloc::in_parallel()) || thread == thread_alloc::thread_num());
...
local::ADTape<Base> *AD<Base>::tape_ptr(tape_id_t tape_id) {
    ...
    CPPAD_ASSERT_KNOWN(thread == thread_alloc::thread_num(),
...
local::ADTape<Base> *AD<Base>::tape_manage(tape_manage_enum job) {
    CPPAD_ASSERT_UNKNOWN(job == new_tape_manage || job == delete_tape_manage);
    ...
    if (job == new_tape_manage) {
        ...
        *tape_h = new local::ADTape<Base>();
        ...
        if (*tape_id_p == 0) {
            ...
            CPPAD_ASSERT_KNOWN(size_t(std::numeric_limits<tape_id_t>::max()) >= new_tape_id,
        ...
        CPPAD_ASSERT_UNKNOWN(size_t(*tape_id_p % CPPAD_MAX_NUM_THREADS) == thread);
        // set the tape_id for this tape
        (*tape_h)->id_ = *tape_id_p;
    }
    // -----------------------------------------------------------------------
    // delete_tape_manage
    if (job == delete_tape_manage) {  // delete this tape
        CPPAD_ASSERT_UNKNOWN(*tape_h != CPPAD_NULL);
        delete *tape_h;
        *tape_h = CPPAD_NULL;
        //
        // advance tape_id so that all AD<Base> variables become parameters
        CPPAD_ASSERT_KNOWN(std::numeric_limits<CPPAD_TAPE_ID_TYPE>::max() - CPPAD_MAX_NUM_THREADS > *tape_id_p,
