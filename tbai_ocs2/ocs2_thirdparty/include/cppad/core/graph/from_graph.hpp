...
void CppAD::ADFun<Base, RecBase>::from_graph(const CppAD::cpp_graph &graph_obj)
// END_PROTOTYPE
{
    using CppAD::isnan;
    ...
    for (size_t index = 0; index < n_graph_atomic; ++index) atomic_three_index[index] = 0;  // invalid atomic index
    {
        bool set_null = true;
        size_t index_in = 0;
        size_t type;
        std::string name;
        void *ptr;
        size_t n_atomic = CppAD::local::atomic_index<double>(set_null, index_in, type, &name, ptr);
        set_null = false;
        for (index_in = 1; index_in <= n_atomic; ++index_in) {
            CppAD::local::atomic_index<double>(set_null, index_in, type, &name, ptr);
            if (type == 3) {
    ...
    for (size_t op_index = 0; op_index < n_usage; ++op_index) {  // op_enum, name_index, n_result, arg_node
        if (op_index == 0)
        ...
        CPPAD_ASSERT_UNKNOWN(n_arg == size_t(n_con_arg + n_dyn_arg + n_var_arg));
        //
        addr_t i_result = 0;  // invalid value
        // -------------------------------------------------------------------
        // conditional expressions
        // -------------------------------------------------------------------
        if (op_enum == cexp_eq_graph_op || op_enum == cexp_le_graph_op || op_enum == cexp_lt_graph_op) {
            ...
            //
            if (n_var_arg == 0) {
                if (n_dyn_arg == 0) {  // result is a constant parameter
                    ...
                    i_result = rec.put_dyn_cond_exp(nan, cop, arg[0], arg[1], arg[2], arg[3]);
                }
            } else {  // flag marking which arguments are variables
                addr_t flag = 0;
                addr_t bit = 1;
                for (size_t j = 0; j < 4; ++j) {
                    if (type_x[j] == variable_enum) flag |= bit;
                    bit = 2 * bit;
                }
        ...
        // compare operators
        // -------------------------------------------------------------------
        else if (op_enum == comp_eq_graph_op || op_enum == comp_le_graph_op || op_enum == comp_lt_graph_op ||
            ...
            switch (op_enum) {
                case comp_eq_graph_op:
                    result = true;
                    rec.comp_eq(var_left, var_right, dyn_left, dyn_right, ax[0], ax[1], result);
                    break;

                case comp_le_graph_op:
                    result = true;
                    rec.comp_le(var_left, var_right, dyn_left, dyn_right, ax[0], ax[1], result);
                    break;

                case comp_lt_graph_op:
                    result = true;
                    rec.comp_lt(var_left, var_right, dyn_left, dyn_right, ax[0], ax[1], result);
                    break;
        ...
        // -------------------------------------------------------------------
            ...
            CPPAD_ASSERT_KNOWN(n_result == 1, "Json: sum operator: n_result is not 1");
            if (n_var_arg == 0) {  // result of the sum is a parameter
                ...
                for (addr_t j = 0; j < n_dyn_arg; ++j) {
                    i_result = rec.put_dyn_par(nan, local::add_dyn, i_result, temporary[j]);
                    CPPAD_ASSERT_UNKNOWN(isnan(parameter[i_result]));
                }
            } else {  // result of the sum is a variable
                size_t n_temporary = 6 + size_t(n_var_arg + n_dyn_arg);
                if (temporary.size() < n_temporary) temporary.resize(n_temporary);
                Base sum_constant = 0.0;
                addr_t j_variable = 5;
                addr_t j_dynamic = 5 + n_var_arg;
                for (size_t j = 0; j < n_arg; j++) {
                    if (type_x[j] == constant_enum) sum_constant += parameter[arg[j]];
                    if (type_x[j] == variable_enum) temporary[j_variable++] = arg[j];
                    if (type_x[j] == dynamic_enum) temporary[j_dynamic++] = arg[j];
                }
                ...
                for (size_t j = 0; j < n_temporary; ++j) rec.PutArg(temporary[j]);
        ...
        // -------------------------------------------------------------------
            ...
            void *v_ptr;
            CppAD::local::atomic_index<double>(set_null, atomic_index, type, name, v_ptr);
            ...
            afun->forward(parameter_x, type_x, need_y, order_low, order_up, parameter_x, taylor_y);
            //
            ...
            //
            // ax, ay
            if (record_dynamic || record_variable) {  // tape_id (not a recording AD<Base> operations)
                // ax
                ax.resize(n_arg);
                for (size_t j = 0; j < n_arg; ++j) {
            ...
            if (record_dynamic) rec.put_dyn_atomic(tape_id, atomic_index, type_x, type_y, ax, ay);
            if (record_variable) rec.put_var_atomic(tape_id, atomic_index, type_x, type_y, ax, ay);
            //
            // node_type, node2fun
            for (size_t i = 0; i < n_result; ++i) {
        ...
        // -------------------------------------------------------------------
            ...
            if (type_x[0] == variable_enum) switch (op_enum) {
                    ...
                    case erf_graph_op:
                        ...
                        i_par = rec.put_con_par(Base(1.0 / std::sqrt(std::atan(1.0))));
                        rec.PutArg(i_par);
                        //
                        break;
        ...
        // -------------------------------------------------------------------
            ...
            if (type_x[0] == variable_enum && type_x[1] == variable_enum) switch (op_enum) {
                    case add_graph_op:
            ...
            else if (type_x[0] == variable_enum)
                switch (op_enum) {
            ...
            else if (type_x[1] == variable_enum)
                switch (op_enum) {
                    case add_graph_op:
                        i_result = rec.PutOp(local::AddpvOp);
                        rec.PutArg(arg[0], arg[1]);
        ...
        if (op_enum != atom_graph_op && n_result != 0) {  // set node_type and node2fun for result
            //
    ...
    for (size_t i = 0; i < n_dependent; ++i) {
        CPPAD_ASSERT_KNOWN(node_type[graph_obj.dependent_vec_get(i)] != string_enum,
                           "Json AD graph dependent variable node is a string");
        CPPAD_ASSERT_UNKNOWN(node_type[graph_obj.dependent_vec_get(i)] != number_ad_type_enum);
    ...
    subgraph_info_.resize(ind_taddr_.size(),   // n_dep
