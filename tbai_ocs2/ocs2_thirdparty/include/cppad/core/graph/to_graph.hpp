...
void CppAD::ADFun<Base, RecBase>::to_graph(CppAD::cpp_graph &graph_obj)
// END_PROTOTYPE
{
    ...
    for (size_t i = 1; i <= n_dynamic_ind; ++i) par2node[i] = i;               // independent dynamic parameters
    for (size_t i = n_dynamic_ind + 1; i < n_parameter; ++i) par2node[i] = 0;  // will be set later
    // ----------------------------------------------------------------------
    //
    ...
    for (size_t i = 1; i < n_parameter; ++i) {
        if (!dyn_par_is[i]) {  // this is a constant node
    ...
    for (size_t i_dyn = n_dynamic_ind; i_dyn < n_dynamic; ++i_dyn) {  // operator for this dynamic parameter
        ...
        if (n_arg > node_arg.size()) node_arg.resize(n_arg);
        //
        // arguments in graph node space
        for (size_t i = 0; i < n_arg; ++i) {
            node_arg[i] = par2node[dyn_par_arg[i_arg + i]];
            CPPAD_ASSERT_UNKNOWN(node_arg[i] > 0 || (dyn_op == local::cond_exp_dyn && i == 0));
        }
        //
        // invalid value
        ...
        if (n_arg == 1 || n_arg == 2) {  // unary or binary
            op_usage = op_code;
            //
            graph_obj.operator_vec_push_back(op_usage);
            for (size_t i = 0; i < n_arg; ++i) graph_obj.operator_arg_push_back(node_arg[i]);
        } else if (dyn_op == local::result_dyn) {  // setting par2node[i_dyn] above is all that is necessary
            CPPAD_ASSERT_UNKNOWN(op_code == 0);
            CPPAD_ASSERT_UNKNOWN(n_arg == 0);
        } else if (dyn_op == local::call_dyn) {  // arg[0]: atomic function index
            ...
            std::string name;
            {
                ...
                size_t type;
                void *ptr;
                CppAD::local::atomic_index<double>(set_null, atom_index, type, &name, ptr);
            }
            ...
            for (size_t i = 0; i < graph_obj.atomic_name_vec_size(); ++i) {
                if (graph_obj.atomic_name_vec_get(i) == name) {
                    if (name_index == graph_obj.atomic_name_vec_size())
                        ...
                        std::string msg = "The atomic function name " + name + " is used for multiple functions";
                        CPPAD_ASSERT_KNOWN(false, msg.c_str());
                    }
                }
            }
            if (name_index == graph_obj.atomic_name_vec_size()) graph_obj.atomic_name_vec_push_back(name);
            //
            // for atom_graph_op:
            ...
            graph_obj.operator_vec_push_back(op_usage);
            //
            for (size_t j = 0; j < n_arg; ++j) {  // arg[4 + j] is j-th argument to the function
    ...
    for (size_t i = 1; i <= n_variable_ind; ++i) var2node[i] = n_dynamic_ind + i;
    for (size_t i = n_variable_ind + 1; i < n_variable; ++i) var2node[i] = 0;  // invalid node value
    //
    ...
    while (more_operators) {  // if non-zero, this is a fixed size operator with this many arguments
        ...
        // -------------------------------------------------------------------
            ...
            switch (var_op) {
                ...
                case local::LepvOp:
                case local::LevpOp:
                case local::LevvOp: {  // node_0, node_1
                    size_t node_0, node_1;
                    switch (var_op) {  // both nodes parameters
                        case local::EqppOp:
                        case local::NeppOp:
                        case local::LtppOp:
                        case local::LeppOp:
                ...
                // --------------------------------------------------------------
                // CSumOp
                case local::CSumOp: {  // does this case have subtraction terms
                    bool has_subtract = (arg[1] != arg[2]) | (arg[3] != arg[4]);
                    //
                    // var2node for this operator
                    if (has_subtract) {  // two cumulative sum and one subtract operators
                        var2node[i_var] = previous_node + 3;
                    } else {  // one cumulative sum operator
                    ...
                    if (has_subtract) {  // previous_node + 2 = sum corresponding to subtract terms
                ...
                case local::AFunOp:
                    ...
                    if (in_atomic_call) {
                    ...
                    } else {  // This is the AFunOp at the end of the call
                        ...
                        for (size_t i = 0; i < graph_obj.atomic_name_vec_size(); ++i) {
                            if (graph_obj.atomic_name_vec_get(i) == name) {
                                if (name_index == graph_obj.atomic_name_vec_size())
                                    ...
                                    std::string msg =
                                        "The atomic function name " + name + " is used for multiple functions";
                                    CPPAD_ASSERT_KNOWN(false, msg.c_str());
                                }
                            }
                        }
                        ...
                        op_usage = op_code;
                        graph_obj.operator_vec_push_back(op_usage);
                        for (size_t i = 0; i < n_arg; ++i) graph_obj.operator_arg_push_back(atom_node_arg[i]);
                    }
                    break;
                // --------------------------------------------------------------
                // CExpOp:
    ...
    for (size_t i = 0; i < n_dependent; ++i) graph_obj.dependent_vec_push_back(var2node[dep_taddr_[i]]);
    //
    return;
}
