...
bool checkpoint<Base>::rev_sparse_hes(const vector<bool> &vx, const vector<bool> &s, vector<bool> &t, size_t q,
                                      const sparsity_type &r, const sparsity_type &u, sparsity_type &v,
    ...
    if (member_[thread]->hes_sparse_bool_.size() == 0) set_hes_sparse_bool();
...
#ifndef NDEBUG
    for (size_t j = 0; j < n; j++) CPPAD_ASSERT_UNKNOWN(vx[j] || !t[j])
#endif
    ...
    for (size_t i = 0; i < n; i++) {
        for (size_t k = 0; k < q; k++) {  // initialize sparsity pattern for H(i,k)
            bool h_ik = false;
...
bool checkpoint<Base>::rev_sparse_hes(const vector<bool> &vx, const vector<bool> &s, vector<bool> &t, size_t q,
    ...
    if (member_[thread]->hes_sparse_set_.n_set() == 0) set_hes_sparse_set();
    ...
    for (size_t i = 0; i < n; i++) {
        v[i].clear();
        local::sparse_list::const_iterator set_itr(member_[thread]->hes_sparse_set_, i);
        size_t j = *set_itr;
        while (j < n) {
