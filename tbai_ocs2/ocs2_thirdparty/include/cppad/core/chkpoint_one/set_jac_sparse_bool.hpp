...
template <class Base>
void checkpoint<Base>::set_jac_sparse_bool(void) {  // make sure member_ is allocated for this thread
    ...
    if (n <= m) {
        ...
        for (size_t j = 0; j < n; j++) {
            for (size_t i = 0; i < n; i++) identity[i * n + j] = (i == j);
        }
        member_[thread]->jac_sparse_bool_ = member_[thread]->f_.ForSparseJac(n, identity, transpose, dependency);
        member_[thread]->f_.size_forward_bool(0);
    } else {
        vectorBool identity(m * m);
        for (size_t j = 0; j < m; j++) {
            for (size_t i = 0; i < m; i++) identity[i * m + j] = (i == j);
        }
        member_[thread]->jac_sparse_bool_ = member_[thread]->f_.RevSparseJac(m, identity, transpose, dependency);
    }
