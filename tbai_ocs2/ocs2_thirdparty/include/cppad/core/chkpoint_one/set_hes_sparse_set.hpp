...
template <class Base>
void checkpoint<Base>::set_hes_sparse_set(void) {  // make sure member_ is allocated for this thread
    ...
    for (size_t i = 0; i < m; i++) all_one[i] = true;
    ...
    member_[thread]->f_.ForSparseJacCheckpoint(n, identity, transpose, dependency, member_[thread]->jac_sparse_set_);
    member_[thread]->f_.RevSparseHesCheckpoint(n, all_one, transpose, member_[thread]->hes_sparse_set_);
