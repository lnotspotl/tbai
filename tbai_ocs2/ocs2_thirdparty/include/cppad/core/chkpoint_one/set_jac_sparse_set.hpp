...
template <class Base>
void checkpoint<Base>::set_jac_sparse_set(void) {  // make sure member_ is allocated for this thread
    ...
    if (n <= m) {
        ...
        identity.resize(n, n);
        for (size_t j = 0; j < n; j++) {  // use add_element because only adding one element per set
            identity.add_element(j, j);
        }
        member_[thread]->f_.ForSparseJacCheckpoint(n, identity, transpose, dependency,
                                                   member_[thread]->jac_sparse_set_);
        member_[thread]->f_.size_forward_set(0);
    } else {
        local::sparse_list identity;
        identity.resize(m, m);
        for (size_t i = 0; i < m; i++) {  // use add_element because only adding one element per set
            identity.add_element(i, i);
        }
        member_[thread]->f_.RevSparseJacCheckpoint(m, identity, transpose, dependency,
                                                   member_[thread]->jac_sparse_set_);
    }
