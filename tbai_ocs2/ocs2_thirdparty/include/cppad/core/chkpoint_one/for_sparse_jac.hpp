...
bool checkpoint<Base>::for_sparse_jac(size_t q, const sparsity_type &r, sparsity_type &s,
    ...
    for (size_t i = 0; i < m; i++) {
        for (size_t k = 0; k < q; k++) s[i * q + k] = false;
    }
    // sparsity for  s = jac_sparse_bool_ * r
    for (size_t i = 0; i < m; i++) {
        for (size_t k = 0; k < q; k++) {  // initialize sparsity for S(i,k)
            bool s_ik = false;
...
template <class Base>
bool checkpoint<Base>::for_sparse_jac(size_t q, const vectorBool &r, vectorBool &s,
                                      const vector<Base> &x) {  // make sure member_ is allocated for this thread
    size_t thread = thread_alloc::thread_num();
    allocate_member(thread);
    //
...
template <class Base>
bool checkpoint<Base>::for_sparse_jac(size_t q, const vector<bool> &r, vector<bool> &s,
                                      const vector<Base> &x) {  // make sure member_ is allocated for this thread
    size_t thread = thread_alloc::thread_num();
    allocate_member(thread);
    //
...
template <class Base>
bool checkpoint<Base>::for_sparse_jac(size_t q, const vector<std::set<size_t> > &r, vector<std::set<size_t> > &s,
                                      const vector<Base> &x) {  // make sure member_ is allocated for this thread
    size_t thread = thread_alloc::thread_num();
    allocate_member(thread);
    //
    ...
    bool ok = true;
    for (size_t i = 0; i < m; i++) s[i].clear();

    // sparsity for  s = jac_sparse_set_ * r
    for (size_t i = 0; i < m; i++) {  // compute row i of the return pattern
