...
AD<Base> pow(const AD<Base> &x, const AD<Base> &y) {
    ...
    CPPAD_ASSERT_KNOWN(x.tape_id_ == y.tape_id_ || !match_x || !match_y,
                       "pow: AD variables or dynamic parameters on different threads.");
    if (var_x) {
        if (var_y) {  // result = variable^variable
        ...
        } else if (IdenticalZero(y.value_)) {  // result = variable^0
        } else {                               // result = variable^parameter
            CPPAD_ASSERT_UNKNOWN(local::NumRes(local::PowvpOp) == 3);
            CPPAD_ASSERT_UNKNOWN(local::NumArg(local::PowvpOp) == 2);

            // put operand addresses in tape
            addr_t p = y.taddr_;
            if (!dyn_y) p = tape->Rec_.put_con_par(y.value_);
        ...
        if (IdenticalZero(x.value_)) {  // result = 0^variable
        } else {                        // result = parameter^variable
            CPPAD_ASSERT_UNKNOWN(local::NumRes(local::PowpvOp) == 3);
            CPPAD_ASSERT_UNKNOWN(local::NumArg(local::PowpvOp) == 2);

            // put operand addresses in tape
            addr_t p = x.taddr_;
            if (!dyn_x) p = tape->Rec_.put_con_par(x.value_);
        ...
        if (!dyn_x) arg0 = tape->Rec_.put_con_par(x.value_);
        if (!dyn_y) arg1 = tape->Rec_.put_con_par(y.value_);
        //
        // parameters with a dynamic parameter result
        result.taddr_ = tape->Rec_.put_dyn_par(result.value_, local::pow_dyn, arg0, arg1);
        result.tape_id_ = tape_id;
        result.ad_type_ = dynamic_enum;
    } else {
        CPPAD_ASSERT_KNOWN(!(dyn_x | dyn_y), "pow: one operand is a dynamic parameter and other not a variable");
    }
    return result;
}
// =========================================================================
...
inline AD<double> pow(const double &x, const AD<double> &y) {
    return pow(AD<double>(x), y);
}

inline AD<double> pow(const double &x, const VecAD_reference<double> &y) {
    return pow(AD<double>(x), y.ADBase());
}

inline AD<double> pow(const AD<double> &x, const double &y) {
    return pow(x, AD<double>(y));
}

inline AD<double> pow(const VecAD_reference<double> &x, const double &y) {
