...
AD<Base> azmul(const AD<Base> &x, const AD<Base> &y) {
    ...
    if (var_x) {
        if (var_y) {  // result = azmul(variable, variable)
        ...
        } else if (IdenticalOne(y.value_)) {   // result = variable * 1
            result.make_variable(x.tape_id_, x.taddr_);
        } else {  // result = zmul(variable, parameter)
        ...
        } else if (IdenticalOne(x.value_)) {  // result = 1 * variable
            result.make_variable(y.tape_id_, y.taddr_);
        } else {  // result = zmul(parameter, variable)
        ...
        result.taddr_ = tape->Rec_.put_dyn_par(result.value_, local::zmul_dyn, arg0, arg1);
        result.tape_id_ = tape_id;
