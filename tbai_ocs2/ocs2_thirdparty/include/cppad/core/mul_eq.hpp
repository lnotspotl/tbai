...
AD<Base> &AD<Base>::operator*=(const AD<Base> &right) {
    ...
    if (tape == CPPAD_NULL) return *this;
    ...
    CPPAD_ASSERT_KNOWN(tape_id_ == right.tape_id_ || !match_left || !match_right,
                       "*= : AD variables or dynamic parameters on different threads.");
    if (var_left) {
        if (var_right) {  // this = variable * variable
        ...
        } else if ((!dyn_right) & IdenticalOne(right.value_)) {   // this = variable * 1
        } else if ((!dyn_right) & IdenticalZero(right.value_)) {  // this = variable * 0
            tape_id_ = 0;                                         // not in current tape
        } else {                                                  // this = variable  * parameter
            //      = parameter * variable
        ...
        if ((!dyn_left) & IdenticalZero(left)) {        // this = 0 * right
        } else if ((!dyn_left) & IdenticalOne(left)) {  // this = 1 * right
            make_variable(right.tape_id_, right.taddr_);
        } else {  // this = parameter * variable
            CPPAD_ASSERT_UNKNOWN(local::NumRes(local::MulpvOp) == 1);
            CPPAD_ASSERT_UNKNOWN(local::NumArg(local::MulpvOp) == 2);

            // put operand addresses in tape
            addr_t p = taddr_;
            if (!dyn_left) p = tape->Rec_.put_con_par(left);
        ...
        if (!dyn_left) arg0 = tape->Rec_.put_con_par(left);
        if (!dyn_right) arg1 = tape->Rec_.put_con_par(right.value_);
        //
        // parameters with a dynamic parameter results
        taddr_ = tape->Rec_.put_dyn_par(value_, local::mul_dyn, arg0, arg1);
        tape_id_ = tape_id;
