...
// BEGIN_PROTOTYPE
template <class Base>
bool atomic_three<Base>::jac_sparsity(const vector<Base> &parameter_x, const vector<ad_type_enum> &type_x,
...
bool atomic_three<Base>::for_jac_sparsity(bool dependency, const vector<Base> &parameter_x,
                                          const vector<ad_type_enum> &type_x, const local::pod_vector<size_t> &x_index,
    ...
    for (size_t i = 0; i < m; ++i) select_y[i] = y_index[i] != 0;

    // determine select_x
    vector<bool> select_x(n);
    for (size_t j = 0; j < n; ++j) {  // check if x_j depends on any previous variable
    ...
    sparse_rc<vector<size_t> > pattern_out;
    bool ok = jac_sparsity(parameter_x, type_x, dependency, select_x, select_y, pattern_out);
    if (!ok) return false;
    //
    ...
    for (size_t i = 0; i < m; ++i) var_sparsity.process_post(y_index[i]);
    //
    return true;
}
/*!
...
template <class Base>
template <class InternalSparsity>
bool atomic_three<Base>::rev_jac_sparsity(bool dependency, const vector<Base> &parameter_x,
                                          const vector<ad_type_enum> &type_x, const local::pod_vector<size_t> &x_index,
                                          const local::pod_vector<size_t> &y_index, InternalSparsity &var_sparsity) {
    typedef typename InternalSparsity::const_iterator iterator;
    ...
    for (size_t j = 0; j < n; ++j) select_x[j] = true;

    // determine select_y
    for (size_t i = 0; i < m; ++i) {  // check if y_i has sparsity is non-empty
    ...
    for (size_t j = 0; j < n; ++j) var_sparsity.process_post(x_index[j]);
    //
    return true;
}
