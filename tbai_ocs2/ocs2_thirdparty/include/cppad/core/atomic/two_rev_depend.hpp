...
// BEGIN_PROTOTYPE
template <class Base>
bool atomic_base<Base>::rev_depend(const vector<Base> &parameter_x, const vector<ad_type_enum> &type_x,
...
// END_PROTOTYPE
{
    ...
    if (sparsity_ == pack_sparsity_enum) {
        ...
        st.resize(n * 1);
        rt.resize(m * 1);
        for (size_t i = 0; i < m; ++i) rt[i] = depend_y[i];
        ok = rev_sparse_jac(1, rt, st, parameter_x);
        if (!ok) ok = rev_sparse_jac(1, rt, st);
        if (!ok) return false;
        for (size_t j = 0; j < n; ++j) depend_x[j] = st[j];
    } else if (sparsity_ == bool_sparsity_enum) {
        ok = rev_sparse_jac(1, depend_y, depend_x, parameter_x);
        if (!ok) ok = rev_sparse_jac(m, depend_y, depend_x);
        if (!ok) return false;
    } else {
        ...
        for (size_t i = 0; i < m; ++i) {
            if (depend_y[i]) rt[i].insert(0);
        }
        ok = rev_sparse_jac(m, rt, st, parameter_x);
        if (!ok) ok = rev_sparse_jac(m, rt, st);
        if (!ok) return false;
        for (size_t j = 0; j < n; ++j) depend_x[j] = !st[j].empty();
    }
    return ok;
}
