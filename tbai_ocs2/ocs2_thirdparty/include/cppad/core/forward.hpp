...
BaseVector ADFun<Base, RecBase>::Forward(size_t q, const BaseVector &xq, std::ostream &s) {
    ...
    CPPAD_ASSERT_KNOWN(size_t(xq.size()) == n || size_t(xq.size()) == n * (q + 1),
    ...
    if ((cap_order_taylor_ <= q) | (num_direction_taylor_ != 1)) {
        if (p == 0) {  // no need to copy old values during capacity_order
            num_order_taylor_ = 0;
        } else
            num_order_taylor_ = q;
    ...
    for (j = 0; j < num_var_tape_; j++) {
        for (k = p; k <= q; k++) taylor_[C * j + k] = CppAD::numeric_limits<Base>::quiet_NaN();
    }
    ...
    for (j = 0; j < n; j++) {
        ...
        if (p == q)
            ...
            for (k = 0; k <= q; k++) taylor_[C * ind_taddr_[j] + k] = xq[(q + 1) * j + k];
        }
    }
    ...
    if (q == 0) {
        local::sweep::forward0(&play_, s, true, n, num_var_tape_, C, taylor_.data(), cskip_op_.data(), load_op_,
                               compare_change_count_, compare_change_number_, compare_change_op_index_,
                               not_used_rec_base);
    } else {
        local::sweep::forward1(&play_, s, true, p, q, n, num_var_tape_, C, taylor_.data(), cskip_op_.data(), load_op_,
                               compare_change_count_, compare_change_number_, compare_change_op_index_,
                               not_used_rec_base);
    }
    ...
    if (p == q) {
        ...
        yq.resize(m * (q + 1));
        for (i = 0; i < m; i++) {
            for (k = 0; k <= q; k++) yq[(q + 1) * i + k] = taylor_[C * dep_taddr_[i] + k];
        }
    }
#ifndef NDEBUG
    if (check_for_nan_) {
        ...
        size_t index = m;
        if (p == 0) {
            for (i = 0; i < m; i++) {  // Visual Studio 2012, CppAD required in front of isnan ?
        ...
        if (!ok) {
            ...
            for (j = 0; j < n; j++) x0[j] = taylor_[C * ind_taddr_[j] + 0];
            std::string file_name;
            put_check_for_nan(x0, file_name);
            std::stringstream ss;
            ss << "yq = f.Forward(q, xq): a zero order Taylor coefficient is nan.\n"
            ...
            ErrorHandler::Call(true, __LINE__, __FILE__, "if( CppAD::isnan( yq[ (q+1) * index + 0 ] )", msg_char_star);
        }
        CPPAD_ASSERT_KNOWN(ok, "with the value nan.");
        if (0 < q) {
            for (i = 0; i < m; i++) {
                for (k = p; k <= q; k++) {  // Studio 2012, CppAD required in front of isnan ?
...
template <class Base, class RecBase>
template <class BaseVector>
BaseVector ADFun<Base, RecBase>::Forward(size_t q, size_t r, const BaseVector &xq) {
    // used to identify the RecBase type in calls to sweeps
    RecBase not_used_rec_base;
    ...
    CPPAD_ASSERT_KNOWN(size_t(xq.size()) == r * n, "Forward(q, r, xq): xq.size() is not equal r * n");
    CPPAD_ASSERT_KNOWN(q <= num_order_taylor_,
                       "Forward(q, r, xq): Number of Taylor coefficient orders stored in"
                       " this ADFun is less than q");
    CPPAD_ASSERT_KNOWN(q == 1 || num_direction_taylor_ == r,
    ...
    if (cap_order_taylor_ <= q || num_direction_taylor_ != r) {
        if (num_direction_taylor_ != r) num_order_taylor_ = 1;
    ...
    CPPAD_ASSERT_UNKNOWN(cskip_op_.size() == play_.num_op_rec());
    CPPAD_ASSERT_UNKNOWN(load_op_.size() == play_.num_load_op_rec());
    local::sweep::forward2(&play_, q, r, n, num_var_tape_, c, taylor_.data(), cskip_op_.data(), load_op_,
                           not_used_rec_base);
    ...
    if (check_for_nan_) {
        bool ok = true;
        for (i = 0; i < m; i++) {
            for (ell = 0; ell < r; ell++) {  // Studio 2012, CppAD required in front of isnan ?
