...
class discrete {
    ...
    */
    discrete(const char *Name, F f) : name_(Name), f_(f), index_(List().size()) {
    ...
    */
    AD<Base> ad(const AD<Base> &ax) const {
        CPPAD_ASSERT_KNOWN(size_t(std::numeric_limits<addr_t>::max()) >= index_,
        ...
        if (tape == CPPAD_NULL) return ay;
        //
        // check if argument is a constant parameter
        if (ax.tape_id_ != tape->id_) return ay;
        //
        if (ax.ad_type_ == dynamic_enum) {
            // tape dynamic paramter operation
            ay.taddr_ = tape->Rec_.put_dyn_par(ay.value_, local::dis_dyn, addr_t(index_), ax.taddr_);
        ...
        } else {  // other types not yet being used and should have this tape id
    ...
    static const char *name(size_t index) { return List()[index]->name_.c_str(); }
