...
void ADFun<Base, RecBase>::abs_normal_fun(ADFun &g, ADFun &a) const {
    ...
    while (op != EndOp) {
        // next op
        (++itr).op_info(op, arg, i_var);
        switch (op) {  // absolute value operator
            case AbsOp:
    ...
    for (size_t i_par = 0; i_par < num_par; ++i_par) {
        ...
        if (!dyn_par_is[i_par])
        ...
        else {  // operator for this dynamic parameter
            ...
            switch (n_arg) {
                ...
                case 1:
                    CPPAD_J_PAR_EQUAL_REC.put_dyn_par(par, op_dyn, dyn_par_arg[i_arg + 0]);
                    break;

                case 2:
                    CPPAD_J_PAR_EQUAL_REC.put_dyn_par(par, op_dyn, dyn_par_arg[i_arg + 0], dyn_par_arg[i_arg + 1]);
                    break;

                case 5:
                    CPPAD_J_PAR_EQUAL_REC.put_dyn_cond_exp(par, CompareOp(dyn_par_arg[i_arg + 0]),
                                                           dyn_par_arg[i_arg + 1], dyn_par_arg[i_arg + 2],
                                                           dyn_par_arg[i_arg + 3], dyn_par_arg[i_arg + 4]);
                    break;
    ...
    CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= num_var);
    CppAD::vector<addr_t> f2g_var(num_var);
    for (i_var = 0; i_var < num_var; i_var++) f2g_var[i_var] = addr_t(num_var);  // invalid (should not be used)
    //
    ...
    while (more_operators) {
        ...
        if (more_operators) (++itr).op_info(op, arg, i_var);
    }
    ...
    size_t index_abs;
    for (index_abs = 0; index_abs < f_abs_res.size(); index_abs++) f2g_var[f_abs_res[index_abs]] = rec.PutOp(InvOp);
    //
    ...
    for (size_t i = 0; i < m; i++) g.dep_parameter_[i] = dep_parameter_[i];
    for (size_t i = 0; i < s; i++) g.dep_parameter_[m + i] = false;
    ...
    g.subgraph_info_.resize(g.ind_taddr_.size(),   // n_ind
