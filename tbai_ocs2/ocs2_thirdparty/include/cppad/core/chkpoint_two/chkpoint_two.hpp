...
class chkpoint_two : public atomic_three<Base> {
   ...
   private:
    ...
    void allocate_member(size_t thread) {
        ...
        if (member_[thread] == CPPAD_NULL) {  // allocaate raw memory
    ...
    void free_member(size_t thread) {
        if (member_[thread] != CPPAD_NULL) {  // call destructor
   ...
   public:
    // ctor
    chkpoint_two(const ADFun<Base> &fun, const std::string &name, bool internal_bool, bool use_hes_sparsity,
                 bool use_base2ad, bool use_in_parallel);
    //
    // destructor
    ~chkpoint_two(void);
    //
    ...
    void operator=(const chkpoint_two &other) {
        CPPAD_ASSERT_KNOWN(false, "cannot use chkpoint_two assignment operator");
    }
    // copy constructor
