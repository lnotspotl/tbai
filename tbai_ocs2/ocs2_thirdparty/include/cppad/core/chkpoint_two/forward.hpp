...
*/
template <class Base>
bool chkpoint_two<Base>::forward(const vector<Base> &parameter_x, const vector<ad_type_enum> &type_x, size_t need_y,
...
*/
template <class Base>
bool chkpoint_two<Base>::forward(const vector<AD<Base> > &aparameter_x, const vector<ad_type_enum> &type_x,
                                 size_t need_y, size_t order_low, size_t order_up, const vector<AD<Base> > &ataylor_x,
                                 vector<AD<Base> > &ataylor_y) {
    if (!use_base2ad_) return false;
    //
