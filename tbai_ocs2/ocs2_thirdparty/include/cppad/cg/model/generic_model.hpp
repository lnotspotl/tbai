...
class GenericModel {
   ...
   public:
    ...
    virtual void ForwardZero(const CppAD::vector<bool> &vx, CppAD::vector<bool> &vy, ArrayView<const Base> tx,
                             ArrayView<Base> ty) = 0;
     ...
     * @param ty1
     */
    virtual void ForwardOne(ArrayView<const Base> x, size_t tx1Nnz, const size_t idx[], const Base tx1[],
     ...
     * @warning do not used it as a generic reverse mode function!
     */
    virtual void ReverseOne(ArrayView<const Base> tx, ArrayView<const Base> ty, ArrayView<Base> px,
     ...
     */
    virtual void ReverseOne(ArrayView<const Base> x, ArrayView<Base> px, size_t pyNnz, const size_t idx[],
                            const Base py[]) = 0;
     ...
     *          px[j * (k+1) + 1] is not used during the hessian evaluation.
     */
    virtual void ReverseTwo(ArrayView<const Base> tx, ArrayView<const Base> ty, ArrayView<Base> px,
                            ArrayView<const Base> py) = 0;
    /**
     * Computes second-order results during a reverse mode sweep (p = 2).
     * This method can be used during the evaluation of the hessian when
     ...
     */
    virtual void ReverseTwo(ArrayView<const Base> x, size_t tx1Nnz, const size_t idx[], const Base tx1[],
                            ArrayView<Base> px2, ArrayView<const Base> py2) = 0;
    ...
    virtual void SparseJacobian(const std::vector<Base> &x, std::vector<Base> &jac, std::vector<size_t> &row,
                                std::vector<size_t> &col) = 0;

    virtual void SparseJacobian(ArrayView<const Base> x, ArrayView<Base> jac, size_t const **row,
                                size_t const **col) = 0;
     ...
     */
    virtual void SparseJacobian(const std::vector<const Base *> &x, ArrayView<Base> jac, size_t const **row,
                                size_t const **col) = 0;
    ...
    virtual void SparseHessian(const std::vector<Base> &x, const std::vector<Base> &w, std::vector<Base> &hess,
                               std::vector<size_t> &row, std::vector<size_t> &col) = 0;

    virtual void SparseHessian(ArrayView<const Base> x, ArrayView<const Base> w, ArrayView<Base> hess,
                               size_t const **row, size_t const **col) = 0;
     ...
     */
    virtual void SparseHessian(const std::vector<const Base *> &x, ArrayView<const Base> w, ArrayView<Base> hess,
                               size_t const **row, size_t const **col) = 0;
