...
std::vector<std::pair<CG<Base>, IndexPattern *> > generateReverseTwoGroupOps(
    CodeHandler<Base> &handler, const LoopModel<Base> &lModel, const loops::HessianWithLoopsInfo<Base> &info,
    HessianRowGroup<Base> &group, const CG<Base> &tx1, const std::map<size_t, std::map<size_t, CG<Base> > > &dzDx,
    std::map<size_t, std::set<size_t> > &jrow2CompressedLoc);

template <class Base>
std::pair<CG<Base>, IndexPattern *> createReverseMode2Contribution(
    CodeHandler<Base> &handler, HessianRowGroup<Base> &group, const std::vector<HessianElement> &positions,
    const CG<Base> &ddfdxdx, const CG<Base> &tx1, IndexOperationNode<Base> &iterationIndexOp,
    std::map<size_t, std::set<size_t> > &jrow2CompressedLoc);
}  // namespace loops
        ...
        for (size_t g = 0; g < loopGroups.size(); g++) {
            ...
            if (createsLoop) {
                ...
                if (IndexPattern::isConstant(*indexLocalItCountPattern.get())) {
                    ...
                    itCountAssignOp =
                        handler.makeIndexAssignNode(indexLocalItCountDcl, *indexLocalItCountPattern.get(), jrowIndexOp);
...
template <class Base>
void generateHessianRowGroups(const LoopModel<Base> &lModel, const HessianWithLoopsInfo<Base> &info, size_t n,
                              SmartVectorPointer<HessianRowGroup<Base> > &loopGroups) {
    using namespace std;
    using namespace CppAD::cg::loops;
    ...
    map<HessianTermContrib<Base>, set<size_t> > contrib2jrows =
        groupHessianRowsByContrib(info, n, indexedIndexed2jrow2Iter, indexedNonIndexed2jrow2Iter, indexedTemp2jrow2Iter,
                                  nonIndexedIndexed2jrow2Iter, tempIndexed2jrow2Iter);
...
template <class Base>
std::vector<std::pair<CG<Base>, IndexPattern *> > generateReverseTwoGroupOps(
    CodeHandler<Base> &handler, const LoopModel<Base> &lModel, const HessianWithLoopsInfo<Base> &info,
    HessianRowGroup<Base> &group, const CG<Base> &tx1, const std::map<size_t, std::map<size_t, CG<Base> > > &dzDx,
    std::map<size_t, std::set<size_t> > &jrow2CompressedLoc) {
    using namespace std;
    using namespace CppAD::cg::loops;
...
inline std::map<HessianTermContrib<Base>, std::set<size_t> > groupHessianRowsByContrib(
    const loops::HessianWithLoopsInfo<Base> &info, size_t n,
...
inline void subgroupHessianRowsByContrib(
    const HessianWithLoopsInfo<Base> &info, const HessianTermContrib<Base> &c, const std::set<size_t> &jrows,
 ...
 */
template <class Base>
CG<Base> createReverseMode2Contribution(CodeHandler<Base> &handler, HessianRowGroup<Base> &group,
                                        LinearIndexPattern &pattern, const std::set<size_t> &iterations,
                                        const CG<Base> &ddfdxdx, IndexOperationNode<Base> &iterationIndexOp,
...
template <class Base>
void ModelCSourceGen<Base>::generateFunctionNameLoopRev2(std::ostringstream &cache, const LoopModel<Base> &loop,
                                                         size_t g) {
    generateFunctionNameLoopRev2(cache, _name, loop, g);
}

template <class Base>
void ModelCSourceGen<Base>::generateFunctionNameLoopRev2(std::ostringstream &cache, const std::string &modelName,
