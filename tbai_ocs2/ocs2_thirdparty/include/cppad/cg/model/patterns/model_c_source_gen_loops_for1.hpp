        ...
        for (size_t g = 0; g < loopGroups.size(); g++) {
            ...
            if (createsLoop) {
                ...
                if (IndexPattern::isConstant(*indexLocalItCountPattern.get())) {
                    ...
                    itCountAssignOp =
                        handler.makeIndexAssignNode(indexLocalItCountDcl, *indexLocalItCountPattern.get(), jcolIndexOp);
...
template <class Base>
void ModelCSourceGen<Base>::createForwardOneWithLoopsNL(CodeHandler<Base> &handler, size_t j,
...
template <class Base>
void generateForOneColumnGroups(const LoopModel<Base> &lModel, const std::vector<JacobianWithLoopsRowInfo> &loopEqInfo,
                                size_t max, size_t n, SmartVectorPointer<JacobianColGroup<Base> > &loopGroups) {
    using namespace std;
    using namespace CppAD::cg::loops;
...
std::map<JacobianTermContrib<Base>, std::set<size_t> > groupForOneByContrib(
    const LoopModel<Base> &lModel, const std::vector<JacobianWithLoopsRowInfo> &loopEqInfo, size_t n,
...
inline void subgroupForOneByContrib(const std::vector<JacobianWithLoopsRowInfo> &loopEqInfo,
                                    const JacobianTermContrib<Base> &c, const std::set<size_t> &jcols,
...
std::vector<std::pair<CG<Base>, IndexPattern *> > generateForwardOneGroupOps(
    CodeHandler<Base> &handler, const LoopModel<Base> &lModel, const std::vector<JacobianWithLoopsRowInfo> &info,
    JacobianColGroup<Base> &group, IndexOperationNode<Base> &iterationIndexOp, const CG<Base> &dx,
...
template <class Base>
std::pair<CG<Base>, IndexPattern *> createForwardOneElement(CodeHandler<Base> &handler, JacobianColGroup<Base> &group,
...
template <class Base>
void ModelCSourceGen<Base>::generateFunctionNameLoopFor1(std::ostringstream &cache, const LoopModel<Base> &loop,
                                                         size_t g) {
    generateFunctionNameLoopFor1(cache, _name, loop, g);
}

template <class Base>
void ModelCSourceGen<Base>::generateFunctionNameLoopFor1(std::ostringstream &cache, const std::string &modelName,
