...
void ModelCSourceGen<Base>::analyseSparseHessianWithLoops(
    ...
    const std::vector<size_t> &lowerHessOrder, std::vector<std::set<size_t> > &noLoopEvalJacSparsity,
                ...
                if (posJ2 != nullptr) {
                    ...
                    for (size_t iteration = 0; iteration < iter2tapeJ1OrigJ2.size(); iteration++) {
                        ...
                        for (size_t tapeJ1 : tapeJ1s) {
                            ...
                            if (flip) {
                                positions =
                                    &loopInfo.equationGroups[g].nonIndexedIndexedPositions[pairss(posJ2->tape, tapeJ1)];
                                loopInfo.equationGroups[g].evalHessSparsity[posJ2->tape].insert(tapeJ1);
                            } else {
                                positions =
                                    &loopInfo.equationGroups[g].indexedNonIndexedPositions[pairss(tapeJ1, posJ2->tape)];
...
template <class Base>
inline void addContribution(std::vector<std::pair<CG<Base>, IndexPattern *> > &indexedLoopResults, size_t &hessLE,
...
template <class Base>
std::vector<CG<Base> > ModelCSourceGen<Base>::prepareSparseHessianWithLoops(
    CodeHandler<Base> &handler, std::vector<CGBase> &x, std::vector<CGBase> &w,
    const std::vector<size_t> &lowerHessRows, const std::vector<size_t> &lowerHessCols,
    const std::vector<size_t> &lowerHessOrder, const std::map<size_t, size_t> &duplicates) {
    using namespace std;
    using namespace CppAD::cg::loops;
 ...
 */
template <class Base>
CG<Base> createHessianContribution(CodeHandler<Base> &handler, LinearIndexPattern &pattern,
                                   const std::set<size_t> &iterations, size_t nIterations, const CG<Base> &ddfdxdx,
                                   IndexOperationNode<Base> &iterationIndexOp,
                                   std::vector<IfElseInfo<Base> > &ifElses) {
    using namespace std;
...
inline void generateLoopForJacHes(
    ...
    std::vector<std::map<size_t, std::map<size_t, CG<Base> > > > &vhess, bool individualColoring) {
    using namespace std;
