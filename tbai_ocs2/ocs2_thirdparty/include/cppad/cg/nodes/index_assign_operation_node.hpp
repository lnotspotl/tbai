...
class IndexAssignOperationNode : public OperationNode<Base> {
   ...
   protected:
     ...
     * @param index1
     */
    inline IndexAssignOperationNode(CodeHandler<Base> *handler, OperationNode<Base> &index, IndexPattern &indexPattern,
                                    IndexOperationNode<Base> &index1)
        : OperationNode<Base>(handler, CGOpCode::IndexAssign, {index, index1}), indexPattern_(indexPattern) {}

    inline IndexAssignOperationNode(CodeHandler<Base> *handler, OperationNode<Base> &index, IndexPattern &indexPattern,
                                    IndexOperationNode<Base> *index1, IndexOperationNode<Base> *index2)
        : OperationNode<Base>(handler, CGOpCode::IndexAssign, std::vector<size_t>(0),
                              createArguments(index, index1, index2)),
