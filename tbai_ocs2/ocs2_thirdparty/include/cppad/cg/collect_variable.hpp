...
template <class Base>
inline CG<Base> CodeHandler<Base>::collectVariable(OperationNode<Base> &expression, const SourceCodePath &path1,
                                                   const SourceCodePath &path2, size_t lastCommon) {
...
inline bool CodeHandler<Base>::isCollectableVariableAddSub(const SourceCodePath &pathLeft,
                                                           const SourceCodePath &pathRight, bool throwEx) {
    /**
     * Check operations after the divergence
     */
    std::vector<const SourceCodePath *> paths{&pathLeft, &pathRight};
