...
class LanguageGenerationData {
   ...
   public:
    LanguageGenerationData(const std::vector<Node *> &ind, const ArrayView<CG<Base>> &dep, size_t minTempVID,
                           const CodeHandlerVector<Base, size_t> &varIds, const std::vector<Node *> &vo,
                           const std::vector<std::set<Node *>> &variableDependencies, VariableNameGenerator<Base> &ng,
                           const std::map<size_t, size_t> &atomicId2Index,
                           const std::map<size_t, std::string> &atomicId2Name, const std::vector<int> &atomicMaxForward,
                           const std::vector<int> &atomicMaxReverse, const bool ri,
                           const std::set<const Node *> &indexes,
                           ...
                           const CodeHandlerVector<Base, size_t> &totalUseCount,
                           const CodeHandlerVector<Base, ScopeIDType> &scope,
                           IndexOperationNode<Base> &auxIterationIndexOp, bool zero)
        : independent(ind),
          dependent(dep),
