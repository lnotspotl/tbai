...
class LanguageDot : public Language<Base> {
   ...
   public:
    ...
    static void indexPattern2String(std::ostream &os, const IndexPattern &ip,
                                    const std::vector<const OperationNode<Base> *> &indexes);

    static inline void linearIndexPattern2String(std::ostream &os, const LinearIndexPattern &lip,
   ...
   protected:
    ...
    virtual bool requiresVariableDependencies() const override { return false; }
    ...
    inline std::string printNodeDeclaration(const OperationNode<Base> &op, const std::ostringstream &label,
                                            const std::string &shape = "") {
        return printNodeDeclaration(op, label.str(), shape);
    }

    virtual std::string printNodeDeclaration(const OperationNode<Base> &op, const std::string &label = "",
                                             const std::string &shape = "") {
    ...
    inline void printEdges(const std::string &name, const OperationNode<Base> &node,
                           const std::vector<std::string> &args, const std::string &style = "") {
    ...
    inline void printEdges(const std::string &name, const OperationNode<Base> &node,
                           const std::vector<std::string> &args, const std::vector<std::string> &styles) {
        size_t na = node.getArguments().size();
        size_t nna = args.size();
    ...
    inline size_t printArrayCreationUsingLoop(const std::string arrayName, const OperationNode<Base> &array,
                                              size_t startj, const size_t *indexes);
