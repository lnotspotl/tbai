...
class atomic_mat_mul : public CppAD::atomic_three<double> {
   ...
   private:
    ...
    $srccode%cpp% */
    size_t left(size_t i,         // left matrix row index
    ...
    $srccode%cpp% */
    size_t right(size_t i,         // right matrix row index
    ...
    $srccode%cpp% */
    size_t result(size_t i,         // result matrix row index
    ...
    $srccode%cpp% */
    void forward_multiply(size_t k_left,             // order for left coefficients
        ...
        for (size_t i = 0; i < nr_left; i++) {
            for (size_t j = 0; j < nc_right; j++) {
                ...
                for (size_t ell = 0; ell < n_middle; ell++) {
                    size_t i_left = left(i, ell, k_left, nk, nr_left, n_middle, nc_right);
                    size_t i_right = right(ell, j, k_right, nk, nr_left, n_middle, nc_right);
                    sum += tx[i_left] * tx[i_right];
                }
                size_t i_result = result(i, j, k_result, nk, nr_left, n_middle, nc_right);
                ty[i_result] += sum;
            }
        }
    }
    ...
    $srccode%cpp% */
    void reverse_multiply(size_t k_left,             // order for left coefficients
                          size_t k_right,            // order for right coefficients
                          const vector<double> &tx,  // domain space Taylor coefficients
    ...
    $srccode%cpp% */
    // calculate type_y
    virtual bool for_type(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x,
        ...
        size_t k = 0;   // order
        for (size_t i = 0; i < nr_left; ++i) {
            for (size_t j = 0; j < nc_right; ++j) {  // compute type for result[i, j]
                CppAD::ad_type_enum type_yij = CppAD::constant_enum;
                for (size_t ell = 0; ell < n_middle; ++ell) {  // index for left(i, ell)
                    size_t i_left = left(i, ell, k, nk, nr_left, n_middle, nc_right);
                    // indx for right(ell, j)
                    size_t i_right = right(ell, j, k, nk, nr_left, n_middle, nc_right);
                ...
                size_t i_result = result(i, j, k, nk, nr_left, n_middle, nc_right);
    ...
    $srccode%cpp% */
    virtual bool forward(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x, size_t need_y,
                         size_t q, size_t p, const vector<double> &tx, vector<double> &ty) {
        size_t n_order = p + 1;
        ...
        for (i = 0; i < nr_left; i++) {
            for (j = 0; j < nc_right; j++) {
                for (k = q; k <= p; k++) {
                    size_t i_result = result(i, j, k, n_order, nr_left, n_middle, nc_right);
                    ty[i_result] = 0.0;
                }
            }
        }
        for (k = q; k <= p; k++) {  // sum the produces that result in order k
            for (ell = 0; ell <= k; ell++) forward_multiply(ell, k - ell, tx, ty, nr_left, n_middle, nc_right);
        }
    ...
    $srccode%cpp% */
    virtual bool reverse(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x, size_t p,
                         const vector<double> &tx, const vector<double> &ty, vector<double> &px,
        ...
        size_t k = n_order;
        while (k--) {  // differentiate the produces that result in order k
            for (size_t ell = 0; ell <= k; ell++)
                reverse_multiply(ell, k - ell, tx, ty, px, py, nr_left, n_middle, nc_right);
        }
    ...
    virtual bool jac_sparsity(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x,
                              bool dependency, const vector<bool> &select_x, const vector<bool> &select_y,
            ...
            for (size_t j = 0; j < nc_right; ++j) {
                ...
                if (select_y[i_result]) {
                    for (size_t ell = 0; ell < n_middle; ++ell) {
                        ...
                        zero_right &= parameter_x[i_right] == 0.0;
                        if (!(zero_left | zero_right)) {
                            bool var_left = type_x[i_left] == CppAD::variable_enum;
                            bool var_right = type_x[i_right] == CppAD::variable_enum;
                            if (select_x[i_left] & var_left) ++nnz;
                            if (select_x[i_right] & var_right) ++nnz;
                        }
                    }
                }
            }
                ...
                if (select_y[i_result]) {
                    for (size_t ell = 0; ell < n_middle; ++ell) {
                        ...
                        if (!(zero_left | zero_right)) {
                            bool var_left = type_x[i_left] == CppAD::variable_enum;
                            bool var_right = type_x[i_right] == CppAD::variable_enum;
                            if (select_x[i_left] & var_left) pattern_out.set(idx++, i_result, i_left);
                            if (select_x[i_right] & var_right) pattern_out.set(idx++, i_result, i_right);
                        }
                    }
                }
            }
    ...
    $srccode%cpp% */
    // Jacobian sparsity routine called by CppAD
    virtual bool hes_sparsity(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x,
        ...
        for (size_t i = 0; i < nr_left; ++i) {
            for (size_t j = 0; j < nc_right; ++j) {
                size_t i_result = result(i, j, k, nk, nr_left, n_middle, nc_right);
                if (select_y[i_result]) {
                    for (size_t ell = 0; ell < n_middle; ++ell) {  // i_left depends on i, ell
                        size_t i_left = left(i, ell, k, nk, nr_left, n_middle, nc_right);
                        // i_right depens on ell, j
                        size_t i_right = right(ell, j, k, nk, nr_left, n_middle, nc_right);
                        bool var_left = select_x[i_left] & (type_x[i_left] == CppAD::variable_enum);
                        bool var_right = select_x[i_right] & (type_x[i_right] == CppAD::variable_enum);
                        if (var_left & var_right) nnz += 2;
                    }
                }
            }
        }
            ...
            for (size_t j = 0; j < nc_right; ++j) {
                ...
                if (select_y[i_result]) {
                    for (size_t ell = 0; ell < n_middle; ++ell) {
                        ...
                        bool var_left = select_x[i_left] & (type_x[i_left] == CppAD::variable_enum);
                        bool var_right = select_x[i_right] & (type_x[i_right] == CppAD::variable_enum);
                        if (var_left & var_right) {  // Cannot possibly set the same (i_left, i_right)
                            // pair twice.
    ...
    $srccode%cpp% */
    // calculate depend_x
    virtual bool rev_depend(const vector<double> &parameter_x, const vector<CppAD::ad_type_enum> &type_x,
        ...
        //
        // initialize depend_x
        for (size_t ell = 0; ell < 3; ++ell) depend_x[ell] = true;                 // always need these parameters
        for (size_t ell = 3; ell < depend_x.size(); ++ell) depend_x[ell] = false;  // initialize as false
        //
        // commpute depend_x
        size_t nk = 1;  // number of orders
        size_t k = 0;   // order
        for (size_t i = 0; i < nr_left; ++i) {
            for (size_t j = 0; j < nc_right; ++j) {  // check depend for result[i, j]
                size_t i_result = result(i, j, k, nk, nr_left, n_middle, nc_right);
                if (depend_y[i_result]) {
                    for (size_t ell = 0; ell < n_middle; ++ell) {  // index for left(i, ell)
                        ...
                        bool zero_right = type_x[i_right] == CppAD::constant_enum;
                        zero_right &= parameter_x[i_right] == 0.0;
                        if (!zero_right) depend_x[i_left] = true;
                        if (!zero_left) depend_x[i_right] = true;
                    }
                }
            }
        }
