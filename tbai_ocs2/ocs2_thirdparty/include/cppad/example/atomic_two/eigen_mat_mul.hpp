...
class atomic_eigen_mat_mul : public CppAD::atomic_base<Base> {
   public:
    ...
    atomic_eigen_mat_mul(void)
        : CppAD::atomic_base<Base>("atom_eigen_mat_mul", CppAD::atomic_base<Base>::set_sparsity_enum) {}
    /* %$$
    $subhead op$$
    $srccode%cpp% */
    ...
    ad_matrix op(const ad_matrix &left, const ad_matrix &right) {
        ...
        for (size_t i = 0; i < n_left; i++) packed_arg[3 + i] = left.data()[i];
        for (size_t i = 0; i < n_right; i++) packed_arg[3 + n_left + i] = right.data()[i];
        // ------------------------------------------------------------------
        ...
        for (size_t i = 0; i < n_result; i++) result.data()[i] = packed_result[i];
        //
        return result;
    }
    /* %$$
   ...
   private:
    ...
    virtual bool forward(
        ...
        for (size_t k = 0; k < n_order; k++) {  // unpack left values for this order
            for (size_t i = 0; i < n_left; i++) f_left_[k].data()[i] = tx[(3 + i) * n_order + k];
            //
            // unpack right values for this order
            for (size_t i = 0; i < n_right; i++) f_right_[k].data()[i] = tx[(3 + n_left + i) * n_order + k];
        }
        // -------------------------------------------------------------------
        // result for each order
        // (we could avoid recalculting f_result_[k] for k=0,...,p-1)
        for (size_t k = 0; k < n_order; k++) {  // result[k] = sum_ell left[ell] * right[k-ell]
            f_result_[k] = matrix::Zero(long(nr_left), long(nc_right));
            for (size_t ell = 0; ell <= k; ell++) f_result_[k] += f_left_[ell] * f_right_[k - ell];
        }
        // -------------------------------------------------------------------
        // pack result_ into ty
        for (size_t k = 0; k < n_order; k++) {
            for (size_t i = 0; i < n_result; i++) ty[i * n_order + k] = f_result_[k].data()[i];
        }
        // ------------------------------------------------------------------
        ...
        for (size_t i = 0; i < nr_left; i++) {
            for (size_t j = 0; j < nc_right; j++) {
                bool var = false;
                for (size_t ell = 0; ell < n_middle; ell++) {  // left information
                    size_t index = 3 + i * n_middle + ell;
                    bool var_left = vx[index];
                    bool nz_left = var_left | (f_left_[0](long(i), long(ell)) != zero);
                    // right information
                    index = 3 + n_left + ell * nc_right + j;
                    bool var_right = vx[index];
                    bool nz_right = var_right | (f_right_[0](long(ell), long(j)) != zero);
                    // effect of result
    ...
    virtual bool reverse(
        ...
        for (size_t k = 0; k < n_order; k++) {
            for (size_t i = 0; i < n_result; i++) r_result_[k].data()[i] = py[i * n_order + k];
        }
        // -------------------------------------------------------------------
        ...
        for (size_t k1 = n_order; k1 > 0; k1--) {
            size_t k = k1 - 1;
            for (size_t ell = 0; ell <= k; ell++) {  // nr x nm       = nr x nc      * nc * nm
        ...
        for (size_t k = 0; k < n_order; k++) {  // dimensions are integer constants
            px[0 * n_order + k] = 0.0;
            px[1 * n_order + k] = 0.0;
            px[2 * n_order + k] = 0.0;
            //
            // pack left values for this order
            for (size_t i = 0; i < n_left; i++) px[(3 + i) * n_order + k] = r_left_[k].data()[i];
            //
            // pack right values for this order
            for (size_t i = 0; i < n_right; i++) px[(3 + i + n_left) * n_order + k] = r_right_[k].data()[i];
        }
        //
        return true;
    }
    ...
    virtual bool for_sparse_jac(
        ...
        CppAD::vector<std::set<size_t> > &s, const CppAD::vector<Base> &x) {
        ...
        size_t n_left = nr_left * n_middle;
        for (size_t i = 0; i < nr_left; i++) {
            for (size_t j = 0; j < nc_right; j++) {  // pack index for entry (i, j) in result
                size_t i_result = i * nc_right + j;
                s[i_result].clear();
                for (size_t ell = 0; ell < n_middle; ell++) {  // pack index for entry (i, ell) in left
    ...
    virtual bool rev_sparse_jac(
        ...
        CppAD::vector<std::set<size_t> > &st, const CppAD::vector<Base> &x) {
        size_t nr_left = size_t(CppAD::Integer(x[0]));
        size_t n_middle = size_t(CppAD::Integer(x[1]));
        size_t nc_right = size_t(CppAD::Integer(x[2]));
        size_t nx = 3 + (nr_left + nc_right) * n_middle;
        ...
        for (size_t i = 0; i < nx; i++) st[i].clear();
    ...
    virtual bool for_sparse_hes(
        ...
        CppAD::vector<std::set<size_t> > &h, const CppAD::vector<Base> &x) {
        size_t nr_left = size_t(CppAD::Integer(x[0]));
        size_t n_middle = size_t(CppAD::Integer(x[1]));
        size_t nc_right = size_t(CppAD::Integer(x[2]));
        size_t nx = 3 + (nr_left + nc_right) * n_middle;
        ...
        for (size_t i = 0; i < nx; i++) h[i].clear();
        //
        size_t n_left = nr_left * n_middle;
        for (size_t i = 0; i < nr_left; i++) {
            for (size_t j = 0; j < nc_right; j++) {  // pack index for entry (i, j) in result
