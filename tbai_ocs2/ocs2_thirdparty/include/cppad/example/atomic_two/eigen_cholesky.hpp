...
class atomic_eigen_cholesky : public CppAD::atomic_base<Base> {
   public:
    ...
    atomic_eigen_cholesky(void)
        : CppAD::atomic_base<Base>("atom_eigen_cholesky", CppAD::atomic_base<Base>::set_sparsity_enum) {}
    /* %$$
    $subhead op$$
    $srccode%cpp% */
    ...
    ad_matrix op(const ad_matrix &arg) {
        ...
        for (size_t i = 0; i < nr; i++) {
            for (size_t j = 0; j <= i; j++) packed_arg[index++] = arg(long(i), long(j));
        }
        assert(index == nx);
        // -------------------------------------------------------------------
        ...
        index = 0;
        for (size_t i = 0; i < nr; i++) {
            for (size_t j = 0; j <= i; j++) result(long(i), long(j)) = packed_result[index++];
        }
        return result;
    }
    /* %$$
...
$srccode%cpp% */
   private:
    ...
    virtual bool forward(
        ...
        for (size_t k = 1; k < n_order; k++) {  // initialize sum as A_k
            matrix f_sum = f_arg_[k];
            // compute A_k - B_k
            for (size_t ell = 1; ell < k; ell++) f_sum -= f_result_[ell] * f_result_[k - ell].transpose();
            ...
            for (long i = 0; i < long(nr); i++) temp(i, i) /= scalar(2.0);
    ...
    virtual bool reverse(
        ...
        for (size_t k1 = n_order; k1 > 1; k1--) {
            ...
            //
            // low[ L_0^T * bar{L}_k ]
            for (long i = 0; i < long(nr); i++) tmp1(i, i) /= scalar(2.0);
            ...
            matrix M_k = L_0.transpose().template solve<Eigen::OnTheLeft>(tmp1.transpose());
            //
            ...
            //
            // remove B_k
            for (size_t ell = 1; ell < k; ell++) {  // bar{L}_ell = 2 * lower( \bar{B}_k * L_{k-ell} )
        ...
        for (long i = 0; i < long(nr); i++) M_0(i, i) /= scalar(2.0);
