...
class atomic_eigen_mat_inv : public CppAD::atomic_base<Base> {
   public:
    ...
    atomic_eigen_mat_inv(void)
        : CppAD::atomic_base<Base>("atom_eigen_mat_inv", CppAD::atomic_base<Base>::set_sparsity_enum) {}
    /* %$$
    $subhead op$$
    $srccode%cpp% */
   ...
   private:
    ...
    virtual bool forward(
        ...
        for (size_t k = 0; k < n_order; k++) {  // unpack arg values for this order
            for (size_t i = 0; i < ny; i++) f_arg_[k].data()[i] = tx[(1 + i) * n_order + k];
        }
        // -------------------------------------------------------------------
        ...
        for (size_t k = 1; k < n_order; k++) {  // initialize sum
            ...
            // compute sum
            for (size_t ell = 1; ell <= k; ell++) f_sum -= f_arg_[ell] * f_result_[k - ell];
        ...
        for (size_t k = 0; k < n_order; k++) {
            for (size_t i = 0; i < ny; i++) ty[i * n_order + k] = f_result_[k].data()[i];
        }
        // -------------------------------------------------------------------
    ...
    virtual bool reverse(
        ...
        for (size_t k = 0; k < n_order; k++) {
            for (size_t i = 0; i < ny; i++) r_result_[k].data()[i] = py[i * n_order + k];
        }
        // -------------------------------------------------------------------
        // initialize r_arg_ as zero
        for (size_t k = 0; k < n_order; k++) r_arg_[k] = matrix::Zero(long(nr), long(nr));
        // -------------------------------------------------------------------
        ...
        for (size_t k1 = n_order; k1 > 1; k1--) {
            ...
            r_result_[0] += r_result_[k] * f_result_[k].transpose() * f_arg_[0].transpose();
            //
            for (size_t ell = 1; ell <= k; ell++) {  // bar{A}_l = bar{A}_l - R_0^T bar{R}_k R_{k-l}^T
                r_arg_[ell] -= f_result_[0].transpose() * r_result_[k] * f_result_[k - ell].transpose();
                // bar{R}_{k-l} = bar{R}_{k-1} - (R_0 A_l)^T bar{R}_k
                r_result_[k - ell] -= f_arg_[ell].transpose() * f_result_[0].transpose() * r_result_[k];
            }
        }
        r_arg_[0] -= f_result_[0].transpose() * r_result_[0] * f_result_[0].transpose();
        // -------------------------------------------------------------------
        // pack r_arg into px
        for (size_t k = 0; k < n_order; k++) {
            for (size_t i = 0; i < ny; i++) px[(1 + i) * n_order + k] = r_arg_[k].data()[i];
        }
        //
        return true;
    }
