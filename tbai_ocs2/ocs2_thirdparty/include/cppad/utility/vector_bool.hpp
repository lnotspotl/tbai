...
class vectorBool {
    ...
    */
    ...
    vectorBool(size_t n) : n_unit_(0), length_(n), data_(CPPAD_NULL) { resize(n); }
    vectorBool(const vectorBool &other) : n_unit_(0), length_(0), data_(CPPAD_NULL) {
        resize(other.length_);
        size_t n_used = unit_min();
        CPPAD_ASSERT_UNKNOWN(n_used <= n_unit_);
        for (size_t i = 0; i < n_used; ++i) data_[i] = other.data_[i];
    }
    ~vectorBool(void) { clear(); }
    /* %$$
    $end
    -----------------------------------------------------------------------------
    ...
    */
    ...
    {  // If original length is zero, then resize it to other.
        ...
        if (length_ == 0) resize(other.length_);
        CPPAD_ASSERT_KNOWN(length_ == other.length_, "vectorBool: size miss match in assignment operation");
        size_t n_used = unit_min();
        CPPAD_ASSERT_UNKNOWN(n_used <= n_unit_);
        for (size_t i = 0; i < n_used; i++) data_[i] = other.data_[i];
        return *this;
    ...
    // END_MOVE_SEMANTICS
    {
        CPPAD_ASSERT_KNOWN(length_ == other.length_ || (length_ == 0),
    ...
    bool operator[](size_t i) const {
        CPPAD_ASSERT_KNOWN(i < length_, "vectorBool: index greater than or equal vector size");
    ...
    */
    ...
    // END_PUSH_BACK
    {
        ...
        if (length_ + 1 > n_unit_ * bit_per_unit_) {
            ...
            size_t n_used = unit_min();
            CPPAD_ASSERT_UNKNOWN(n_used <= n_unit_);
            for (size_t i = 0; i < n_used; ++i) vec.data_[i] = data_[i];
    ...
    */
    ...
    void push_vector(const Vector &other)
    // END_PUSH_VECTOR
    {
        CPPAD_ASSERT_UNKNOWN(unit_min() <= n_unit_);
