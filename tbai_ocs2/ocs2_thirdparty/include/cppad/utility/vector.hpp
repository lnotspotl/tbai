...
class vector {
   private:
    ...
    Type *data_;
    /// delete data pointer
    void delete_data(Type *data_ptr) { thread_alloc::delete_array(data_ptr); }
   ...
   public:
    ...
    vector(
        ...
        // copy the data
        for (size_t i = 0; i < length_; i++) data_[i] = x.data_[i];
    }
    /// destructor
    ~vector(void) {
    ...
    vector &operator=(
        ...
        if (length_ == 0) resize(x.length_);
        CPPAD_ASSERT_KNOWN(length_ == x.length_, "vector: size miss match in assignment operation");
        for (i = 0; i < length_; i++) data_[i] = x.data_[i];
        return *this;
    }
    ...
    vector &operator=(
        /// right hand size of the assingment operation
        vector &&x) {
        CPPAD_ASSERT_KNOWN(length_ == x.length_ || (length_ == 0), "vector: size miss match in assignment operation");
        if (this != &x) {
            clear();
            //
            length_ = x.length_;
    ...
    Type &operator[](
        ...
        size_t i) {
        CPPAD_ASSERT_KNOWN(i < length_, "vector: index greater than or equal vector size");
        return data_[i];
    }
    /// non-constant element access; i.e., we can change this element value
    ...
    void push_back(
        ...
        const Type &s) {  // case where no allocation is necessary
        ...
        for (size_t i = 0; i < old_length; i++) data_[i] = old_data[i];
        ...
        // free old data
        if (old_capacity > 0) delete_data(old_data);
    ...
    */
    ...
    void push_vector(
        ...
        if (length_ + m <= capacity_) {
            for (size_t i = 0; i < m; i++) data_[length_++] = v[i];
            return;
        }
        ...
        for (size_t i = 0; i < m; i++) data_[old_length + i] = v[i];
...
class vectorBool {
   ...
   public:
    ...
    static size_t bit_per_unit(void) { return bit_per_unit_; }
    ...
    vectorBool(
        ...
        if (length_ > 0) {  // set n_unit and data
    ...
    vectorBool(
        ...
        if (length_ > 0) {  // set n_unit and data
            ...
            size_t i;
            for (i = 0; i < min_unit; i++) data_[i] = v.data_[i];
        }
    }
    /// destructor
    ~vectorBool(void) {
        if (n_unit_ > 0) thread_alloc::delete_array(data_);
    }
    ...
    /// maximum number of elements current allocation can store
    size_t capacity(void) const { return n_unit_ * bit_per_unit_; }
    ...
    vectorBool &operator=(
        ...
        // If original length is zero, then resize it.
        // Otherwise a length mismatch is an error.
        if (length_ == 0) resize(v.length_);
        CPPAD_ASSERT_KNOWN(length_ == v.length_, "vectorBool: size miss match in assignment operation");
        size_t min_unit = unit_min();
    ...
    vectorBool &operator=(
        /// right hand size of the assingment operation
        vectorBool &&x) {
        CPPAD_ASSERT_KNOWN(length_ == x.length_ || (length_ == 0),
    ...
    vectorBoolElement operator[](
        ...
        size_t k) {
        size_t i, j;
        CPPAD_ASSERT_KNOWN(k < length_, "vectorBool: index greater than or equal vector size");
        i = k / bit_per_unit_;
        j = k - i * bit_per_unit_;
    ...
    void push_back(
        ...
        if (length_ + 1 > n_unit_ * bit_per_unit_) {
            ...
            // copy old data values
            for (i = 0; i < old_n_unit; i++) data_[i] = old_data[i];
            // free old data
            if (old_n_unit > 0) thread_alloc::delete_array(old_data);
        }
    ...
    void push_vector(
        ...
        min_unit = unit_min();
        if (length_ >= n_unit_ * bit_per_unit_) {  // store old n_unit and data value
            size_t old_n_unit = n_unit_;
            UnitType *old_data = data_;
            // set new n_unit and data values
            data_ = thread_alloc::create_array<UnitType>(min_unit, n_unit_);
