...
class thread_alloc {
   ...
   private:
    class capacity_t {
       public:
        ...
        /// ctor
        capacity_t(void) {  // Cannot figure out how to call thread_alloc::in_parallel here.
        ...
        if (clear) {
            if (info != CPPAD_NULL) {
                ...
                for (size_t c = 0; c < CPPAD_MAX_NUM_CAPACITY; c++) {
                    CPPAD_ASSERT_UNKNOWN(info->root_inuse_[c].next_ == CPPAD_NULL &&
                                         info->root_available_[c].next_ == CPPAD_NULL);
                }
#endif
                if (thread != 0) ::operator delete(reinterpret_cast<void *>(info));
                info = CPPAD_NULL;
    ...
    */
    static void inc_inuse(size_t inc, size_t thread) {
        ...
        CPPAD_ASSERT_UNKNOWN(thread == thread_num() || (!in_parallel()));
    ...
    */
    static void dec_inuse(size_t dec, size_t thread) {
        CPPAD_ASSERT_UNKNOWN(thread < num_threads() || (!in_parallel()));
        CPPAD_ASSERT_UNKNOWN(thread == thread_num() || (!in_parallel()));
        thread_alloc_info *info = thread_info(thread);
    ...
    */
    static size_t set_get_num_threads(size_t number_new) {
        ...
        if (number_new != 0) number_user = number_new;
    ...
    */
    static size_t set_get_thread_num(size_t (*thread_num_new)(void), bool set = false) {
        ...
        CPPAD_ASSERT_KNOWN(thread < set_get_num_threads(0), "parallel_setup: thread_num() >= num_threads");
        return thread;
    }
    // ============================================================================
   public:
    ...
    */
    static void parallel_setup(size_t num_threads, bool (*in_parallel)(void), size_t (*thread_num)(void)) {
        ...
        CPPAD_ASSERT_KNOWN(num_threads <= CPPAD_MAX_NUM_THREADS, "parallel_setup: num_threads is too large");
        CPPAD_ASSERT_KNOWN(num_threads != 0, "parallel_setup: num_threads == zero");
        CPPAD_ASSERT_KNOWN(in_parallel != CPPAD_NULL, "parallel_setup: num_threads != 1 and in_parallel == CPPAD_NULL");
        CPPAD_ASSERT_KNOWN(thread_num != CPPAD_NULL, "parallel_setup: num_threads != 1 and thread_num == CPPAD_NULL");

        // Make sure that constructors for all static variables in this file
        // are called in sequential mode.
        for (size_t thread = 0; thread < num_threads; thread++) thread_info(thread);
        capacity_info();
        size_t cap_bytes;
    ...
    */
    static size_t num_threads(void) { return set_get_num_threads(0); }
    /* -----------------------------------------------------------------------
    ...
    */
    ...
    static bool in_parallel(void) {  // emphasize that this routine is outside thread_alloc class
    ...
    */
    ...
    static size_t thread_num(void) { return set_get_thread_num(CPPAD_NULL); }
    /* -----------------------------------------------------------------------
    $begin ta_get_memory$$
    $spell
        std
    ...
    */
    static void *get_memory(size_t min_bytes, size_t &cap_bytes) {  // see first_trace below
        CPPAD_ASSERT_FIRST_CALL_NOT_PARALLEL;

        // check that number of requested bytes is not to large
        CPPAD_ASSERT_KNOWN(min_bytes < std::numeric_limits<size_t>::max() / 2,
        ...
        if (cap_bytes == CPPAD_TRACE_CAPACITY && thread == CPPAD_TRACE_THREAD && first_trace) {
            cout << endl;
            cout << "thread_alloc: Trace for Thread = " << thread;
            cout << " and capacity = " << cap_bytes << endl;
            if (first_trace) first_trace = false;
        }
        ...
        if (node != CPPAD_NULL) {
            ...
            // trace allocation
            if (cap_bytes == CPPAD_TRACE_CAPACITY && thread == CPPAD_TRACE_THREAD) {
    ...
    */
    static void return_memory(void *v_ptr) {
        ...
        CPPAD_ASSERT_UNKNOWN(thread < CPPAD_MAX_NUM_THREADS);
        CPPAD_ASSERT_KNOWN(thread == thread_num() || (!in_parallel()),
    ...
    */
    static void free_available(size_t thread) {
        CPPAD_ASSERT_KNOWN(thread < CPPAD_MAX_NUM_THREADS,
        ...
        if (inuse(thread) == 0) {  // clear the information for this thread
    ...
    */
    template <class Type>
    static Type *create_array(size_t size_min, size_t &size_out) {  // minimum number of bytes to allocate
        ...
        size_t i;
        for (i = 0; i < size_out; i++) new (array + i) Type();
    ...
    */
    template <class Type>
    static void delete_array(Type *array) {  // determine the number of values in the array
        block_t *node = reinterpret_cast<block_t *>(array) - 1;
        size_t size = node->extra_;

        // call destructor for each element
        size_t i;
        for (i = 0; i < size; i++) (array + i)->~Type();
    ...
    */
    static bool free_all(void) {
        CPPAD_ASSERT_KNOWN(!in_parallel(), "free_all cannot be used while in parallel execution");
        bool ok = true;
