...
template <class Base>
void forward_zmulvv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                       Base *taylor) {
    // check assumptions
    ...
    for (size_t d = p; d <= q; d++) {
        z[d] = Base(0.0);
        for (k = 0; k <= d; k++) z[d] += azmul(x[d - k], y[k]);
    }
}
/*!
...
template <class Base>
void forward_zmulvv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                           Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ZmulvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(ZmulvvOp) == 1);
    ...
    for (ell = 0; ell < r; ell++) {
        ...
        for (k = 1; k < q; k++) z[m] += azmul(x[(q - k - 1) * r + ell + 1], y[(k - 1) * r + ell + 1]);
    }
}
...
template <class Base>
void reverse_zmulvv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
...
template <class Base>
void forward_zmulpv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                       Base *taylor) {
    // check assumptions
    ...
    for (size_t d = p; d <= q; d++) z[d] = azmul(x, y[d]);
}
/*!
...
template <class Base>
void forward_zmulpv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                           Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ZmulpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(ZmulpvOp) == 1);
    ...
    for (size_t ell = 0; ell < r; ell++) z[ell] = azmul(x, y[ell]);
}
/*!
...
template <class Base>
void reverse_zmulpv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                       const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ZmulpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(ZmulpvOp) == 1);
...
template <class Base>
void forward_zmulvp_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                       Base *taylor) {
    // check assumptions
    ...
    for (size_t d = p; d <= q; d++) z[d] = azmul(x[d], y);
}
/*!
...
template <class Base>
void forward_zmulvp_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                           Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ZmulvpOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(ZmulvpOp) == 1);
    ...
    for (size_t ell = 0; ell < r; ell++) z[ell] = azmul(x[ell], y);
}
/*!
...
template <class Base>
void reverse_zmulvp_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                       const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ZmulvpOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(ZmulvpOp) == 1);
