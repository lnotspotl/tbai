...
template <class Base>
void forward_addvv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
    ...
    for (size_t j = p; j <= q; j++) z[j] = x[j] + y[j];
}
/*!
...
template <class Base>
void forward_addvv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(AddvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(AddvvOp) == 1);
    ...
    size_t m = (q - 1) * r + 1;
    for (size_t ell = 0; ell < r; ell++) z[m + ell] = x[m + ell] + y[m + ell];
}
...
template <class Base>
void reverse_addvv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
...
*/
template <class Base>
void forward_addpv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
    ...
    if (p == 0) {  // Paraemter value
        Base x = parameter[arg[0]];
        z[0] = x + y[0];
        p++;
    }
    for (size_t j = p; j <= q; j++) z[j] = y[j];
}
/*!
...
*/
template <class Base>
void forward_addpv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(AddpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(AddpvOp) == 1);
...
template <class Base>
void reverse_addpv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(AddvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(AddvvOp) == 1);
