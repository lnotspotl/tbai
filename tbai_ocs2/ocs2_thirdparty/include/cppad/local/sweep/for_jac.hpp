...
void for_jac(const local::player<Base> *play, bool dependency, size_t n, size_t numvar, Vector_set &var_sparsity,
    ...
    if (num_vecad_vec > 0) {
        ...
        for (i = 0; i < num_vecad_vec; i++) {  // length of this VecAD
            ...
            vecad_ind[j] = i;
            for (k = 1; k <= length; k++) vecad_ind[j + k] = num_vecad_vec;  // invalid index
            // start of next VecAD
            j += length + 1;
        }
    ...
    while (more_operators) {
        ...
        switch (op) {
            case AbsOp:
                ...
                forward_sparse_jacobian_unary_op(i_var, size_t(arg[0]), var_sparsity);
                break;
                // -------------------------------------------------

            case AddvvOp:
                CPPAD_ASSERT_NARG_NRES(op, 2, 1);
                forward_sparse_jacobian_binary_op(i_var, arg, var_sparsity);
                break;
                // -------------------------------------------------

            case AddpvOp:
                CPPAD_ASSERT_NARG_NRES(op, 2, 1);
                forward_sparse_jacobian_unary_op(i_var, size_t(arg[1]), var_sparsity);
                break;
                // -------------------------------------------------
            ...
            case CSumOp:
                forward_sparse_jacobian_csum_op(i_var, arg, var_sparsity);
                itr.correct_before_increment();
                break;
                // -------------------------------------------------

            case CExpOp:
                forward_sparse_jacobian_cond_op(dependency, i_var, arg, num_par, var_sparsity);
                break;
                // --------------------------------------------------
            ...
            case LdpOp:
                forward_sparse_load_op(dependency, op, i_var, arg, num_vecad_ind, vecad_ind.data(), var_sparsity,
                                       vecad_sparsity);
                break;
                // -------------------------------------------------
            ...
            case StpvOp:
                forward_sparse_store_op(dependency, op, arg, num_vecad_ind, vecad_ind.data(), var_sparsity,
                                        vecad_sparsity);
                break;
                // -------------------------------------------------
            ...
            case AFunOp:
                ...
                if (flag) {
                    ...
                    //
                    call_atomic_for_jac_sparsity<Base, RecBase>(atom_index, atom_old, dependency, atom_x, type_x,
                                                                atom_ix, atom_iy, var_sparsity);
                }
                break;
        ...
        if (op == AFunOp && atom_state == start_atom) {  // print operators that have been delayed
            ...
            for (i = 0; i < atom_m; i++) {
                ...
                for (j = 0; j < limit; j++) z_value[j] = false;
                ...
                printOp<Base, RecBase>(std::cout, play, itr.op_index() - atom_m + i, j_var, op_tmp, arg_tmp);
                if (j_var > 0) printOpResult(std::cout, 1, &z_value, 0, (CppAD::vectorBool *)CPPAD_NULL);
                std::cout << std::endl;
            }
        }
        // value for this variable
        ...
        if (!delay_print) {
            ...
            if (NumRes(op) > 0 && (!delay_print))
                printOpResult(std::cout, 1, &z_value, 0, (CppAD::vectorBool *)CPPAD_NULL);
            std::cout << std::endl;
        }
    }
    std::cout << std::endl;
