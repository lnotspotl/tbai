...
void forward2(const local::player<Base> *play, const size_t q, const size_t r, const size_t n, const size_t numvar,
              const size_t J, Base *taylor, const bool *cskip_op, const pod_vector<Addr> &var_by_load_op,
    ...
    while (more_operators) {
        ...
        while (cskip_op[itr.op_index()]) {
            switch (op) {
                case AFunOp: {  // get information for this atomic function call
                    ...
                    for (i = 0; i < atom_m + atom_n + 1; ++i) ++itr;
#ifndef NDEBUG
                    itr.op_info(op, arg, i_var);
        ...
        switch (op) {
            ...
            case CExpOp:
                forward_cond_op_dir(q, r, i_var, arg, num_par, parameter, J, taylor);
                break;
                // ---------------------------------------------------
            ...
            case CSumOp:
                forward_csum_op_dir(q, r, i_var, arg, num_par, parameter, J, taylor);
                itr.correct_before_increment();
                break;
                // -------------------------------------------------
            ...
            case LdpOp:
            case LdvOp:
                forward_load_op(play, op, p, q, r, J, i_var, arg, var_by_load_op.data(), taylor);
                break;
                // ---------------------------------------------------
            ...
            case ParOp:
                ...
                for (ell = 0; ell < r; ell++) taylor[k + ell] = Base(0.0);
                break;
                // -------------------------------------------------
            ...
            case AFunOp:
                ...
                if (flag) {
                    ...
                    for (ell = 0; ell < r; ell++) {  // set atom_tx
                        for (j = 0; j < atom_n; j++) {
                        ...
                        call_atomic_forward<Base, RecBase>(atom_par_x, atom_type_x, need_y, order_low, order_up,
                                                           atom_index, atom_old, atom_tx_one, atom_ty_one);
                        for (i = 0; i < atom_m; i++) {
                            if (atom_iy[i] > 0) {
            ...
            case FunapOp:
                ...
                for (ell = 0; ell < r; ell++)
                    for (k = 1; k < atom_q1; k++) atom_tx_all[atom_j * (q * r + 1) + (k - 1) * r + 1 + ell] = Base(0.0);
                //
                ++atom_j;
            ...
            case FunrpOp:
                ...
                for (ell = 0; ell < r; ell++)
                    for (k = 1; k < atom_q1; k++) atom_ty_all[atom_i * (q * r + 1) + (k - 1) * r + 1 + ell] = Base(0.0);
                //
                ++atom_i;
        ...
        if (atom_trace) {
            ...
            for (i = 0; i < atom_m; i++)
                if (atom_iy[i] > 0) {
                    ...
                    printOp<Base, RecBase>(std::cout, play, i_tmp, atom_iy[i], FunrvOp, CPPAD_NULL);
                    Base *Z_tmp = taylor + atom_iy[i] * ((J - 1) * r + 1);
                    {
                        Z_vec[0] = Z_tmp[0];
                        for (ell = 0; ell < r; ell++) {
                            std::cout << std::endl << "     ";
                            for (size_t p_tmp = 1; p_tmp <= q; p_tmp++) Z_vec[p_tmp] = Z_tmp[(p_tmp - 1) * r + ell + 1];
                            printOpResult(std::cout, q + 1, Z_vec.data(), 0, (Base *)CPPAD_NULL);
                        }
                    }
                    std::cout << std::endl;
                }
