...
void reverse(size_t d, size_t n, size_t numvar, const local::player<Base> *play, size_t J, const Base *Taylor, size_t K,
             Base *Partial, bool *cskip_op, const pod_vector<Addr> &var_by_load_op, Iterator &play_itr,
    ...
    if (num_par > 0) parameter = play->GetPar();
    ...
    while (op != BeginOp) {
        ...
        while (cskip_op[i_op]) {
            switch (op) {
                case AFunOp: {  // get information for this atomic function call
                    CPPAD_ASSERT_UNKNOWN(atom_state == end_atom);
                    play::atom_op_info<Base>(op, arg, atom_index, atom_old, atom_m, atom_n);
                    //
                    // skip to the first AFunOp
                    for (size_t i = 0; i < atom_m + atom_n + 1; ++i) --play_itr;
                    play_itr.op_info(op, arg, i_var);
        ...
        printOp<Base, RecBase>(std::cout, play, i_op, i_tmp, op, arg);
        if (NumRes(op) > 0 && op != BeginOp) printOpResult(std::cout, d + 1, Z_tmp, d + 1, pZ_tmp);
        std::cout << std::endl;
#endif
        switch (op) {
            case AbsOp:
                reverse_abs_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case AcosOp:
                ...
                reverse_acos_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case AcoshOp:
                // sqrt(x * x - 1), acosh(x)
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_acosh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
#endif
                // --------------------------------------------------

            case AddvvOp:
                reverse_addvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case AddpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_addpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case AsinOp:
                // sqrt(1 - x * x), asin(x)
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_asin_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case AsinhOp:
                // sqrt(1 + x * x), asinh(x)
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_asinh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
#endif
                // --------------------------------------------------
            ...
            case AtanOp:
                // 1 + x * x, atan(x)
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_atan_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // -------------------------------------------------
            ...
            case AtanhOp:
                // 1 - x * x, atanh(x)
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_atanh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
#endif
                // -------------------------------------------------
            ...
            case CSumOp:
                play_itr.correct_after_decrement(arg);
                reverse_csum_op(d, i_var, arg, K, Partial);
                // end of a cumulative summation
                break;
                // -------------------------------------------------

            case CExpOp:
                reverse_cond_op(d, i_var, arg, num_par, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case CosOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_cos_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case CoshOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_cosh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case DivvvOp:
                reverse_divvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case DivpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_divpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case DivvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_divvp_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
            // --------------------------------------------------
            case EndOp:
                CPPAD_ASSERT_UNKNOWN(i_op == play->num_op_rec() - 1);
                break;
...
#if CPPAD_USE_CPLUSPLUS_2011
            case ErfOp:
                reverse_erf_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
#endif
                // --------------------------------------------------

            case ExpOp:
                reverse_exp_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

#if CPPAD_USE_CPLUSPLUS_2011
            case Expm1Op:
                reverse_expm1_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
#endif
                // --------------------------------------------------
            ...
            case LdpOp:
                reverse_load_op(op, d, i_var, arg, J, Taylor, K, Partial, var_by_load_op.data());
                break;
                // -------------------------------------------------
            ...
            case LogOp:
                reverse_log_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

#if CPPAD_USE_CPLUSPLUS_2011
            case Log1pOp:
                reverse_log1p_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
#endif
                // --------------------------------------------------
            ...
            case MulpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_mulpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case MulvvOp:
                reverse_mulvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case PowvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_powvp_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case PowpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_powpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case PowvvOp:
                reverse_powvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case SignOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_sign_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case SinOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_sin_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case SinhOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_sinh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case SqrtOp:
                reverse_sqrt_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case SubvvOp:
                reverse_subvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case SubpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_subpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case SubvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_subvp_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case TanOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_tan_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // -------------------------------------------------

            case TanhOp:
                CPPAD_ASSERT_UNKNOWN(i_var < numvar);
                reverse_tanh_op(d, i_var, size_t(arg[0]), J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
            ...
            case AFunOp:
                ...
                play::atom_op_info<RecBase>(op, arg, atom_index, atom_old, atom_m, atom_n);
                if (flag) {
                    atom_state = ret_atom;
                    atom_i = atom_m;
                    atom_j = atom_n;
                    ...
                    call_atomic_reverse<Base, RecBase>(atom_par_x, atom_type_x, atom_k, atom_index, atom_old, atom_tx,
                                                       atom_ty, atom_px, atom_py);
                    for (j = 0; j < atom_n; j++)
                        if (atom_ix[j] > 0) {
                            for (ell = 0; ell < atom_k1; ell++)
                                Partial[atom_ix[j] * K + ell] += atom_px[j * atom_k1 + ell];
                        }
                }
                break;
            ...
            case FunapOp:
                ...
                for (ell = 1; ell < atom_k1; ell++) atom_tx[atom_j * atom_k1 + ell] = Base(0.);
                //
                if (atom_j == 0) atom_state = start_atom;
                break;
            ...
            case FunavOp:
                ...
                for (ell = 0; ell < atom_k1; ell++) atom_tx[atom_j * atom_k1 + ell] = Taylor[size_t(arg[0]) * J + ell];
                //
                if (atom_j == 0) atom_state = start_atom;
                break;
            ...
            case FunrpOp:
                ...
                //
                if (atom_i == 0) atom_state = arg_atom;
                break;
            ...
            case FunrvOp:
                ...
                --atom_i;
                for (ell = 0; ell < atom_k1; ell++) {
                    atom_py[atom_i * atom_k1 + ell] = Partial[i_var * K + ell];
                    atom_ty[atom_i * atom_k1 + ell] = Taylor[i_var * J + ell];
                }
                if (atom_i == 0) atom_state = arg_atom;
                break;
                // ------------------------------------------------------------
            ...
            case ZmulpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_zmulpv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case ZmulvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_zmulvp_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------

            case ZmulvvOp:
                reverse_zmulvv_op(d, i_var, arg, parameter, J, Taylor, K, Partial);
                break;
                // --------------------------------------------------
