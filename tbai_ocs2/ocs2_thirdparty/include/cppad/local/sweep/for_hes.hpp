...
void for_hes(const local::player<Base> *play, size_t n, size_t numvar, const Vector_set &for_jac_sparse,
             const Vector_set &rev_jac_sparse, Vector_set &for_hes_sparse, const RecBase &not_used_rec_base) {
    ...
    while (more_operators) {
        ...
        if (include) include = rev_jac_sparse.is_element(i_var, 0);
        //
        ...
        include |= op == FunrvOp;
        //
        if (include) switch (op) {  // operators that should not occurr
                // case BeginOp
                // -------------------------------------------------
                ...
                case Log1pOp:
                    ...
                    forward_sparse_hessian_nonlinear_unary_op(size_t(arg[0]), for_jac_sparse, for_hes_sparse);
                    break;
                    // -------------------------------------------------
                ...
                case DivvvOp:
                    ...
                    forward_sparse_hessian_div_op(arg, for_jac_sparse, for_hes_sparse);
                    break;
                    // -------------------------------------------------

                case DivpvOp:
                    CPPAD_ASSERT_NARG_NRES(op, 2, 1)
                    forward_sparse_hessian_nonlinear_unary_op(size_t(arg[1]), for_jac_sparse, for_hes_sparse);
                    break;
                    // -------------------------------------------------
                ...
                case MulvvOp:
                    CPPAD_ASSERT_NARG_NRES(op, 2, 1)
                    forward_sparse_hessian_mul_op(arg, for_jac_sparse, for_hes_sparse);
                    break;
                    // -------------------------------------------------
                ...
                case PowvvOp:
                    CPPAD_ASSERT_NARG_NRES(op, 2, 3)
                    forward_sparse_hessian_pow_op(arg, for_jac_sparse, for_hes_sparse);
                    break;
                    // -------------------------------------------------
                ...
                case AFunOp:
                    ...
                    if (flag) {
                        ...
                        //
                        call_atomic_for_hes_sparsity<Base, RecBase>(atom_index, atom_old, atom_x, type_x, atom_ix,
                                                                    atom_iy, for_jac_sparse, rev_jac_sparse,
                                                                    for_hes_sparse);
                    }
                    break;
        ...
        if (op == AFunOp && atom_state == start_atom) {  // print operators that have been delayed
            ...
            for (k = 0; k < atom_m; k++) {
                ...
                for (i = 0; i < limit; i++) {
                    zf_value[i] = false;
                    for (j = 0; j < limit; j++) zh_value[i * limit + j] = false;
                }
                ...
                printOp<Base, RecBase>(std::cout, play, itr.op_index() - atom_m + k, k_var, op_tmp, arg_tmp);
                if (k_var > 0) printOpResult(std::cout, 1, &zf_value, 1, &zh_value);
                std::cout << std::endl;
            }
        }
        for (i = 0; i < limit; i++) {
        ...
        if (!delay_print) {
            ...
            if (NumRes(op) > 0 && (!delay_print)) printOpResult(std::cout, 1, &zf_value, 1, &zh_value);
            std::cout << std::endl;
        }
    }
    std::cout << std::endl;
