...
void forward1(const local::player<Base> *play, std::ostream &s_out, const bool print, const size_t p, const size_t q,
              const size_t n, const size_t numvar, const size_t J, Base *taylor, bool *cskip_op,
              pod_vector<Addr> &var_by_load_op, size_t compare_change_count, size_t &compare_change_number,
              size_t &compare_change_op_index, const RecBase &not_used_rec_base) {
    ...
    */
    ...
    if (p == 0) {
        ...
        num = play->num_op_rec();
        for (i = 0; i < num; i++) cskip_op[i] = false;
    }
    ...
    if (num_text > 0) text = play->GetTxt(0);
    /*
    ...
    */
    ...
    while (more_operators) {
        ...
        switch (op) {
            ...
            case CExpOp:
                forward_cond_op(p, q, i_var, arg, num_par, parameter, J, taylor);
                break;
                // ---------------------------------------------------
            ...
            case CSkipOp:
                if (p == 0) {
                    forward_cskip_op_0(i_var, arg, num_par, parameter, J, taylor, cskip_op);
                }
                itr.correct_before_increment();
                break;
                // -------------------------------------------------
            ...
            case CSumOp:
                forward_csum_op(p, q, i_var, arg, num_par, parameter, J, taylor);
                itr.correct_before_increment();
                break;
                // -------------------------------------------------
            ...
            case EqppOp:
                if (compare_change_count) {
                    forward_eqpp_op_0(compare_change_number, arg, parameter);
                    {
                        if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                    }
                }
                break;
                // -------------------------------------------------
            ...
            case EqpvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_eqpv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case EqvvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_eqvv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LdpOp:
                if (p == 0) {
                    forward_load_p_op_0(play, i_var, arg, parameter, J, taylor, isvar_by_ind.data(),
                                        index_by_ind.data(), var_by_load_op.data());
                    if (p < q) forward_load_op(play, op, p + 1, q, r, J, i_var, arg, var_by_load_op.data(), taylor);
                } else
            ...
            case LdvOp:
                if (p == 0) {
                    forward_load_v_op_0(play, i_var, arg, parameter, J, taylor, isvar_by_ind.data(),
                                        index_by_ind.data(), var_by_load_op.data());
                    if (p < q) forward_load_op(play, op, p + 1, q, r, J, i_var, arg, var_by_load_op.data(), taylor);
                } else
                    forward_load_op(play, op, p, q, r, J, i_var, arg, var_by_load_op.data(), taylor);
            ...
            case LeppOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_lepp_op_0(compare_change_number, arg, parameter);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LepvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_lepv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LevpOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_levp_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LevvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_levv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LtppOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_ltpp_op_0(compare_change_number, arg, parameter);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LtpvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_ltpv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LtvpOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_ltvp_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case LtvvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_ltvv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case NeppOp:
                if (compare_change_count) {
                    forward_nepp_op_0(compare_change_number, arg, parameter);
                    {
                        if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                    }
                }
            ...
            case NepvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_nepv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case NevvOp:
                if ((p == 0) & (compare_change_count > 0)) {
                    forward_nevv_op_0(compare_change_number, arg, parameter, J, taylor);
                    if (compare_change_count == compare_change_number) compare_change_op_index = itr.op_index();
                }
                break;
                // -------------------------------------------------
            ...
            case ParOp:
                i = p;
                if (i == 0) {
                    forward_par_op_0(i_var, arg, num_par, parameter, J, taylor);
                    i++;
                }
                while (i <= q) {
            ...
            case PriOp:
                if ((p == 0) & print) forward_pri_0(s_out, arg, num_text, text, num_par, parameter, J, taylor);
                break;
                // -------------------------------------------------
            ...
            case StppOp:
                if (p == 0) {
                    forward_store_pp_op_0(i_var, arg, num_par, J, taylor, isvar_by_ind.data(), index_by_ind.data());
                }
                break;
                // -------------------------------------------------
            ...
            case StpvOp:
                if (p == 0) {
                    forward_store_pv_op_0(i_var, arg, num_par, J, taylor, isvar_by_ind.data(), index_by_ind.data());
                }
                break;
                // -------------------------------------------------
            ...
            case StvpOp:
                if (p == 0) {
                    forward_store_vp_op_0(i_var, arg, num_par, J, taylor, isvar_by_ind.data(), index_by_ind.data());
                }
                break;
                // -------------------------------------------------
            ...
            case StvvOp:
                if (p == 0) {
                    forward_store_vv_op_0(i_var, arg, num_par, J, taylor, isvar_by_ind.data(), index_by_ind.data());
                }
                break;
                // -------------------------------------------------
            ...
            case AFunOp:
                ...
                if (flag) {
                    ...
                    call_atomic_forward<Base, RecBase>(atom_par_x, atom_type_x, need_y, order_low, order_up, atom_index,
                                                       atom_old, atom_tx, atom_ty);
                    for (i = 0; i < atom_m; i++)
                        if (atom_iy[i] > 0)
                            for (k = p; k <= q; k++) taylor[atom_iy[i] * J + k] = atom_ty[i * atom_q1 + k];
#if CPPAD_FORWARD1_TRACE
                    atom_trace = true;
#endif
                }
            ...
            case FunapOp:
                ...
                for (k = 1; k < atom_q1; k++) atom_tx[atom_j * atom_q1 + k] = Base(0.0);
                //
                ++atom_j;
            ...
            case FunavOp:
                ...
                for (k = 0; k < atom_q1; k++) atom_tx[atom_j * atom_q1 + k] = taylor[size_t(arg[0]) * J + k];
                //
                ++atom_j;
                if (atom_j == atom_n) atom_state = ret_atom;
                break;
            ...
            case FunrpOp:
                ...
                for (k = 1; k < p; k++) atom_ty[atom_i * atom_q1 + k] = Base(0.0);
                //
                ++atom_i;
            ...
            case FunrvOp:
                ...
                for (k = 0; k < p; k++) atom_ty[atom_i * atom_q1 + k] = taylor[i_var * J + k];
                //
                ++atom_i;
                if (atom_i == atom_m) atom_state = end_atom;
                break;
        ...
        if (atom_trace) {
            ...
            for (i = 0; i < atom_m; i++)
                if (atom_iy[i] > 0) {
                    ...
                    printOpResult(std::cout, q + 1, Z_tmp, 0, (Base *)CPPAD_NULL);
                    std::cout << std::endl;
                }
        }
        ...
        if (op != FunrvOp) {
            ...
            if (NumRes(op) > 0) printOpResult(std::cout, q + 1, Z_tmp, 0, (Base *)CPPAD_NULL);
            std::cout << std::endl;
        }
    }
    std::cout << std::endl;
    ...
    if ((p == 0) & (compare_change_count == 0)) compare_change_number = 0;
    return;
}
