...
void dynamic(pod_vector_maybe<Base> &all_par_vec, const BaseVector &ind_dynamic, const pod_vector<bool> &dyn_par_is,
             const pod_vector<addr_t> &dyn_ind2par_ind, const pod_vector<opcode_t> &dyn_par_op,
             const pod_vector<addr_t> &dyn_par_arg, const RecBase &not_used_rec_base) {
    ...
    for (size_t j = 0; j < ind_dynamic.size(); ++j)
        CPPAD_ASSERT_UNKNOWN(dyn_par_is[j + 1] && op_code_dyn(dyn_par_op[j]) == ind_dyn);
#endif
#if CPPAD_DYNAMIC_TRACE
    const char *cond_exp_name[] = {"CondExpLt", "CondExpLe", "CondExpEq", "CondExpGe", "CondExpGt", "CondExpNe"};
    std::cout << std::endl
              << std::setw(10) << std::left << "index" << std::setw(10) << std::left << "old" << std::setw(10)
              << std::left << "new" << std::setw(11) << std::left << "op" << std::setw(26) << std::right
              << "dynamic i=, constant v=" << std::endl;
#endif
    // used to hold the first two parameter arguments
    const Base *par[2];
    for (size_t j = 0; j < 2; ++j) par[j] = CPPAD_NULL;
    //
    ...
    size_t i_dyn = 0;
    while (i_dyn < num_dynamic_par) {  // number of dynamic parameters created by this operator
        size_t n_dyn = 1;
        //
        ...
        //
        if ((op != cond_exp_dyn) & (op != dis_dyn)) {  // all arguments are parameters
            // except for call_dyn and result_dyn which has n_arg = 0
            CPPAD_ASSERT_UNKNOWN(n_arg <= 2);
            for (size_t j = 0; j < n_arg; ++j) par[j] = &all_par_vec[dyn_par_arg[i_arg + j]];
        }
        //
        switch (op) {
            // ---------------------------------------------------------------
            ...
            case dis_dyn:
                CPPAD_ASSERT_UNKNOWN(n_arg == 2);
                all_par_vec[i_par] = discrete<Base>::eval(size_t(dyn_par_arg[i_arg + 0]),      // index
                                                          all_par_vec[dyn_par_arg[i_arg + 1]]  // argument
                );
#if CPPAD_DYNAMIC_TRACE
                std::cout << std::setw(10) << std::left << i_par << std::setw(10) << std::left << old_value
                          << std::setw(10) << std::left << all_par_vec[i_par] << "=" << std::setw(10) << std::right
                          << op_name_dyn(op) << "(" << std::setw(12) << std::right
                          << discrete<Base>::name(size_t(dyn_par_arg[i_arg + 0]));
                if (dyn_par_is[dyn_par_arg[i_arg + 1]]) {
                    std::cout << ", i=" << std::setw(10) << std::right << dyn_par_arg[i_arg + 1];
                } else {
                    std::cout << ", v=" << std::setw(10) << std::right << all_par_vec[dyn_par_arg[i_arg + 1]];
                }
                std::cout << ")" << std::endl;
#endif
                break;
            ...
            case cond_exp_dyn:
                ...
                all_par_vec[i_par] = CondExpOp(CompareOp(dyn_par_arg[i_arg + 0]),    // cop
                ...
                for (size_t i = 1; i < 5; ++i) {
                    if (dyn_par_is[dyn_par_arg[i_arg + i]]) {
                        std::cout << "i=" << std::setw(10) << std::right << dyn_par_arg[i_arg + i];
                    } else {
                        std::cout << "v=" << std::setw(10) << std::right << all_par_vec[dyn_par_arg[i_arg + i]];
                    }
                    if (i < 4) std::cout << ",";
                }
                std::cout << ")" << std::endl;
            ...
            case call_dyn: {
                ...
                n_arg = 5 + n + m;
                CPPAD_ASSERT_UNKNOWN(size_t(dyn_par_arg[i_arg + 4 + n + m]) == n_arg);
                //
                ...
                call_atomic_forward<Base, RecBase>(taylor_x, type_x, need_y, order_low, order_up, atom_index, atom_old,
                                                   taylor_x, taylor_y);
#if CPPAD_DYNAMIC_TRACE
                ...
                for (size_t j = 0; j < n; ++j) {
                    std::cout << "index = " << j << ", value = " << taylor_x[j] << std::endl;
                }
...
#if CPPAD_DYNAMIC_TRACE
                        std::cout << std::setw(10) << std::left << i_par << std::setw(10) << std::left << old_value
                                  << std::setw(10) << std::left << all_par_vec[i_par] << "= " << name << "_" << i
                                  << std::endl;
#endif
                    }
                }
...
#if CPPAD_DYNAMIC_TRACE
        if ((op != cond_exp_dyn) & (op != dis_dyn) & (op != call_dyn) & (op != result_dyn)) {
            std::cout << std::setw(10) << std::left << i_par << std::setw(10) << std::left << old_value << std::setw(10)
                      << std::left << all_par_vec[i_par] << "=" << std::setw(10) << std::right << op_name_dyn(op)
                      << "(";
            if (0 < n_arg) {
                if (dyn_par_is[dyn_par_arg[i_arg + 0]]) {
                    std::cout << "i=" << std::setw(10) << std::right << dyn_par_arg[i_arg + 0];
                } else {
                    std::cout << "v=" << std::setw(10) << std::right << all_par_vec[dyn_par_arg[i_arg + 0]];
                }
            }
            if (1 < n_arg) {
                if (dyn_par_is[dyn_par_arg[i_arg + 1]]) {
