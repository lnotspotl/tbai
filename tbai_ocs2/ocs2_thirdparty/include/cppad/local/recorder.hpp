...
class recorder {
   ...
   public:
    /// Default constructor
    recorder(void) : num_var_rec_(0), num_dynamic_ind_(0), num_load_op_rec_(0), par_hash_table_(CPPAD_HASH_TABLE_SIZE) {
    ...
    void set_record_compare(bool record_compare) { record_compare_ = record_compare; }

    /// Set the abort index
    void set_abort_op_index(size_t abort_op_index) { abort_op_index_ = abort_op_index; }

    /// Set number of independent dynamic parameters
    void set_num_dynamic_ind(size_t num_dynamic_ind) { num_dynamic_ind_ = num_dynamic_ind; }

    /// Get record_compare option
    bool get_record_compare(void) const { return record_compare_; }

    /// Get the abort_op_index
    size_t get_abort_op_index(void) const { return abort_op_index_; }

    /// Get number of independent dynamic parameters
    size_t get_num_dynamic_ind(void) const { return num_dynamic_ind_; }
    ...
    addr_t put_dyn_par(const Base &par, op_code_dyn op, addr_t arg0, addr_t arg1);
    addr_t put_dyn_cond_exp(const Base &par, CompareOp cop, addr_t left, addr_t right, addr_t if_true, addr_t if_false);
    ...
    void put_dyn_atomic(tape_id_t tape_id, size_t atom_index, const vector<ad_type_enum> &type_x,
                        const vector<ad_type_enum> &type_y, const VectorAD &ax, VectorAD &ay);
    ...
    void cond_exp(tape_id_t tape_id, enum CompareOp cop, AD<Base> &result, const AD<Base> &left, const AD<Base> &right,
                  const AD<Base> &if_true, const AD<Base> &if_false);
    ...
    size_t Memory(void) const {
        return op_vec_.capacity() * sizeof(opcode_t) + vecad_ind_vec_.capacity() * sizeof(size_t) +
               arg_vec_.capacity() * sizeof(addr_t) + all_par_vec_.capacity() * sizeof(Base) +
...
addr_t recorder<Base>::PutOp(OpCode op) {
    ...
    CPPAD_ASSERT_KNOWN((abort_op_index_ == 0) || (abort_op_index_ != i),
    ...
    // (if NumRes(op) > 0)
    CPPAD_ASSERT_KNOWN((size_t)std::numeric_limits<addr_t>::max() >= num_var_rec_ - 1,
...
addr_t recorder<Base>::PutVecInd(addr_t vec_ind) {
    ...
    CPPAD_ASSERT_KNOWN(size_t(std::numeric_limits<addr_t>::max()) >= i,
                       "cppad_tape_addr_type maximum value has been exceeded");
...
*/
template <class Base>
addr_t recorder<Base>::put_dyn_par(const Base &par, op_code_dyn op) {  // independent parameters come first
    CPPAD_ASSERT_UNKNOWN(op == ind_dyn || op == result_dyn || op == call_dyn);
...
*/
template <class Base>
addr_t recorder<Base>::put_dyn_par(const Base &par, op_code_dyn op, addr_t arg0) {  // independent parameters come first
...
*/
template <class Base>
addr_t recorder<Base>::put_dyn_par(const Base &par, op_code_dyn op, addr_t arg0,
                                   addr_t arg1) {  // independent parameters come first
...
*/
template <class Base>
addr_t recorder<Base>::put_dyn_cond_exp(const Base &par, CompareOp cop, addr_t left, addr_t right, addr_t if_true,
                                        addr_t if_false) {  // independent parameters come first
...
void recorder<Base>::put_dyn_arg_vec(const pod_vector<addr_t> &arg_vec) {
    for (size_t i = 0; i < arg_vec.size(); ++i) dyn_par_arg_.push_back(arg_vec[i]);
}
// ---------------------------------------------------------------------------
/*!
...
template <class Base>
template <class VectorAD>
void recorder<Base>::put_dyn_atomic(tape_id_t tape_id, size_t atom_index, const vector<ad_type_enum> &type_x,
                                    const vector<ad_type_enum> &type_y, const VectorAD &ax, VectorAD &ay) {
    ...
    for (size_t i = 0; i < m; ++i)
        if (type_y[i] == dynamic_enum) ++num_dyn;
...
addr_t recorder<Base>::put_con_par(const Base &par) {
    ...
    if (all_par_vec_.size() == 0) CPPAD_ASSERT_UNKNOWN(isnan(par));
#endif
    // ---------------------------------------------------------------------
    ...
    if ((0 < index) & (index < all_par_vec_.size())) {
        if (!dyn_par_is_[index])
            if (IdenticalEqualCon(all_par_vec_[index], par)) return static_cast<addr_t>(index);
    }
    // ---------------------------------------------------------------------
    ...
    CPPAD_ASSERT_KNOWN(static_cast<size_t>(std::numeric_limits<addr_t>::max()) >= index,
                       "cppad_tape_addr_type maximum value has been exceeded")
...
*/
inline void prototype_put_arg(void) {  // This routine should not be called
    CPPAD_ASSERT_UNKNOWN(false);
}
/*!
Put one operation argument index in the recording
...
addr_t recorder<Base>::PutTxt(const char *text) {
    ...
    size_t j;
    for (j = 0; j < n; j++) text_vec_[i + j] = text[j];
...
*/
template <class Base>
void recorder<Base>::cond_exp(tape_id_t tape_id, enum CompareOp cop, AD<Base> &result, const AD<Base> &left,
                              const AD<Base> &right, const AD<Base> &if_true,
                              const AD<Base> &if_false) {  // check for invalid tape_id
    CPPAD_ASSERT_UNKNOWN(tape_id != 0);
    ...
    if (Constant(left))
        ...
        if (left.ad_type_ != dynamic_enum) arg1 += 1;
    }
    ...
    if (Constant(right))
        ...
        CPPAD_ASSERT_KNOWN(tape_id == right.tape_id_,
                           "CondExpRel: arguments are variables or dynamics for different thread");
        if (right.ad_type_ != dynamic_enum) arg1 += 2;
    }
    ...
    if (Constant(if_true))
        ...
        CPPAD_ASSERT_KNOWN(tape_id == if_true.tape_id_,
                           "CondExpRel: arguments are variables or dynamics for different thread");
        if (if_true.ad_type_ != dynamic_enum) arg1 += 4;
    }
    ...
    if (Constant(if_false))
        ...
        CPPAD_ASSERT_KNOWN(tape_id == if_false.tape_id_,
                           "CondExpRel: arguments are variables or dynamics for different thread");
        if (if_false.ad_type_ != dynamic_enum) arg1 += 8;
    }
    if (arg1 == 0) {  // none of the arguments are variables, record cond_exp_dyn

        // put the result at the end of the parameter vector as dynamic
        // put_dyn_cond_exp(par, cop, left, right, if_true, if_false)
        result.taddr_ = put_dyn_cond_exp(result.value_, CompareOp(arg0), arg2, arg3, arg4, arg5);
