...
void recorder<Base>::put_var_atomic(tape_id_t tape_id, size_t atomic_index, const vector<ad_type_enum> &type_x,
                                    const vector<ad_type_enum> &type_y, const VectorAD &ax, VectorAD &ay)
// END_PROTOTYPE
{
    CPPAD_ASSERT_KNOWN(
        size_t(std::numeric_limits<addr_t>::max()) >= std::max(std::max(atomic_index, ax.size()), ay.size()),
        "atomic_three: cppad_tape_addr_type maximum not large enough");
    CPPAD_ASSERT_UNKNOWN((tape_id == 0) == (AD<Base>::tape_ptr() == CPPAD_NULL));
    ...
    for (size_t j = 0; j < n; j++) {
        if (type_x[j] == variable_enum) {  // information for an argument that is a variable
            ...
            if (type_x[j] == constant_enum) par = put_con_par(ax[j].value_);
            PutArg(par);
            PutOp(local::FunapOp);
        }
    }
    ...
    for (size_t i = 0; i < m; i++) {
        if (type_y[i] == variable_enum) {
            ...
            if (type_y[i] == constant_enum) par = put_con_par(ay[i].value_);
            PutArg(par);
            PutOp(local::FunrpOp);
        }
    }
