...
template <class Base>
void forward_divvv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
...
template <class Base>
void forward_divvv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(DivvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(DivvvOp) == 1);
    ...
    for (size_t ell = 0; ell < r; ell++) {
        ...
        for (size_t k = 1; k < q; k++) z[m + ell] -= z[(q - k - 1) * r + 1 + ell] * y[(k - 1) * r + 1 + ell];
        z[m + ell] /= y[0];
    }
}
...
template <class Base>
void reverse_divvv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
...
template <class Base>
void forward_divpv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
...
template <class Base>
void forward_divpv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(DivpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(DivpvOp) == 1);
...
template <class Base>
void reverse_divpv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(DivvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(DivvvOp) == 1);
...
template <class Base>
void forward_divvp_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
    ...
    for (size_t d = p; d <= q; d++) z[d] = x[d] / y;
}
/*!
...
template <class Base>
void forward_divvp_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(DivvpOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(DivvpOp) == 1);
    ...
    // to divide by zero so do not make it an error.
    size_t m = (q - 1) * r + 1;
    for (size_t ell = 0; ell < r; ell++) z[m + ell] = x[m + ell] / y;
}
...
template <class Base>
void reverse_divvp_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(DivvpOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(DivvpOp) == 1);
