...
void subgraph_sparsity(player<Base> *play, subgraph_info &sub_info, const pod_vector<size_t> &dep_taddr,
                       const BoolVector &select_domain, const BoolVector &select_range, pod_vector<size_t> &row_out,
    ...
    CPPAD_ASSERT_UNKNOWN(dep_taddr.size() == sub_info.n_dep());
    CPPAD_ASSERT_UNKNOWN(size_t(select_domain.size()) == sub_info.n_ind());
    CPPAD_ASSERT_UNKNOWN(size_t(select_range.size()) == sub_info.n_dep());
    ...
    CPPAD_ASSERT_UNKNOWN(sub_info.map_user_op().size() == play->num_op_rec());
    ...
    CPPAD_ASSERT_UNKNOWN(sub_info.in_subgraph().size() == play->num_op_rec());
    //
#ifndef NDEBUG
    ...
    for (size_t i_dep = 0; i_dep < n_dep; ++i_dep)
        if (select_range[i_dep]) {
            ...
            sub_info.get_rev(random_itr, dep_taddr, addr_t(i_dep), subgraph);
            //
