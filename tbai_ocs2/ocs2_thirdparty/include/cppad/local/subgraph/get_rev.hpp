...
*/
template <class Addr>
void subgraph_info::get_rev(const play::const_random_iterator<Addr> &random_itr, const pod_vector<size_t> &dep_taddr,
                            addr_t i_dep,
                            pod_vector<addr_t> &subgraph) {  // check sizes
    ...
    size_t sub_index = 0;
    while (sub_index < subgraph.size()) {  // this operator connected to this dependent and selected independent
        ...
        for (size_t j = 0; j < argument_variable.size(); ++j) {  // add the corresponding operators to the subgraph
