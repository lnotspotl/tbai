...
void subgraph_info::init_rev(const local::play::const_random_iterator<Addr> &random_itr,
    ...
    for (size_t j = 0; j < n_ind_; ++j) select_domain_[j] = select_domain[j];

    // process_range_
    process_range_.resize(n_dep_);
    for (size_t i = 0; i < n_dep_; ++i) process_range_[i] = false;
        ...
        switch (op) {
            case InvOp:
                ...
                {  // get user index for this independent variable
                    ...
                    if (select_domain[j]) in_subgraph_[i_op] = depend_yes;
                }
#ifndef NDEBUG
                ++count_independent;
#endif
            ...
            case AFunOp:
                ...
                if (begin_atomic_call) {
                    ...
                    for (size_t j = 0; j < argument_variable.size(); ++j) {
                        ...
                        if (in_subgraph_[j_op] == depend_yes) in_subgraph_[i_op] = depend_yes;
                    }
                }
                break;
...
void subgraph_info::init_rev(player<Base> *play, const BoolVector &select_domain) {
