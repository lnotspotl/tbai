...
class subgraph_info {
   ...
   public:
    ...
    size_t n_ind(void) const { return n_ind_; }

    /// number of dependent variables
    size_t n_dep(void) const { return n_dep_; }

    /// number of operators
    size_t n_op(void) const { return n_op_; }

    // number of variables
    size_t n_var(void) const { return n_var_; }

    /// map atomic function calls to first operator in the call
    const pod_vector<addr_t> &map_user_op(void) const { return map_user_op_; }

    /// previous select_domain argument to init_rev
    const pod_vector<bool> &select_domain(void) const { return select_domain_; }

    /// dependent variables that have been processed since previous init_rev
    const pod_vector<bool> &process_range(void) const { return process_range_; }
    ...
    */
    ...
    bool check_map_user_op(const player<Base> *play) const {
        if (map_user_op_.size() != n_op_) return false;
        bool ok = true;
        size_t i_op = 0;
        while (i_op < n_op_) {
    ...
    pod_vector<addr_t> &in_subgraph(void) { return in_subgraph_; }

    /// default constructor (all sizes are zero)
    subgraph_info(void) : n_ind_(0), n_dep_(0), n_op_(0), n_var_(0) {
    ...
    void swap(subgraph_info &info) {  // size_t objects
    ...
    */
    void resize(size_t n_ind, size_t n_dep, size_t n_op, size_t n_var) {
        CPPAD_ASSERT_UNKNOWN(n_op <= size_t(std::numeric_limits<addr_t>::max()));
        // n_ind_
        n_ind_ = n_ind;
        // n_dep_
        n_dep_ = n_dep;
    ...
    */
    ...
    void set_map_user_op(const player<Base> *play) {
        ...
        for (size_t i_op = 0; i_op < n_op_; ++i_op) {  // this operator
            OpCode op = play->GetOp(i_op);
            //
            // value of map_user_op when op is not in atomic function call)
            map_user_op_[i_op] = addr_t(i_op);
            //
            if (op == AFunOp) {  // first AFunOp in an atomic function call sequence
                //
    ...
    void init_rev(const play::const_random_iterator<Addr> &random_itr, const BoolVector &select_domain);
    template <class Addr, class Base, class BoolVector>
    void init_rev(player<Base> *play, const BoolVector &select_domain);
    // -----------------------------------------------------------------------
    // see get_rev.hpp
    template <class Addr>
    void get_rev(const play::const_random_iterator<Addr> &random_itr, const pod_vector<size_t> &dep_taddr, addr_t i_dep,
