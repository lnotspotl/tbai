...
void forward_abs_op(size_t p, size_t q, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    for (size_t j = p; j <= q; j++) z[j] = sign(x[0]) * x[j];
}
...
void forward_abs_op_dir(size_t q, size_t r, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    size_t m = (q - 1) * r + 1;
    for (size_t ell = 0; ell < r; ell++) z[m + ell] = sign(x[0]) * x[m + ell];
}
...
template <class Base>
void reverse_abs_op(size_t d, size_t i_z, size_t i_x, size_t cap_order, const Base *taylor, size_t nc_partial,
                    Base *partial) {
    size_t j;
    ...
    for (j = 0; j <= d; j++) px[j] += sign(x[0]) * pz[j];
}
