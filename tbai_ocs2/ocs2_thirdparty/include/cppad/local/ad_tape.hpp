...
namespace CppAD {
namespace local {  // BEGIN_CPPAD_LOCAL__NAMESPACE
...
class ADTape {
    ...
    // functions -----------------------------------------------------------
    // PrintFor
    friend void CppAD::PrintFor<Base>(const AD<Base> &flag, const char *before, const AD<Base> &var, const char *after);
    // CondExpOp
    friend AD<Base> CppAD::CondExpOp<Base>(enum CompareOp cop, const AD<Base> &left, const AD<Base> &right,
                                           const AD<Base> &trueCase, const AD<Base> &falseCase);
    // pow
    friend AD<Base> CppAD::pow<Base>(const AD<Base> &x, const AD<Base> &y);
    // azmul
    friend AD<Base> CppAD::azmul<Base>(const AD<Base> &x, const AD<Base> &y);
    // Parameter
    friend bool CppAD::Parameter<Base>(const AD<Base> &u);
    // Variable
    friend bool CppAD::Variable<Base>(const AD<Base> &u);
    // operators -----------------------------------------------------------
    // arithematic binary operators
    friend AD<Base> CppAD::operator+ <Base>(const AD<Base> &left, const AD<Base> &right);
    friend AD<Base> CppAD::operator- <Base>(const AD<Base> &left, const AD<Base> &right);
    friend AD<Base> CppAD::operator* <Base>(const AD<Base> &left, const AD<Base> &right);
    friend AD<Base> CppAD::operator/ <Base>(const AD<Base> &left, const AD<Base> &right);

    // comparison operators
    friend bool CppAD::operator< <Base>(const AD<Base> &left, const AD<Base> &right);
    friend bool CppAD::operator<= <Base>(const AD<Base> &left, const AD<Base> &right);
    friend bool CppAD::operator><Base>(const AD<Base> &left, const AD<Base> &right);
    friend bool CppAD::operator>= <Base>(const AD<Base> &left, const AD<Base> &right);
    friend bool CppAD::operator== <Base>(const AD<Base> &left, const AD<Base> &right);
    ...
    */
    ...
    // see CondExp.h
    void RecordCondExp(enum CompareOp cop, AD<Base> &returnValue, const AD<Base> &left, const AD<Base> &right,
                       const AD<Base> &trueCase, const AD<Base> &falseCase);

    // place a VecAD object in the tape
    size_t AddVec(size_t length, const pod_vector_maybe<Base> &data);

   public:
    // public function only used by CppAD::Independent
    template <class ADBaseVector>
    void Independent(ADBaseVector &x, size_t abort_op_index, bool record_compare, ADBaseVector &dynamic);
};
// ---------------------------------------------------------------------------
// Private functions
//
