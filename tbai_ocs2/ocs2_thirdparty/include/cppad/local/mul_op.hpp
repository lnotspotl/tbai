...
template <class Base>
void forward_mulvv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
...
template <class Base>
void forward_mulvv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(MulvvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(MulvvOp) == 1);
    ...
    for (ell = 0; ell < r; ell++) {
        m = (q - 1) * r + ell + 1;
        z[m] = x[0] * y[m] + x[m] * y[0];
        for (k = 1; k < q; k++) z[m] += x[(q - k - 1) * r + ell + 1] * y[(k - 1) * r + ell + 1];
    }
}
...
template <class Base>
void reverse_mulvv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
...
template <class Base>
void forward_mulpv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // check assumptions
    ...
    for (size_t d = p; d <= q; d++) z[d] = x * y[d];
}
/*!
...
template <class Base>
void forward_mulpv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(MulpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(MulpvOp) == 1);
    ...
    for (size_t ell = 0; ell < r; ell++) z[ell] = x * y[ell];
}
/*!
...
template <class Base>
void reverse_mulpv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(MulpvOp) == 2);
    CPPAD_ASSERT_UNKNOWN(NumRes(MulpvOp) == 1);
