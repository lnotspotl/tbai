...
void color_symmetric_cppad(const SetVector &pattern, CppAD::vector<size_t> &row, CppAD::vector<size_t> &col,
    ...
    for (size_t o1 = 0; o1 < m; o1++) row2order[order2row[o1]] = o1;
    ...
    for (size_t o1 = 0; o1 < m; o1++)  // for each row that appears (in order)
        if (color[order2row[o1]] < m) {
            ...
            for (size_t c2 = 0; c2 <= c1; c2++) forbidden[c2] = false;

            // -----------------------------------------------------
            // Forbid grouping with rows that would destroy results that are
            // needed for this row.
            itr1 = pair_needed[i1].begin();
            while (itr1 != pair_needed[i1].end()) {  // entry (i1, j1) is needed for this row
                size_t j1 = *itr1;
            ...
            for (size_t o2 = 0; o2 < o1; o2++) {
                ...
                while (itr2 != pair_needed[i2].end()) {
                    ...
                    if (pattern.is_element(i1, j2)) forbidden[c2] = true;
                    itr2++;
                }
            }
            ...
            while (itr1 != pair_needed[i1].end()) {
                ...
                if (row2order[j1] > o1) {
                    ...
                    if (itr2 != pair_needed[j1].end()) {
                        ...
                        if (pair_needed[j1].empty()) color[j1] = m;
                    }
                }
                itr1++;
            }
...
*/
template <class SetVector>
void color_symmetric_colpack(const SetVector &pattern, CppAD::vector<size_t> &row, CppAD::vector<size_t> &col,
                             CppAD::vector<size_t> &color) {
