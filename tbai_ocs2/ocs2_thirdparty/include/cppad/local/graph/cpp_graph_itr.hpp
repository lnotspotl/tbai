...
class cpp_graph_itr {
    ...
    */
    {  // initialize output values
        ...
        for (size_t i = 0; i < n_arg; i++) arg_node_[i] = (*operator_arg_)[first_node_ + i];
   ...
   public:
    ...
    $end
    */
    cpp_graph_itr(void) : operator_vec_(CPPAD_NULL), operator_arg_(CPPAD_NULL) {}
    // BEGIN_CTOR
    cpp_graph_itr(const vector<graph_op_enum> &operator_vec, const vector<size_t> &operator_arg, size_t op_index)
        // END_CTOR
        : operator_vec_(&operator_vec), operator_arg_(&operator_arg), op_index_(op_index) {  // end constructor
        if (op_index == operator_vec.size()) return;
        //
        // begin constructor
        CPPAD_ASSERT_KNOWN(op_index == 0, "cpp_graph_itr: constructor op_index not 0 or operator_vec.size()");
    ...
    $srccode%hpp% */
    // itr == other
    bool operator==(const cpp_graph_itr &other) const { return op_index_ == other.op_index_; }
    // itr != other
    bool operator!=(const cpp_graph_itr &other) const { return op_index_ != other.op_index_; }
    // *itr
    value_type operator*(void) {
        CPPAD_ASSERT_KNOWN(operator_vec_ != CPPAD_NULL, "cpp_graph_itr: attempt to dereference default iterator");
