...
template <class Type>
class const_cppad_vector_itr;

// ==========================================================================
template <class Type>
class CPPAD_VECTOR_ITR {
// ==========================================================================
/*
-----------------------------------------------------------------------------
...
$srccode%hpp% */
   ...
   private:
    ...
    void check_cop(const CPPAD_VECTOR_ITR &other) const CPPAD_NDEBUG_NOEXCEPT {
        CPPAD_ASSERT_KNOWN(data_ == other.data_, "CppAD vector iterator: comparing indices from different vectors");
    }

   public:
    CPPAD_VECTOR_ITR(void) CPPAD_NOEXCEPT : data_(CPPAD_NULL), length_(CPPAD_NULL), index_(0) {}
#if CPPAD_CONST
    const_cppad_vector_itr(const Type *const *data, const size_t *length, difference_type index) CPPAD_NOEXCEPT
        : data_(data),
          length_(length),
          index_(difference_type(index)) {}
    // ctor a const_iterator from an iterator
    const_cppad_vector_itr(const cppad_vector_itr<Type> &non_const_other) CPPAD_NOEXCEPT {
        data_ = non_const_other.data_;
        length_ = non_const_other.length_;
        index_ = non_const_other.index_;
    }
#else
    cppad_vector_itr(Type *const *data, const size_t *length, difference_type index) CPPAD_NOEXCEPT
        : data_(data),
          length_(length),
          index_(difference_type(index)) {}
#endif
    ...
    CPPAD_VECTOR_ITR(const CPPAD_VECTOR_ITR &other) CPPAD_NOEXCEPT { *this = other; }
    /* %$$
    $end
    -------------------------------------------------------------------------------
   ...
   public:
    ...
    $srccode%hpp% */
   public:
    CPPAD_VECTOR_ITR operator[](difference_type n) { return *(*this + n); }
    ...
    difference_type operator-(const CPPAD_VECTOR_ITR &other) const CPPAD_NOEXCEPT { return index_ - other.index_; }
...
// BEGIN_BINARY_OP
template <class Type>
CPPAD_VECTOR_ITR<Type> operator+(typename CPPAD_VECTOR_ITR<Type>::difference_type n,
                                 const CPPAD_VECTOR_ITR<Type> &other) CPPAD_NOEXCEPT {
    return CPPAD_VECTOR_ITR<Type>(other.data_, other.length_, n + other.index_);
}
template <class Type>
CPPAD_VECTOR_ITR<Type> operator-(typename CPPAD_VECTOR_ITR<Type>::difference_type n,
                                 const CPPAD_VECTOR_ITR<Type> &other) CPPAD_NOEXCEPT {
