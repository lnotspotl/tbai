...
class pack_setvec {
   ...
   public:
    pack_setvec(void)
        : n_bit_(std::numeric_limits<Pack>::digits), zero_(0), one_(1), n_set_(0), end_(0), n_pack_(0), data_(0) {}
    /* %$$
    $end
    -------------------------------------------------------------------------------
   ...
   public:
    ...
    $srccode%hpp% */
   public:
    pack_setvec(const pack_setvec &v) : n_bit_(std::numeric_limits<Pack>::digits), zero_(0), one_(1) {
    ...
    */
    {
        ...
        for (size_t bit = 0; bit < n_bit_; ++bit) {
            ...
            size_t k = n_pack_;
            while (--k) {
                if (data_[i * n_pack_ + k] & mask) ++count;
            }
            mask = mask << 1;
        }
        return count;
    ...
    */
    {
        CPPAD_ASSERT_UNKNOWN(i < n_set_);
        CPPAD_ASSERT_UNKNOWN(element < end_);
        if (end_ == 1) return data_[i] != zero_;
        //
        size_t j = element / n_bit_;
        size_t k = element - j * n_bit_;
        Pack mask = one_ << k;
    ...
    */
    ...
    $srccode%hpp% */
   public:
    void assignment(size_t this_target, size_t other_value, const pack_setvec &other)
    /* %$$
    $end
    */
    {
    ...
    $srccode%hpp% */
   public:
    void binary_union(size_t this_target, size_t this_left, size_t other_right, const pack_setvec &other)
    /* %$$
    $end
    */
    {
    ...
    $srccode%hpp% */
   public:
    void binary_intersection(size_t this_target, size_t this_left, size_t other_right, const pack_setvec &other)
    /* %$$
    $end
    */
    {
   ...
   public:
    pack_setvec_const_iterator(const pack_setvec &pack, size_t set_index)
        ...
        */
        ...
        //
        next_element_ = 0;
        if (data_[data_index_] & one_) return;
        //
    ...
    */
    {
        ...
        if (bit == 0) ++data_index_;
        //
        // initialize mask
        size_t mask = one_ << bit;
        //
        while (next_element_ < end_) {  // check if this element is in the set
            if (data_[data_index_] & mask) return *this;
            //
            ...
            if (bit == n_bit_) {  // get next packed value
                bit = 0;
                mask = one_;
                ++data_index_;
            }
...
void sparsity_user2internal(pack_setvec &internal, const SetVector &user, size_t n_set, size_t end, bool transpose,
...
*/
{
    ...
    if (transpose) {  // transposed pattern case
        for (size_t j = 0; j < end; j++) {
            for (size_t i = 0; i < n_set; i++) {  // no advantage to using post_element for pack_setvec
        ...
        for (size_t i = 0; i < n_set; i++) {
            for (size_t j = 0; j < end; j++) {  // no advantage to using post_element for pack_setvec
