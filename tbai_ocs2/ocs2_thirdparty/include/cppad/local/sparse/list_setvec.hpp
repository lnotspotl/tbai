...
*/
    ...
    {  // number of sets
        ...
        for (size_t i = 0; i < n_set; i++) temporary_[i] = reference_count(i);
        // -----------------------------------------------------------
   ...
   public:
    ...
    $srccode%hpp% */
   public:
    list_setvec(void) : end_(0), number_not_used_(0), data_not_used_(0) {}
    /* %$$
    $end
    -------------------------------------------------------------------------------
    ...
    $srccode%hpp% */
   public:
    ~list_setvec(void) { check_data_structure(); }
    /* %$$
    $end
    -------------------------------------------------------------------------------
    ...
    $srccode%hpp% */
   public:
    list_setvec(const list_setvec &v) { CPPAD_ASSERT_UNKNOWN(false); }
    /* %$$
    $end
    -------------------------------------------------------------------------------
    ...
    */
    {  // post
        ...
        size_t i_value = end_;
        if (i_next > 0) {  // skip reference count
        ...
        for (size_t j = 0; j < number_post; ++j) {
            ...
            while (i_value < post_value) {  // i_value is in union
    ...
    */
    ...
    $srccode%hpp% */
   public:
    void assignment(size_t this_target, size_t other_source, const list_setvec &other)
    /* %$$
    $end
    */
    {
        ...
        if (this == &other) {
            ...
            this_start = 0;
        } else {  // make a copy of the other list in this list_setvec
    ...
    $srccode%hpp% */
   public:
    void binary_union(size_t this_target, size_t this_left, size_t other_right, const list_setvec &other)
    /* %$$
    $end
    */
    {
        ...
        while ((value_left < end_) & (value_right < end_)) {
            if (value_left == value_right) {  // value is in both sets
            ...
            } else if (value_left < value_right) {  // need a value from left that is not in right
    ...
    $srccode%hpp% */
   public:
    void binary_intersection(size_t this_target, size_t this_left, size_t other_right, const list_setvec &other)
    /* %$$
    $end
    */
    {
        ...
        while ((value_left < end_) & (value_right < end_)) {
            if (value_left == value_right) {  // value is in both sets
                ...
                next_right = other.data_[next_right].next;
                value_right = other.data_[next_right].value;
            } else if (value_left < value_right) {  // there is a value in left that is not in right
                left_is_subset = false;
                //
                // advance left to its next element
                next_left = data_[next_left].next;
...
void sparsity_user2internal(list_setvec &internal, const SetVector &user, size_t n_set, size_t end, bool transpose,
...
**/
{
    ...
    if (transpose) {  // transposed pattern case
        ...
        for (size_t i = 0; i < n_set; ++i) internal.process_post(i);
    } else {
        for (size_t i = 0; i < n_set; i++) {
            itr = user[i].begin();
...
namespace local {
template <>
inline bool is_pod<sparse::list_setvec::pair_size_t>(void) {
    return true;
}
}  // namespace local
}  // namespace CppAD
