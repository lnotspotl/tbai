...
class svec_setvec {
    ...
    */
    ...
    void check_data_structure(void) {  // number of sets
        ...
        CPPAD_ASSERT_UNKNOWN(data_used + data_not_used_ == data_.size());
        return;
    }
#endif
    // -----------------------------------------------------------------
    ...
    */
    size_t is_subset(size_t one_this, size_t two_other, const svec_setvec &other) const {
        ...
        while ((one_subset | two_subset) & (value_union < end_)) {  //
            if (value_one > value_union)
                two_subset = false;
            else {  // value_one <= value_two and value_one < end_
                value_one = data_[++index_one];
            }
            //
            if (value_two > value_union)
                one_subset = false;
            else {  // value_two <= value_one and value_two < end_
    ...
    */
    void collect_garbage(void) {
        if (data_not_used_ < data_.size() / 2 + 100) return;
        ...
        for (size_t i = 0; i < n_set; i++) {
            ...
            if (start == 0)
                start_tmp[i] = 0;
            else {                        // check if this set has already been copied
                if (data_[start] == 0) {  // starting address in data_tmp has been stored here
                    ...
                    //
                    for (size_t j = 2; data_[start + j] != end_; ++j) data_tmp.push_back(data_[start + j]);
    ...
    */
    svec_setvec(void) : end_(0), data_not_used_(0), data_(0), start_(0), post_(0) {}
    // -----------------------------------------------------------------
    /// Destructor
    ~svec_setvec(void) { check_data_structure(); }
    // -----------------------------------------------------------------
    /*!
    ...
    */
    svec_setvec(const svec_setvec &v) {  // Error: Probably a svec_setvec argument has been passed by value
    ...
    */
    void swap(svec_setvec &other) {  // size_t objects
    ...
    */
    void post_element(size_t i, size_t element) {
        ...
        size_t post = post_[i];
        if (post == 0) {  // minimum capacity for an post vector
            ...
            if (length == capacity) {
                ...
                //
                for (size_t j = 0; j < length; j++) data_[post_new + 2 + j] = data_[post + 2 + j];
    ...
    */
    void process_post(size_t i) {  // start
        size_t start = start_[i];
        // post
        size_t post = post_[i];
        //
        ...
        while (subset & (value_post != end_)) {
            while (value_set < value_post) value_set = data_[++current_set];
        ...
        //
        if (subset) {  // drop the post_ elements
            post_[i] = 0;
            //
        ...
        while ((value_set < end_) | (current_post != last_post)) {
            if (value_set == value_post) {  // advance left so left and right are no longer equal
                ++current_set;
                value_set = data_[current_set];
                CPPAD_ASSERT_UNKNOWN(value_post < value_set);
            }
            //
            if (value_set < value_post) {  // add value_set
    ...
    */
    void add_element(size_t i, size_t element) {
        ...
        if (is_element(i, element)) return;
    ...
    */
    bool is_element(size_t i, size_t element) const {
        ...
        if (length < 10) {
            bool result = false;
            while (last > first) result |= *(--last) == element;
            return result;
        }
        //
    ...
    */
    void assignment(size_t this_target, size_t other_source, const svec_setvec &other) {
        ...
        if (post > 0) {  // do not need to worry about target being same as source
            size_t capacity = data_[post + 1];
            number_lost += capacity + 2;
            post_[this_target] = 0;
        }
        ...
        if (this == &other) {
            ...
            if (other_start != 0) {  // increment reference count
                data_[other_start]++;
            }
        } else if (other_start == 0) {  // the target list is empty
            start_[this_target] = 0;
        } else {  // make a copy of the other list in this svec_setvec
            ...
            for (size_t j = 0; j < length; ++j) data_.push_back(other.data_[other_start + 2 + j]);
            data_.push_back(end_);
        }
    ...
    svec_setvec object).
    */
    void binary_union(size_t this_target, size_t this_left, size_t other_right, const svec_setvec &other) {
        ...
        // drop any posting for the target set
        size_t post = post_[this_target];
        if (post > 0) {  // do not need to worry about target being same as left or right
            size_t capacity = data_[post + 1];
            number_lost += capacity + 2;
            post_[this_target] = 0;
        }
    ...
    svec_setvec object).
    */
    void binary_intersection(size_t this_target, size_t this_left, size_t other_right, const svec_setvec &other) {
        CPPAD_ASSERT_UNKNOWN(post_[this_left] == 0);
        CPPAD_ASSERT_UNKNOWN(other.post_[other_right] == 0);
        //
        CPPAD_ASSERT_UNKNOWN(this_target < start_.size());
    ...
    */
    size_t memory(void) const { return data_.capacity() * sizeof(size_t); }
    /*!
...
class svec_setvec_const_iterator {
   ...
   public:
    ...
    svec_setvec_const_iterator(const svec_setvec &vec_set, size_t i) : data_(vec_set.data_), end_(vec_set.end_) {
        CPPAD_ASSERT_UNKNOWN(vec_set.post_[i] == 0);
        //
        size_t start = vec_set.start_[i];
        if (start == 0) {
            data_index_ = 0;
        } else {  // data index of the first element in the set
            data_index_ = start + 2;
    ...
    svec_setvec_const_iterator &operator++(void) {
        if (data_index_ != 0) {
            ++data_index_;
            if (data_[data_index_] == end_) data_index_ = 0;
        }
        return *this;
    }
...
void sparsity_user2internal(svec_setvec &internal, const SetVector &user, size_t n_set, size_t end, bool transpose,
