...
template <class Base>
void forward_powvv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    ...
    if (p == 0) {  // Taylor coefficients corresponding to arguments and result
    ...
    if (p <= q) forward_exp_op(p, q, i_z + 2, i_z + 1, cap_order, taylor);
}
/*!
...
template <class Base>
void forward_powvv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // convert from final result to first result
    i_z -= 2;  // 2 = NumRes(PowvvOp) - 1
...
template <class Base>
void reverse_powvv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
    ...
    // z_2 = exp(z_1)
    reverse_exp_op(d, i_z + 2, i_z + 1, cap_order, taylor, nc_partial, partial);

    // z_1 = z_0 * y
    addr_t adr[2];
    adr[0] = addr_t(i_z);
    adr[1] = arg[1];
    reverse_mulvv_op(d, i_z + 1, adr, parameter, cap_order, taylor, nc_partial, partial);

    // z_0 = log(x)
    reverse_log_op(d, i_z, size_t(arg[0]), cap_order, taylor, nc_partial, partial);
}
...
template <class Base>
void forward_powpv_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // convert from final result to first result
    ...
    CPPAD_ASSERT_KNOWN(size_t(std::numeric_limits<addr_t>::max()) >= i_z * cap_order,
...
template <class Base>
void forward_powpv_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // convert from final result to first result
    i_z -= 2;  // 2 = NumRes(PowpvOp) - 1;
    ...
    // z_0 = log(x)
    size_t m = (q - 1) * r + 1;
    for (size_t ell = 0; ell < r; ell++) z_0[m + ell] = Base(0.0);

    // 2DO: remove requirement i_z * num_taylor_per_var <= max addr_t value
    CPPAD_ASSERT_KNOWN(size_t(std::numeric_limits<addr_t>::max()) >= i_z * num_taylor_per_var,
                       "cppad_tape_addr_type maximum value has been exceeded\n"
                       "This is due to a kludge in the pow operation and should be fixed.");
...
template <class Base>
void reverse_powpv_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // convert from final result to first result
    i_z -= 2;  // NumRes(PowpvOp) - 1;
    ...
    reverse_mulpv_op(d, i_z + 1, adr, taylor, cap_order, taylor, nc_partial, partial);
...
template <class Base>
void forward_powvp_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      Base *taylor) {
    // convert from final result to first result
...
template <class Base>
void forward_powvp_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                          Base *taylor) {
    // convert from final result to first result
    i_z -= 2;  // 2 = NumRes(PowvpOp) - 1
...
template <class Base>
void reverse_powvp_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                      const Base *taylor, size_t nc_partial, Base *partial) {
    // convert from final result to first result
    i_z -= 2;  // NumRes(PowvpOp) - 1;
    ...
    adr[0] = arg[1];
    adr[1] = addr_t(i_z);
    reverse_mulpv_op(d, i_z + 1, adr, parameter, cap_order, taylor, nc_partial, partial);
