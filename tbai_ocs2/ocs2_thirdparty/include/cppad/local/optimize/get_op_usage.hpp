...
inline bool op_add_or_sub(OpCode op  ///< operator we are checking
) {
    bool result;
    switch (op) {
        case AddpvOp:
...
*/
template <class Base>
void op_inc_arg_usage(const player<Base> *play, bool sum_result, size_t i_result, size_t i_arg,
                      pod_vector<usage_t> &op_usage,
                      sparse_list &cexp_set) {  // value of argument input on input to this routine
    ...
    //
    // cexp_set
    if (cexp_set.n_set() == 0) return;
    //
    if (arg_usage == no_usage) {  // set[i_arg] = set[i_result]
        cexp_set.assignment(i_arg, i_result, cexp_set);
    } else {  // set[i_arg] = set[i_arg] intersect set[i_result]
...
void get_op_usage(bool conditional_skip, bool compare_op, bool print_for_op, const player<Base> *play,
                  const play::const_random_iterator<Addr> &random_itr, const pod_vector<size_t> &dep_taddr,
                  pod_vector<addr_t> &cexp2op, sparse_list &cexp_set, pod_vector<bool> &vecad_used,
                  pod_vector<usage_t> &op_usage) {
    ...
    CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= num_op);
    // -----------------------------------------------------------------------
    ...
    for (size_t i = 0; i < num_vecad; i++) vecad_used[i] = false;
    //
    vector<size_t> arg2vecad(num_vecad_ind);
    for (size_t i = 0; i < num_vecad_ind; i++) arg2vecad[i] = num_vecad;  // invalid value
    ...
    if (conditional_skip) {
        for (i_op = 0; i_op < num_op; ++i_op) {
            if (random_itr.get_op(i_op) == CExpOp) {  // count the number of conditional expressions.
                ++num_cexp_op;
            }
        }
    }
        ...
        switch (op) {
            ...
            case ZmulvpOp:
                ...
                if (use_result != usage_t(no_usage)) {
                    ...
                    op_inc_arg_usage(play, sum_op, i_op, j_op, op_usage, cexp_set);
                }
                break;  // --------------------------------------------
            ...
            case CExpOp:
                ...
                if (use_result != usage_t(no_usage)) {
                    ...
                    if (arg[1] & 4) {
                        ...
                        op_inc_arg_usage(play, sum_op, i_op, j_op, op_usage, cexp_set);
                        if (can_skip) {  // j_op corresponds to the value used when the
            ...
            case PriOp:
                ...
                if (print_for_op) {
                    op_usage[i_op] = usage_t(yes_usage);
                    if (arg[0] & 1) {  // arg[1] is a variable
                        size_t j_op = random_itr.var2op(size_t(arg[1]));
                        op_inc_arg_usage(play, sum_op, i_op, j_op, op_usage, cexp_set);
                    }
                    if (arg[0] & 2) {  // arg[3] is a variable
                        size_t j_op = random_itr.var2op(size_t(arg[3]));
                        op_inc_arg_usage(play, sum_op, i_op, j_op, op_usage, cexp_set);
                    }
                }
            ...
            case AFunOp:
                ...
                if (atom_state == end_atom) {  // reverse_user using random_itr instead of play
                    ...
                    CPPAD_ASSERT_UNKNOWN(op_usage[last_atom_i_op] == usage_t(no_usage));
#ifndef NDEBUG
                    if (cexp_set.n_set() > 0) {
                    ...
                    for (size_t i = 0; i < atom_m; i++) depend_y[i] = false;
                } else {  // reverse_user using random_itr instead of play
                    ...
                    atom_state = end_atom;
                    // -------------------------------------------------------
                    CPPAD_ASSERT_UNKNOWN(i_op + atom_n + atom_m + 1 == last_atom_i_op);
                    if (op_usage[last_atom_i_op] != usage_t(no_usage)) {  // call atomic function for this operation
                        sweep::call_atomic_rev_depend<Base, Base>(atom_index, atom_old, atom_x, type_x, depend_x,
                                                                  depend_y);
                        for (size_t j = 0; j < atom_n; j++)
                            if (depend_x[j]) {  // The parameter or variable correspnding to the j-th
                                // argument gets used
                                ...
                                if (type_x[j] == variable_enum) {
                                    ...
                                    if (depend_x[j]) {
                                        ...
                                        op_inc_arg_usage(play, sum_op, last_atom_i_op, j_op, op_usage, cexp_set);
                                    }
                                }
                            }
                    }
                    // copy set infomation from last to first
                    if (cexp_set.n_set() > 0) cexp_set.assignment(i_op, last_atom_i_op, cexp_set);
            ...
            case FunrvOp:
                ...
                if (use_result) {
                    ...
                    op_inc_arg_usage(play, sum_op, i_op, last_atom_i_op, op_usage, cexp_set);
                }
                break;  // --------------------------------------------------------
