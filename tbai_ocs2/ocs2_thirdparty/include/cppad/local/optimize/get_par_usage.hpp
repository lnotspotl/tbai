...
void get_par_usage(const player<Base> *play, const play::const_random_iterator<Addr> &random_itr,
                   const pod_vector<usage_t> &op_usage, pod_vector<bool> &vecad_used, pod_vector<bool> &par_usage) {
    ...
    for (size_t i_vec = 0; i_vec < num_vecad_vec; ++i_vec) {  // length of this vector (note length is not a parameter)
        size_t length = play->GetVecInd(start_this_vector);
        //
        if (vecad_used[i_vec]) {                    // this vector gets used
            for (size_t k = 1; k <= length; ++k) {  // index of parameter used by this VecAD vector
    ...
    for (size_t i_op = 0; i_op < num_op; ++i_op) {
        ...
        if (!skip) switch (op) {
                ...
                case SubpvOp:
                    ...
                    else {                                      // determine if this parameter will be absorbed by csum
                        if (!(op_usage[i_op] == csum_usage)) {  // determine operator corresponding to variable
                            size_t j_op = random_itr.var2op(size_t(arg[1]));
                            CPPAD_ASSERT_UNKNOWN(op_usage[j_op] != no_usage);
                            if (op_usage[j_op] != csum_usage) par_usage[arg[0]] = true;
                        }
                    }
                    break;
                ...
                case SubvpOp:
                    if (dyn_par_is[arg[1]])
                        ...
                        if (!(op_usage[i_op] == csum_usage)) {  // determine operator corresponding to variable
                            ...
                            CPPAD_ASSERT_UNKNOWN(op_usage[j_op] != no_usage);
                            if (op_usage[j_op] != csum_usage) par_usage[arg[1]] = true;
                        }
                    }
                    break;
                ...
                case CExpOp:
                    ...
                    if ((arg[1] & 1) == 0) par_usage[arg[2]] = true;
                    if ((arg[1] & 2) == 0) par_usage[arg[3]] = true;
                    if ((arg[1] & 4) == 0) par_usage[arg[4]] = true;
                    if ((arg[1] & 8) == 0) par_usage[arg[5]] = true;
                    break;
                ...
                // print function
                case PriOp:
                    if ((arg[0] & 1) == 0) par_usage[arg[1]] = true;
                    if ((arg[0] & 2) == 0) par_usage[arg[3]] = true;
                ...
                case AFunOp:
                    if (atom_state == start_atom) {
                        ...
                        sweep::call_atomic_rev_depend<Base, Base>(atom_index, atom_old, parameter_x, type_x, depend_x,
                                                                  depend_y);
                        for (size_t j = 0; j < atom_n; j++)
                            if (depend_x[j] &&
                                type_x[j] != variable_enum) {  // This user argument is a parameter that is needed
                ...
                case FunavOp:
                    ...
                    ++atom_j;
                    if (atom_j == atom_n) atom_state = ret_atom;
                    break;
                ...
                case FunrpOp:
                    ...
                    ++atom_i;
                    if (atom_i == atom_m) atom_state = end_atom;
                    break;
    ...
    while (i_dyn) {                     // next dynamic parameter in reverse order
        --i_dyn;
        ...
        if (op == call_dyn) {  // number of arguments for this operator
            ...
            type_x.resize(n);
            for (size_t j = 0; j < n; ++j) {  // parameter index zero is used for variable
            ...
            // depend_y
            depend_y.resize(m);
            for (size_t i = 0; i < m; ++i) {  // a constant prameter cannot depend on a dynamic parameter
            ...
            sweep::call_atomic_rev_depend<Base, Base>(atom_index, atom_old, parameter_x, type_x, depend_x, depend_y);
            //
        ...
        } else {  // corresponding parameter index
            ...
            if (par_usage[i_par]) {        // neeed dynamic parameters that are used to generate this one
                if (op == cond_exp_dyn) {  // special case
                    CPPAD_ASSERT_UNKNOWN(n_arg == 5);
                    for (size_t i = 1; i < 5; ++i) par_usage[dyn_par_arg[i_arg + i]] = true;
                } else {
                    for (size_t i = 0; i < n_arg; ++i) par_usage[dyn_par_arg[i_arg + i]] = true;
                }
            }
        }
    }
