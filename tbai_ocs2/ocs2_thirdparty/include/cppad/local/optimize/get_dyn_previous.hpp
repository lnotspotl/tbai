...
*/
inline void dyn_arg_match(size_t i_dyn, const pod_vector<addr_t> &dyn_ind2par_ind, const pod_vector<bool> &dyn_par_is,
                          const pod_vector<addr_t> &dyn_arg_offset, const pod_vector<addr_t> &dyn_par_arg,
                          const pod_vector<addr_t> &par_ind2dyn_ind, const pod_vector<addr_t> &dyn_previous,
    ...
    for (addr_t j = 0; j < n_arg; ++j) {  // parameter index for this argument
...
void get_dyn_previous(const player<Base> *play, const play::const_random_iterator<Addr> &random_itr,
                      pod_vector<bool> &par_usage, pod_vector<addr_t> &dyn_previous) {
    ...
    for (size_t i_dyn = 0; i_dyn < num_dynamic_ind; ++i_dyn) {  // parameter index
    ...
    for (size_t i_dyn = num_dynamic_ind; i_dyn < num_dynamic_par;
         ++i_dyn) {  // Initialize previous for this dynamic parameter. This is only
        ...
        if (par_usage[i_par]) switch (op) {
                ...
                case tanh_dyn:
                    ...
                    CPPAD_ASSERT_UNKNOWN(dyn_par_is[i_par]);
                    {
                        ...
                        dyn_arg_match(i_dyn, dyn_ind2par_ind, dyn_par_is, dyn_arg_offset, dyn_par_arg, par_ind2dyn_ind,
                                      dyn_previous, arg_match);
                        opcode_t op_t = opcode_t(op);
                        code = optimize_hash_code(op_t, num_arg, arg_match.data());
                        //
                        ...
                        if (match) {
                            ...
                            if (count == 10) {  // restart list for this hash code
                ...
                case zmul_dyn:
                    ...
                    if ((!match) & ((op == add_dyn) | (op == mul_dyn))) {
                        ...
                        opcode_t op_t = opcode_t(op);
                        size_t code_swp = optimize_hash_code(op_t, num_arg, arg_match.data());
                        //
                        // iterator for the set with this hash code
