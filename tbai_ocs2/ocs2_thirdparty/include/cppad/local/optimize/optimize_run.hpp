...
void optimize_run(const std::string &options, size_t n, pod_vector<size_t> &dep_taddr, player<Base> *play,
                  recorder<Base> *rec) {  // check that recorder is empty
    CPPAD_ASSERT_UNKNOWN(rec->num_op_rec() == 0);
    //
    // get a random iterator for this player
    play->template setup_random<Addr>();
    local::play::const_random_iterator<Addr> random_itr = play->template get_random<Addr>();
    ...
    while (index < options.size()) {
        while (index < options.size() && options[index] == ' ') ++index;
        std::string option;
        while (index < options.size() && options[index] != ' ') option += options[index++];
        if (option != "") {
    ...
    CPPAD_ASSERT_UNKNOWN(num_op < size_t(std::numeric_limits<addr_t>::max()));
    ...
    get_op_usage(conditional_skip, compare_op, print_for_op, play, random_itr, dep_taddr, cexp2op, cexp_set, vecad_used,
                 op_usage);
    pod_vector<addr_t> op_previous;
    get_op_previous(play, random_itr, cexp_set, op_previous, op_usage);
    ...
    //
    if (cexp2op.size() > 0)
        get_cexp_info(play, random_itr, op_previous, op_usage, cexp2op, cexp_set, cexp_info, skip_op_true,
                      skip_op_false);
    ...
    get_par_usage(play, random_itr, op_usage, vecad_used, par_usage);
    pod_vector<addr_t> dyn_previous;
    get_dyn_previous(play, random_itr, par_usage, dyn_previous);
    // -----------------------------------------------------------------------
    ...
    if (num_cexp > 0) {
        ...
        for (size_t i = 0; i < num_cexp; i++) keys[i] = size_t(cexp_info[i].max_left_right);
    ...
    for (size_t i = 0; i < num_cexp; i++) cskip_new[i].i_arg = 0;
    // =======================================================================
    ...
    for (size_t i_par = 0; i_par < num_par; ++i_par) new_par[i_par] = addr_t_max;  // initialize as not used
    //
    // start new recording
    CPPAD_ASSERT_UNKNOWN(rec->num_op_rec() == 0);
    ...
    for (size_t i_par = num_dynamic_ind + 1; i_par < num_par; ++i_par)
        if (!dyn_par_is[i_par]) {
            ...
            if (par_usage[i_par]) {  // value of this parameter
    ...
    for (size_t i_par = num_dynamic_ind + 1; i_par < num_par; ++i_par)
        if (dyn_par_is[i_par]) {  // operator for this dynamic parameter
            ...
            if (op == call_dyn) {
                ...
                for (size_t i = 0; i < atom_m; ++i) {
                    size_t j_par = size_t(dyn_par_arg[i_arg + 4 + atom_n + i]);
                    if (dyn_par_is[j_par]) call_used |= par_usage[j_par];
                }
#endif
                if (call_used) {
                    arg_vec.resize(0);
                    ...
                    for (size_t i = 0; i < atom_m; ++i) {
                        ...
                        } else {  // this result is a constant parameter
                            if (new_par[res_i] != addr_t_max)
                                arg_vec.push_back(new_par[res_i]);
                            else {                     // this constant parameter is not used
                ...
                if (j_dyn != num_dynamic_par) {
                    ...
                    Base par = play->GetPar(i_par);
                    //
                    if (op == cond_exp_dyn) {  // cond_exp_dyn
                        CPPAD_ASSERT_UNKNOWN(num_dynamic_ind <= i_par);
                        CPPAD_ASSERT_UNKNOWN(n_arg = 5);
                        new_par[i_par] = rec->put_dyn_cond_exp(par,                                // par
                    ...
                    } else if (op == dis_dyn) {  // dis_dyn
                        CPPAD_ASSERT_UNKNOWN(n_arg = 2);
                        new_par[i_par] = rec->put_dyn_par(par,                             // par
                                                          op,                              // op
                                                          dyn_par_arg[i_arg + 0],          // index
                                                          new_par[dyn_par_arg[i_arg + 1]]  // parameter
                        );
                    } else if (n_arg == 1) {  // cases with one argument
                        CPPAD_ASSERT_UNKNOWN(num_dynamic_ind <= i_par);
                        new_par[i_par] = rec->put_dyn_par(par, op, new_par[dyn_par_arg[i_arg + 0]]);
                    } else if (n_arg == 2) {  // cases with two arguments
                        CPPAD_ASSERT_UNKNOWN(num_dynamic_ind <= i_par);
                        new_par[i_par] =
                            rec->put_dyn_par(par, op, new_par[dyn_par_arg[i_arg + 0]], new_par[dyn_par_arg[i_arg + 1]]);
                    } else {  // independent dynamic parmaeter case
    ...
    CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= num_vecad_ind);
    pod_vector<addr_t> new_vecad_ind(num_vecad_ind);
    for (size_t i = 0; i < num_vecad_ind; i++) new_vecad_ind[i] = addr_t(num_vecad_ind);  // invalid index
    {
        size_t j = 0;                                 // index into the old set of indices
        for (size_t i = 0; i < num_vecad_vec; i++) {  // length of this VecAD
            size_t length = play->GetVecInd(j);
            if (vecad_used[i]) {  // Put this VecAD vector in new recording
                CPPAD_ASSERT_UNKNOWN(length < num_vecad_ind);
                new_vecad_ind[j] = rec->PutVecInd(addr_t(length));
                for (size_t k = 1; k <= length; k++)
                    new_vecad_ind[j + k] = rec->PutVecInd(new_par[play->GetVecInd(j + k)]);
            }
            // start of next VecAD
            j += length + 1;
        }
    ...
    //
    i_var = 0;
    for (i_op = 0; i_op < num_op; ++i_op) {  // if non-zero, use previous result in place of this operator.
        ...
        new_var[i_op] = 0;
        if (op_usage[i_op] == usage_t(yes_usage)) new_var[i_op] = new_var[previous];
        //
        ...
        //
        CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= rec->num_op_rec());
        //
        ...
        if (skip) {
            ...
            switch (op) {  // op_usage[i_op] == usage_t(yes_usage)
                ...
                case TanhOp:
                    if (previous == 0) {  //
                        ...
                        CPPAD_ASSERT_UNKNOWN(new_arg[0] < new_var[random_itr.var2op(i_var)]);
                        if (op == ErfOp) {
                            ...
                            rec->PutArg(rec->put_con_par(Base(1.0 / std::sqrt(std::atan(1.0)))));
                        } else {  // some of these operators have an auxillary result;
                ...
                case SubvpOp:
                    ...
                    if (op_usage[random_itr.var2op(size_t(arg[0]))] == usage_t(csum_usage)) {
                        ...
                        size_pair = record_csum(play, random_itr, op_usage, new_par, new_var, i_var, rec, csum_work);
                ...
                case ZmulvpOp:
                    if (previous == 0) {  //
                        size_pair = record_vp(play, random_itr, new_par, new_var, i_op, rec);
                        new_op[i_op] = addr_t(size_pair.i_op);
                        new_var[i_op] = addr_t(size_pair.i_var);
                    }
                    break;
                ...
                case DisOp:
                    ...
                    if (previous == 0) {  //
                        ...
                        new_op[i_op] = addr_t(rec->num_op_rec());
                        new_var[i_op] = rec->PutOp(op);
                        CPPAD_ASSERT_UNKNOWN(new_arg[1] < new_var[random_itr.var2op(i_var)]);
                    }
                    break;
                ...
                case ZmulpvOp:
                    if (previous == 0) {  //
                        size_pair = record_pv(play, random_itr, new_par, new_var, i_op, rec);
                        new_op[i_op] = addr_t(size_pair.i_op);
                        new_var[i_op] = addr_t(size_pair.i_var);
                    }
                    break;
                ...
                case ZmulvvOp:
                    if (previous == 0) {  //
                        size_pair = record_vv(play, random_itr, new_var, i_op, rec);
                        new_op[i_op] = addr_t(size_pair.i_op);
                        new_var[i_op] = addr_t(size_pair.i_var);
                    }
                    break;
                ...
                case CExpOp:
                    ...
                    for (size_t i = 2; i < 6; i++) {
                        if (arg[1] & mask) {
                            ...
                            CPPAD_ASSERT_UNKNOWN(size_t(new_arg[i]) < num_var);
                        } else
                            new_arg[i] = new_par[arg[i]];
                        mask = mask << 1;
                    }
                    rec->PutArg(new_arg[0], new_arg[1], new_arg[2], new_arg[3], new_arg[4], new_arg[5]);
                    new_op[i_op] = addr_t(rec->num_op_rec());
                    new_var[i_op] = rec->PutOp(op);
                    //
                    ...
                    if (conditional_skip) {
                        ...
                        CPPAD_ASSERT_UNKNOWN(size_t(cexp_info[cexp_next].i_op) == i_op);
                ...
                case PriOp:
                    ...
                    //
                    rec->PutArg(new_arg[0], new_arg[1], new_arg[2], new_arg[3], new_arg[4]);
                    // new operator
                    new_op[i_op] = addr_t(rec->num_op_rec());
                    // no new variable
                    rec->PutOp(op);
                ...
                case LdpOp:
                    ...
                    new_arg[1] = arg[1];
                    CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= rec->num_load_op_rec());
                    new_arg[2] = addr_t(rec->num_load_op_rec());
                    CPPAD_ASSERT_UNKNOWN(size_t(new_arg[0]) < num_vecad_ind);
                    rec->PutArg(new_arg[0], new_arg[1], new_arg[2]);
                    new_op[i_op] = addr_t(rec->num_op_rec());
                ...
                case FunapOp:
                    ...
                    --atom_j;
                    if (atom_j == 0) atom_state = ret_atom;
                    break;
                ...
                case FunavOp:
                    ...
                    if (size_t(new_arg[0]) < num_var) {
                        ...
                        new_op[i_op] = addr_t(rec->num_op_rec());
                        rec->PutOp(FunavOp);
                    } else {  // This argument does not affect the result and
                ...
                case FunrpOp:
                    ...
                    CPPAD_ASSERT_NARG_NRES(op, 1, 0);
                    new_arg[0] = new_par[arg[0]];
                    if (new_arg[0] != addr_t_max) {  // This parameter is used, but may not by this operation
                        rec->PutArg(new_arg[0]);
                    } else {  // This parameter is not used here or anywhere.
                    ...
                    --atom_i;
                    if (atom_i == 0) atom_state = end_atom;
                    break;
                ...
                case FunrvOp:
                    ...
                    else {  // change FunrvOp -> FunrpOp to avoid creating new variable
                        CPPAD_ASSERT_UNKNOWN(op_usage[i_op] == usage_t(no_usage));
    ...
    for (i_op = 0; i_op < num_op; i_op++) {
        ...
        if (NumRes(op) > 0) CPPAD_ASSERT_UNKNOWN(size_t(new_op[i_op]) < rec->num_op_rec());
    }
#endif
    ...
    for (size_t i = 0; i < num_cexp; i++) {  // if cskip_new[i].i_arg == 0, this conditional expression was skipped
        if (cskip_new[i].i_arg > 0) {        // size_t i_arg
