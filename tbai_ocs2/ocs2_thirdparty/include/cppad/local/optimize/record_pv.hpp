...
struct_size_pair record_pv(const player<Base> *play, const play::const_random_iterator<Addr> &random_itr,
                           const pod_vector<addr_t> &new_par, const pod_vector<addr_t> &new_var, size_t i_op,
                           recorder<Base> *rec) {
    // get_op_info
    OpCode op;
    const addr_t *arg;
