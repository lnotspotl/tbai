...
struct_size_pair record_csum(const player<Base> *play, const play::const_random_iterator<Addr> &random_itr,
                             const pod_vector<usage_t> &op_usage, const pod_vector<addr_t> &new_par,
                             const pod_vector<addr_t> &new_var, size_t current, recorder<Base> *rec,
...
#ifndef NDEBUG
    bool ok = false;
    if (info.op == SubvpOp) ok = op_usage[random_itr.var2op(size_t(info.arg[0]))] == usage_t(csum_usage);
    //
    if (info.op == AddpvOp || info.op == SubpvOp)
        ok = op_usage[random_itr.var2op(size_t(info.arg[1]))] == usage_t(csum_usage);
    //
    if (info.op == AddvvOp || info.op == SubvvOp) {
        ok = op_usage[random_itr.var2op(size_t(info.arg[0]))] == usage_t(csum_usage);
        //
        ok |= op_usage[random_itr.var2op(size_t(info.arg[1]))] == usage_t(csum_usage);
        //
    }
    ...
    while (!stack.op_info.empty()) {  // get this summation operator
        ...
        // -------------------------------------------------------------------
        switch (op) {  // cases where first argument is a parameter
            case AddpvOp:
            case SubpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < npar);
                //
                if (dyn_par_is[arg[0]]) {  // first argument is a dynamic parameter
                    // (can't yet be a result, so no nodes below)
                    if (add)
                        stack.add_dyn.push(arg[0]);
                    else
                        stack.sub_dyn.push(arg[0]);
                } else {  // first argument is not a dynamic parameter
                    if (add)
                        sum_par += par[arg[0]];
                    else
                        sum_par -= par[arg[0]];
            ...
            case SubvvOp:
                ...
                if (op_usage[random_itr.var2op(size_t(arg[0]))] == usage_t(csum_usage)) {
                    CPPAD_ASSERT_UNKNOWN(size_t(new_var[random_itr.var2op(size_t(arg[0]))]) == 0);
                    ...
                    info.add = add;
                    stack.op_info.push(info);
                } else {  // there are no nodes below this one
        ...
        // -------------------------------------------------------------------
        switch (op) {  // cases where second argument is a parameter
            case SubvpOp:
                // second argument has opposite sign of parent node
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < npar);
                //
                if (dyn_par_is[arg[1]]) {  // second argument is a dynamic parmaeter
                    // (can't yet be a result, so no nodes below)
                    if (add)
                        stack.sub_dyn.push(arg[1]);
                    else
                        stack.add_dyn.push(arg[1]);
                } else {  // second argument is not a dynamic parameter
                    if (add)
                        sum_par -= par[arg[1]];
                    else
                        sum_par += par[arg[1]];
            ...
            case AddpvOp:
                ...
                if (op_usage[random_itr.var2op(size_t(arg[1]))] == usage_t(csum_usage)) {
                    CPPAD_ASSERT_UNKNOWN(size_t(new_var[random_itr.var2op(size_t(arg[1]))]) == 0);
