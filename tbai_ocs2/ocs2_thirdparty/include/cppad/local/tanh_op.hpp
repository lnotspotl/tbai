...
void forward_tanh_op(size_t p, size_t q, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    for (size_t j = p; j <= q; j++) {
        ...
        z[j] = x[j];
        for (k = 1; k <= j; k++) z[j] -= Base(double(k)) * x[k] * y[j - k] / base_j;

        y[j] = z[0] * z[j];
        for (k = 1; k <= j; k++) y[j] += z[k] * z[j - k];
    }
}
...
void forward_tanh_op_dir(size_t q, size_t r, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    for (size_t ell = 0; ell < r; ell++) {
        ...
        for (k = 1; k < q; k++) z[m + ell] -= Base(double(k)) * x[(k - 1) * r + 1 + ell] * y[(q - k - 1) * r + 1 + ell];
        z[m + ell] /= Base(double(q));
        //
        y[m + ell] = Base(2.0) * z[m + ell] * z[0];
        for (k = 1; k < q; k++) y[m + ell] += z[(k - 1) * r + 1 + ell] * z[(q - k - 1) * r + 1 + ell];
    }
}
...
template <class Base>
void reverse_tanh_op(size_t d, size_t i_z, size_t i_x, size_t cap_order, const Base *taylor, size_t nc_partial,
                     Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(TanOp) == 1);
    CPPAD_ASSERT_UNKNOWN(NumRes(TanOp) == 2);
    ...
    while (j) {
        ...
        for (k = 0; k < j; k++) pz[k] += azmul(py[j - 1], z[j - k - 1]) * base_two;
