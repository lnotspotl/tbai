...
class player {
   ...
   public:
    ...
    player(void) : num_dynamic_ind_(0), num_var_rec_(0), num_load_op_rec_(0), num_vecad_vec_rec_(0) {}
    // =================================================================
    /// copy constructor (needed for base2ad)
    player(const player &play) {  // 2DO: want to use move semantics here because it is a temporary
        // in base2ad case.
        *this = play;
    }
    ...
    play::addr_enum address_type(void) const {
        ...
        //
        // unsigned short
        if (required <= std::numeric_limits<unsigned short>::max()) return play::unsigned_short_enum;
        //
        // unsigned int
        if (required <= std::numeric_limits<unsigned int>::max()) return play::unsigned_int_enum;
        //
        // unsigned size_t
        CPPAD_ASSERT_UNKNOWN(required <= std::numeric_limits<size_t>::max());
    ...
    */
#ifdef NDEBUG
    void check_inv_op(size_t n_ind) const { return; }
#else
    void check_inv_op(size_t n_ind) const {
        ...
        size_t i_op = 0;
        while (op != EndOp) {  // start at second operator
            (++itr).op_info(op, op_arg, var_index);
            ++i_op;
    ...
    */
#ifdef NDEBUG
    void check_variable_dag(void) const { return; }
#else
        ...
        while (op != EndOp) {
            ...
            switch (op) {
                ...
                case CSumOp: {
                    ...
                    for (addr_t j = 5; j < op_arg[2]; j++) CPPAD_ASSERT_UNKNOWN(op_arg[j] <= arg_var_bound);
                }
                ...
                // CExpOp
                case CExpOp:
                    if (op_arg[1] & 1) CPPAD_ASSERT_UNKNOWN(op_arg[2] <= arg_var_bound);
                    if (op_arg[1] & 2) CPPAD_ASSERT_UNKNOWN(op_arg[3] <= arg_var_bound);
                    if (op_arg[1] & 4) CPPAD_ASSERT_UNKNOWN(op_arg[4] <= arg_var_bound);
                    if (op_arg[1] & 8) CPPAD_ASSERT_UNKNOWN(op_arg[5] <= arg_var_bound);
                    break;

                // PriOp
                case PriOp:
                    if (op_arg[0] & 1) CPPAD_ASSERT_UNKNOWN(op_arg[1] <= arg_var_bound);
                    if (op_arg[0] & 2) CPPAD_ASSERT_UNKNOWN(op_arg[3] <= arg_var_bound);
                    break;
    ...
    */
#ifdef NDEBUG
    void check_dynamic_dag(void) const { return; }
#else
    void check_dynamic_dag(void) const {  // number of dynamic parameters
        size_t num_dyn = dyn_par_op_.size();
        //
        size_t i_arg = 0;                                   // initialize dynamic parameter argument index
        for (size_t i_dyn = 0; i_dyn < num_dyn; ++i_dyn) {  // i_par is parameter index
            ...
            //
            if ((op == cond_exp_dyn) | (op == dis_dyn)) {  // for these two cases, first argument not a parameter index
                for (size_t i = 1; i < n_arg; ++i) CPPAD_ASSERT_UNKNOWN(dyn_par_arg_[i_arg + i] < i_par);
            } else {
                for (size_t i = 0; i < n_arg; ++i) CPPAD_ASSERT_UNKNOWN(dyn_par_arg_[i_arg + i] < i_par);
            }
            //
    ...
    */
    ...
    player<AD<Base> > base2ad(void) const {
        ...
        for (size_t i = 0; i < all_par_vec_.size(); ++i) play.all_par_vec_[i] = all_par_vec_[i];
        //
        return play;
    }
    // ===============================================================
    ...
    void swap(player &other) {  // size_t objects
    ...
    void setup_random(void) {
        play::random_setup(num_var_rec_, op_vec_, arg_vec_, op2arg_vec_.pod_vector_ptr<Addr>(),
                           op2var_vec_.pod_vector_ptr<Addr>(), var2op_vec_.pod_vector_ptr<Addr>());
    }
    ...
    pod_vector_maybe<Base> &all_par_vec(void) { return all_par_vec_; }
    /// get non-const version of all_par_vec
    const pod_vector_maybe<Base> &all_par_vec(void) const { return all_par_vec_; }
    // ================================================================
    // const functions that retrieve infromation from this player
    // ================================================================
    /// const version of dynamic parameter flag
    const pod_vector<bool> &dyn_par_is(void) const { return dyn_par_is_; }
    /// const version of dynamic parameter index to parameter index
    const pod_vector<addr_t> &dyn_ind2par_ind(void) const { return dyn_ind2par_ind_; }
    /// const version of dynamic parameter operator
    const pod_vector<opcode_t> &dyn_par_op(void) const { return dyn_par_op_; }
    /// const version of dynamic parameter arguments
    const pod_vector<addr_t> &dyn_par_arg(void) const { return dyn_par_arg_; }
    /*!
    \brief
    ...
    */
    OpCode GetOp(size_t i) const { return OpCode(op_vec_[i]); }
    ...
    */
    size_t GetVecInd(size_t i) const { return size_t(vecad_ind_vec_[i]); }
    ...
    */
    Base GetPar(size_t i) const { return all_par_vec_[i]; }
    ...
    */
    const Base *GetPar(void) const { return all_par_vec_.data(); }
    ...
    */
    ...
    size_t num_dynamic_ind(void) const { return num_dynamic_ind_; }

    /// Fetch number of dynamic parameters in the recording
    size_t num_dynamic_par(void) const { return dyn_par_op_.size(); }

    /// Fetch number of dynamic parameters operator arguments in the recording
    size_t num_dynamic_arg(void) const { return dyn_par_arg_.size(); }

    /// Fetch number of variables in the recording.
    size_t num_var_rec(void) const { return num_var_rec_; }

    /// Fetch number of vecad load operations
    size_t num_load_op_rec(void) const { return num_load_op_rec_; }

    /// Fetch number of operators in the recording.
    size_t num_op_rec(void) const { return op_vec_.size(); }

    /// Fetch number of VecAD indices in the recording.
    size_t num_vec_ind_rec(void) const { return vecad_ind_vec_.size(); }

    /// Fetch number of VecAD vectors in the recording
    size_t num_vecad_vec_rec(void) const { return num_vecad_vec_rec_; }

    /// Fetch number of argument indices in the recording.
    size_t num_op_arg_rec(void) const { return arg_vec_.size(); }

    /// Fetch number of parameters in the recording.
    size_t num_par_rec(void) const { return all_par_vec_.size(); }

    /// Fetch number of characters (representing strings) in the recording.
    size_t num_text_rec(void) const { return text_vec_.size(); }
    ...
    size_t size_op_seq(void) const {  // check assumptions made by ad_fun<Base>::size_op_seq()
        ...
        return op_vec_.size() * sizeof(opcode_t) + arg_vec_.size() * sizeof(addr_t) +
               all_par_vec_.size() * sizeof(Base) + dyn_par_is_.size() * sizeof(bool) +
               dyn_ind2par_ind_.size() * sizeof(addr_t) + dyn_par_op_.size() * sizeof(opcode_t) +
               dyn_par_arg_.size() * sizeof(addr_t) + text_vec_.size() * sizeof(char) +
    ...
    size_t size_random(void) const {
        ...
        return op2arg_vec_.size() + op2var_vec_.size() + var2op_vec_.size();
    }
    // -----------------------------------------------------------------------
    ...
    play::const_sequential_iterator begin(void) const {
        ...
        return play::const_sequential_iterator(num_var, &op_vec_, &arg_vec_, op_index);
    }
    ...
    play::const_subgraph_iterator<addr_t> begin_subgraph(const play::const_random_iterator<addr_t> &random_itr,
                                                         const pod_vector<addr_t> *subgraph) const {
        size_t subgraph_index = 0;
        return play::const_subgraph_iterator<addr_t>(random_itr, subgraph, subgraph_index);
    }
    /// const subgraph iterator end
    template <class Addr>
    play::const_subgraph_iterator<Addr> end_subgraph(const play::const_random_iterator<Addr> &random_itr,
                                                     const pod_vector<addr_t> *subgraph) const {
        size_t subgraph_index = subgraph->size() - 1;
        return play::const_subgraph_iterator<Addr>(random_itr, subgraph, subgraph_index);
    }
    // -----------------------------------------------------------------------
    /// const random iterator
    template <class Addr>
    play::const_random_iterator<Addr> get_random(void) const {
        return play::const_random_iterator<Addr>(op_vec_, arg_vec_, op2arg_vec_.pod_vector_ptr<Addr>(),
