...
class const_sequential_iterator {
    ...
    */
    const_sequential_iterator(size_t num_var, const pod_vector<opcode_t> *op_vec, const pod_vector<addr_t> *arg_vec,
                              size_t op_index)
    ...
    */
    void correct_before_increment(void) {  // number of arguments for this operator depends on argument data
        CPPAD_ASSERT_UNKNOWN(NumArg(op_) == 0);
        const addr_t *arg = arg_;
        //
        // CSumOp
        if (op_ == CSumOp) {  // add actual number of arguments to arg_
            arg_ += arg[4] + 1;
        }
        //
        // CSkip
    ...
    */
    void correct_after_decrement(
        const addr_t *&arg) {  // number of arguments for this operator depends on argument data
        CPPAD_ASSERT_UNKNOWN(NumArg(op_) == 0);
        //
    ...
    */
    void op_info(OpCode &op, const addr_t *&arg,
    ...
    size_t op_index(void) { return size_t(op_cur_ - op_begin_); }
};
