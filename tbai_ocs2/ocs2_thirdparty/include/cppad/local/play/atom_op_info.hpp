...
template <class Base>
atomic_base<Base> *atom_op_info(const OpCode op, const addr_t *op_arg, size_t &atom_index, size_t &atom_old,
                                size_t &atom_m, size_t &atom_n) {
    ...
    if (type == 3) return CPPAD_NULL;
#ifndef NDEBUG
    if (v_ptr == CPPAD_NULL) {  // atom_fun is null so cannot use atom_fun->afun_name()
        std::string msg = atomic_base<Base>::class_name(atom_index) + ": atomic_base function has been deleted";
