...
class pod_vector {
   private:
    ...
    explicit pod_vector(const pod_vector &) { CPPAD_ASSERT_UNKNOWN(false); }

   public:
    /// default constructor sets byte_capacity_ = byte_length_ = data_ = 0
    pod_vector(void) : byte_capacity_(0), byte_length_(0), data_(CPPAD_NULL) { CPPAD_ASSERT_UNKNOWN(is_pod<Type>()); }
    ...
    */
    ...
    size_t size(void) const { return byte_length_ / sizeof(Type); }

    /// current capacity (amount of allocated storage) for this vector.
    size_t capacity(void) const { return byte_capacity_ / sizeof(Type); }
    ...
    */
    size_t extend(size_t n) {
        ...
        if (byte_length_ <= byte_capacity_) return old_length / sizeof(Type);
        ...
        if (old_length > 0) std::memcpy(v_ptr, old_v_ptr, old_length);

        // return old memory to available pool
        if (old_capacity > 0) thread_alloc::return_memory(old_v_ptr);
    ...
    */
    void resize(size_t n) {
        ...
        if (byte_capacity_ < byte_length_) {
            void *v_ptr;
            //
            if (byte_capacity_ > 0) {  // return old memory to available pool
...
class pod_vector_maybe {
   private:
    ...
    /// do not use the copy constructor
    explicit pod_vector_maybe(const pod_vector_maybe &) { CPPAD_ASSERT_UNKNOWN(false); }

   public:
    /// default constructor sets capacity_ = length_ = data_ = 0
    pod_vector_maybe(void) : capacity_(0), length_(0), data_(CPPAD_NULL) { CPPAD_ASSERT_UNKNOWN(is_pod<size_t>()); }
    ...
    ~pod_vector_maybe(void) {
        if (capacity_ > 0) {
            if (!is_pod<Type>()) {  // call destructor for each element
                for (size_t i = 0; i < capacity_; i++) (data_ + i)->~Type();
            }
            void *v_ptr = reinterpret_cast<void *>(data_);
            thread_alloc::return_memory(v_ptr);
        }
    ...
    */
    size_t extend(size_t n) {
        ...
        // check if we can use current memory
        if (length_ <= capacity_) return old_length;
        ...
        if (!is_pod<Type>()) {  // call constructor for each new element
            for (size_t i = 0; i < capacity_; i++) new (data_ + i) Type();
        }
        ...
        if (old_capacity > 0) {
            if (!is_pod<Type>()) {
                for (size_t i = 0; i < old_capacity; i++) (old_data + i)->~Type();
            }
    ...
    */
    void resize(size_t n) {
        ...
        if (capacity_ < length_) {
            ...
            // return old memory to available pool
            if (capacity_ > 0) {
                if (!is_pod<Type>()) {  // call destructor for each old element
                    for (size_t i = 0; i < capacity_; i++) (data_ + i)->~Type();
                }
                v_ptr = reinterpret_cast<void *>(data_);
                thread_alloc::return_memory(v_ptr);
