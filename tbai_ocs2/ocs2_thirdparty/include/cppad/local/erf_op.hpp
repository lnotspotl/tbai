...
*/
template <class Base>
void forward_erf_op(size_t p, size_t q, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                    Base *taylor) {
    // check assumptions
    ...
    CPPAD_ASSERT_UNKNOWN(size_t(std::numeric_limits<addr_t>::max()) >= i_z + 2);
    ...
    for (size_t j = p; j <= q; j++) {  // z_4' (t) = erf'[x(t)] * x'(t) = z3(t) * x'(t)
        // z_4[1] + 2 * z_4[2] * t +  ... =
        // (z_3[0] + z_3[1] * t +  ...) * (x[1] + 2 * x[2] * t +  ...)
        Base base_j = static_cast<Base>(double(j));
        z_4[j] = static_cast<Base>(0);
        for (size_t k = 1; k <= j; k++) z_4[j] += (Base(double(k)) / base_j) * x[k] * z_3[j - k];
    }
}
...
*/
template <class Base>
void forward_erf_op_dir(size_t q, size_t r, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
                        Base *taylor) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(ErfOp) == 3);
    CPPAD_ASSERT_UNKNOWN(NumRes(ErfOp) == 5);
    ...
    for (size_t ell = 0; ell < r; ell++) {  // index in z_4 and x for q-th order term
...
*/
template <class Base>
void reverse_erf_op(size_t d, size_t i_z, const addr_t *arg, const Base *parameter, size_t cap_order,
    ...
    bool skip(true);
    for (size_t i_d = 0; i_d <= d; i_d++) skip &= IdenticalZero(pz[i_d]);
    if (skip) return;
    ...
    addr[0] = arg[2];           // 2 / sqrt(pi)
    addr[1] = addr_t(i_z + 2);  // z_2
    reverse_mulpv_op(d, i_z + 3, addr, parameter, cap_order, taylor, nc_partial, partial);
    ...
    addr[0] = arg[1];       // zero
    addr[1] = addr_t(i_z);  // z_0
    reverse_subpv_op(d, i_z + 1, addr, parameter, cap_order, taylor, nc_partial, partial);

    // z_0 = x * x
    addr[0] = arg[0];  // x
    addr[1] = arg[0];  // x
    reverse_mulvv_op(d, i_z + 0, addr, parameter, cap_order, taylor, nc_partial, partial);
}
