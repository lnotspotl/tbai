...
void forward_sign_op(size_t p, size_t q, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    for (size_t j = p; j <= q; j++) z[j] = Base(0.);
}
/*!
...
void forward_sign_op_dir(size_t q, size_t r, size_t i_z, size_t i_x, size_t cap_order, Base *taylor) {
    ...
    for (size_t ell = 0; ell < r; ell++) z[m + ell] = Base(0.);
}
...
template <class Base>
void reverse_sign_op(size_t d, size_t i_z, size_t i_x, size_t cap_order, const Base *taylor, size_t nc_partial,
                     Base *partial) {
    // check assumptions
    CPPAD_ASSERT_UNKNOWN(NumArg(SignOp) == 1);
    CPPAD_ASSERT_UNKNOWN(NumRes(SignOp) == 1);
