...
ErrorType InEKF::getErrorType() const {
    return error_type_;
}
...
void InEKF::setMagneticField(Eigen::Vector3d &true_magnetic_field) {
    magnetic_field_ = true_magnetic_field;
}

// Get the true magnetic field
Eigen::Vector3d InEKF::getMagneticField() const {
...
Eigen::MatrixXd InEKF::StateTransitionMatrix(Eigen::Vector3d &w, Eigen::Vector3d &a, double dt) {
    ...
    Eigen::Matrix3d G1 =
        Gamma_SO3(phi, 1);  // TODO: These are also needed for the mean propagation, we should not compute twice
...
Eigen::MatrixXd InEKF::DiscreteNoiseMatrix(Eigen::MatrixXd &Phi, double dt) {
    ...
    for (map<int, int>::iterator it = estimated_contact_positions_.begin(); it != estimated_contact_positions_.end();
         ++it) {
        Qc.block<3, 3>(3 + 3 * (it->second - 3), 3 + 3 * (it->second - 3)) =
            noise_params_.getContactCov();  // Contact noise terms
    }  // TODO: Use kinematic orientation to map noise from contact frame to body frame (not needed if noise is
       // isotropic)
    ...
    Eigen::MatrixXd Qd =
        PhiG * Qc * PhiG.transpose() * dt;  // Approximated discretized noise matrix (TODO: compute analytical)
    return Qd;
}
    ...
    for (vectorKinematicsIterator it = measured_kinematics.begin(); it != measured_kinematics.end(); ++it) {
        ...
        if (!contact_indicated && found) {
            ...
            N.block(startIndex, startIndex, 3, 3) =
                state_.getWorldRotation() * it->covariance.block<3, 3>(3, 3) * state_.getWorldRotation().transpose();
        ...
        for (vectorKinematicsIterator it = new_contacts.begin(); it != new_contacts.end(); ++it) {
            ...
            if (state_.getStateType() == StateType::WorldCentric) {
                X_aug.block(0, startIndex, 3, 1) =
                    state_.getPosition() + state_.getRotation() * it->pose.block<3, 1>(0, 3);
            } else {
            ...
            F.block(state_.dimP() - state_.dimTheta() + 3, state_.dimP() - state_.dimTheta(), state_.dimTheta(),
                    state_.dimTheta()) = Eigen::MatrixXd::Identity(state_.dimTheta(), state_.dimTheta());  // for theta
...
void InEKF::CorrectLandmarks(const vectorLandmarks &measured_landmarks) {
    ...
    for (vectorLandmarksIterator it = measured_landmarks.begin(); it != measured_landmarks.end(); ++it) {
        ...
        if (it_prior != prior_landmarks_.end()) {
            ...
            N.block(0, startIndex, startIndex, 3) = Eigen::MatrixXd::Zero(startIndex, 3);
            N.block(startIndex, startIndex, 3, 3) =
                state_.getWorldRotation() * it->covariance * state_.getWorldRotation().transpose();
...
void InEKF::CorrectPosition(const Eigen::Vector3d &measured_position, const Eigen::Matrix3d &covariance,
...
void InEKF::CorrectContactPosition(const int id, const Eigen::Vector3d &measured_contact_position,
                                   const Eigen::Matrix3d &covariance, const Eigen::Vector3d &indices) {
