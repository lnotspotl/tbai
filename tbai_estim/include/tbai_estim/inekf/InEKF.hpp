...
#include <Eigen/Dense>
#include <tbai_estim/inekf/LieGroup.hpp>
#include <tbai_estim/inekf/NoiseParams.hpp>
...
class InEKF {
   public:
    ...
    /// @{
    /**
     * Default Constructor. Initializes the filter with default state (identity rotation, zero velocity, zero position)
...
the default.
     ...
     * Gets the current state estimate.
     */
    RobotState &getState() { return state_; }
     ...
     * If contact is indicated but not included in the state, the state is augmented to include the estimated contact
     * position. If contact is not indicated but is included in the state, the contact position is marginalized out of
     * the state. This is a right-invariant measurement model. Example usage can be found in @include kinematics.cpp
     * @param measured_kinematics: the measured kinematics containing the contact id, relative pose measurement in the
     * IMU frame, and covariance
     */
     ...
     * @param measured_landmarks: the measured landmarks containing the contact id, relative position measurement in the
     * IMU frame, and covariance
     */
    ...
    void CorrectPosition(const Eigen::Vector3d &measured_position, const Eigen::Matrix3d &covariance,
                         const Eigen::Vector3d &indices);
    /** TODO: Untested contact position measurement*/
    void CorrectContactPosition(const int id, const Eigen::Vector3d &measured_contact_position,
